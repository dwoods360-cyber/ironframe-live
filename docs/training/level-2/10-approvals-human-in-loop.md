...
> **Target length:** ~1600 words · **Primary route:** `/dashboard/admin/approvals`

## Learning objectives

After completing this chapter, you will be able to navigate to `/dashboard/admin/approvals`, execute the prescribed task flow, and verify HITL-001 controls using tenant-scoped session context.

## Feature location in the SaaS application

| Attribute | Value |
|-----------|-------|
| Primary route | `/dashboard/admin/approvals` |
| Capture route | `/dashboard/admin/approvals` |
| GRC function IDs | HITL-001 |
...
2. Review pending draft queue
3. DISPATCH approved outbound email via admin action
4. PURGE soft-archived entries per queue policy

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Navigate to /dashboard/admin/approvals | /dashboard/admin/approvals |
| 2 | Review pending draft queue | /dashboard/admin/approvals |
| 3 | DISPATCH approved outbound email via admin action | /dashboard/admin/approvals |
| 4 | PURGE soft-archived entries per queue policy | /dashboard/admin/approvals |
## Reference screenshot

![Chapter 10 — Human-in-the-Loop Approvals & Email Dispatch](/docs/training/assets/level-2-10-approvals-human-in-loop.png)

*Figure: Chapter 10 — Human-in-the-Loop Approvals & Email Dispatch — captured at `/dashboard/admin/approvals`. Asset path: `/docs/training/assets/level-2-10-approvals-human-in-loop.png`.*
...
2. Navigate to /dashboard/admin/approvals
3. Locate the feature in the SaaS UI at route `/dashboard/admin/approvals`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. DISPATCH approved outbound email via admin action
3. Locate the feature in the SaaS UI at route `/dashboard/admin/approvals`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. PURGE soft-archived entries per queue policy
3. Locate the feature in the SaaS UI at route `/dashboard/admin/approvals`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: HITL-001

This procedure validates HITL-001 against the live Ironframe workspace at `/dashboard/admin/approvals`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Navigate to /dashboard/admin/approvals
...
Cross-reference `docs/qa/complete-feature-glossary.md` for HITL-001 before submitting the lab.

### Procedure 2: HITL-001

This procedure validates HITL-001 against the live Ironframe workspace at `/dashboard/admin/approvals`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Review pending draft queue
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for HITL-001 before submitting the lab.

### Procedure 3: HITL-001

This procedure validates HITL-001 against the live Ironframe workspace at `/dashboard/admin/approvals`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: DISPATCH approved outbound email via admin action
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for HITL-001 before submitting the lab.

### Procedure 4: HITL-001

This procedure validates HITL-001 against the live Ironframe workspace at `/dashboard/admin/approvals`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: PURGE soft-archived entries per queue policy
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for HITL-001 before submitting the lab.

### Procedure 5: HITL-001
...
A: Use primary route `/dashboard/admin/approvals`, follow the navigation path table in this chapter, and archive screenshot `level-2-10-approvals-human-in-loop.png`.
...
- [ ] HITL-001 — procedure completed and screenshot archived
...
- source-file: app/lib/server/approvalQueueCore.ts
- source-file: app/api/admin/approvals
...
ref: config/training-corpus-manifest.json · slug=training/level-2/10-approvals-human-in-loop