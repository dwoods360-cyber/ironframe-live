...
After completing this chapter, you will be able to navigate to `/integrity`, execute the prescribed task flow, and verify ARCH-001 controls using tenant-scoped session context.
...
1. Review Ironframe :3000 vs IronBoard :8082 separation
2. Trace GET /api/board/shared-context ingress
3. Trace POST /api/documentation/execute documentation sync (IronBoard → app_documents)
4. Validate fail-closed 502 on telemetry disconnect

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Review Ironframe :3000 vs IronBoard :8082 separation | /integrity |
| 2 | Trace GET /api/board/shared-context ingress | /integrity |
| 3 | Trace POST /api/documentation/execute documentation sync (IronBoard → app_documents) | /integrity |
| 4 | Validate fail-closed 502 on telemetry disconnect | /integrity |
## Reference screenshot

![Chapter 1 — System Architecture & Dual-Host Topology](/docs/training/assets/level-2-01-architecture-topology.png)

*Figure: Chapter 1 — System Architecture & Dual-Host Topology — captured at `/integrity`. Asset path: `/docs/training/assets/level-2-01-architecture-topology.png`.*
...
2. Review Ironframe :3000 vs IronBoard :8082 separation
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Trace POST /api/documentation/execute documentation sync (IronBoard → app_documents)
3. Locate the feature in the SaaS UI at route `/integrity`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Validate fail-closed 502 on telemetry disconnect
3. Locate the feature in the SaaS UI at route `/integrity`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: ARCH-001

This procedure validates ARCH-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Review Ironframe :3000 vs IronBoard :8082 separation
...
Cross-reference `docs/qa/complete-feature-glossary.md` for ARCH-001 before submitting the lab.

### Procedure 2: ARCH-001

This procedure validates ARCH-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Trace GET /api/board/shared-context ingress
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for ARCH-001 before submitting the lab.

### Procedure 3: ARCH-001

This procedure validates ARCH-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Trace POST /api/documentation/execute documentation sync (IronBoard → app_documents)
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for ARCH-001 before submitting the lab.

### Procedure 4: ARCH-001

This procedure validates ARCH-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Validate fail-closed 502 on telemetry disconnect
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for ARCH-001 before submitting the lab.

### Procedure 5: ARCH-001
...
A: Use primary route `/integrity`, follow the navigation path table in this chapter, and archive screenshot `level-2-01-architecture-topology.png`.
...
- [ ] ARCH-001 — procedure completed and screenshot archived
...
ref: config/training-corpus-manifest.json · slug=training/level-2/01-architecture-topology