...
describe("parseBriefingCitations", () => {
  ...
  it("parses numbered confirmation-style citation lists", () => {
...
`;
    ...
    expect(citations.some((c) => c.locator.includes("aipolicydesk.com"))).toBe(true);
    expect(citations.some((c) => c.locator.includes("cisa.gov/circia"))).toBe(true);
  });
});
