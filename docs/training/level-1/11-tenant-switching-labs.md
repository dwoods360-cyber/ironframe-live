...
After completing this chapter, you will be able to navigate to `/integrity`, execute the prescribed task flow, and verify TENANT-001 / AUTH-001 controls using tenant-scoped session context.
...
| GRC function IDs | TENANT-001, AUTH-001 |
...
1. Use tenant switcher in TopNav (when authorized for multiple tenants)
2. Switch Medshield → Vaultbank → Gridcore
3. Confirm exposure baselines change per tenant profile
4. Verify no cross-tenant data bleed in network responses

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Use tenant switcher in TopNav (when authorized for multiple tenants) | /integrity |
| 2 | Switch Medshield → Vaultbank → Gridcore | /integrity |
| 3 | Confirm exposure baselines change per tenant profile | /integrity |
| 4 | Verify no cross-tenant data bleed in network responses | /integrity |
## Reference screenshot

![Chapter 11 — Tenant Switching & Isolation Labs (TENANT-001)](/docs/training/assets/level-1-11-tenant-switching-labs.png)

*Figure: Chapter 11 — Tenant Switching & Isolation Labs (TENANT-001) — captured at `/integrity`. Asset path: `/docs/training/assets/level-1-11-tenant-switching-labs.png`.*
...
2. Use tenant switcher in TopNav (when authorized for multiple tenants)
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Confirm exposure baselines change per tenant profile
3. Locate the feature in the SaaS UI at route `/integrity`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Verify no cross-tenant data bleed in network responses
3. Locate the feature in the SaaS UI at route `/integrity`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: TENANT-001

This procedure validates TENANT-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Use tenant switcher in TopNav (when authorized for multiple tenants)
...
Cross-reference `docs/qa/complete-feature-glossary.md` for TENANT-001 before submitting the lab.

### Procedure 2: AUTH-001

This procedure validates AUTH-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Switch Medshield → Vaultbank → Gridcore
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for AUTH-001 before submitting the lab.

### Procedure 3: TENANT-001

This procedure validates TENANT-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Confirm exposure baselines change per tenant profile
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for TENANT-001 before submitting the lab.
...
This procedure validates AUTH-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Verify no cross-tenant data bleed in network responses
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for AUTH-001 before submitting the lab.

### Procedure 5: TENANT-001
...
A: Use primary route `/integrity`, follow the navigation path table in this chapter, and archive screenshot `level-1-11-tenant-switching-labs.png`.
...
- [ ] TENANT-001 — procedure completed and screenshot archived
...
ref: config/training-corpus-manifest.json · slug=training/level-1/11-tenant-switching-labs