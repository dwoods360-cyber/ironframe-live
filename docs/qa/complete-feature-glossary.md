...
* **Technical Mechanics:**
  ...
  * SaaS KB delegation: capacity/client-limit questions resolve through `lookupSaasCallKnowledge()` — Path B **1** Primary + **2** Subtenant enclave entitlement; board/heatmap questions use `ale-reporting` id with whole-cent estimated exposure ranges (2026-07-31 control-to-capital alignment)
...
Today's code delta touches the following agents. Use this matrix during audits to confirm boundary integrity for operational date **2026-07-31**:

| Agent # | Codename | Today's Delta Touchpoints |
|---------|----------|-------------------------|
| 1 | Ironcore | Control-to-Capital docs federation in IronBoard matrix; Heatmap Amnesty public landing `/marketing/heatmap-amnesty`; MarketingHomepage copy realignment; operator library catalog expansion (ops-surface-map, control-to-capital, heatmap-amnesty); perimeter Docker `docs/` COPY; prior-cycle Ironleads SUSPECT location, workflow-review LIVE, tri-track HITL, Ops Calendar checklist retained |
| 3 | Irontrust | Constitutional seed baselines unchanged (**1110000000**, **590000000**, **470000000**, **1600000000**); Path B anchor **499900** cents unchanged in `.fingerprint.json`; whole-cent estimated exposure language on public surfaces — no float persistence; July 17 OSINT `riskMetricsCents` retained (**4580000000**, **1015000000**, **762000000**, **143000000**); `priorityScore` explicitly non-monetary integer tier |
| 4 | Ironlogic | IronBoard docs matrix now includes control-to-capital claim bans for board synthesis grounding; `saasCallKnowledgeBase.ts` board-math answer aligned with Mandate 16; prior-cycle `isPerimeterWorkforceHealthQuery` and `BOARD_OPERATOR_PROSE_MANDATE` retained |
| 5 | Ironscribe | Control-to-capital narrative is ACTIVE sales doc — not a quarantined briefing-queue draft; gf-product-boundary desk pass should reject manuscripts violating claim bans from prior cycle |
| 13 | Ironwatch | Product knowledge fingerprint rotation logged via `knowledge:check`; deploy-perimeter-workers workflow triggers on `docs/**` changes |
| 14 | Irongate | Public marketing surfaces remain narrow-funnel unauthenticated paths — no tenant bleed on Heatmap Amnesty; prior-cycle Ironleads ingress and suspect report gates retained |
| 15 | Ironquery | Prior-cycle canonical docs hub location responses retained — no tripane layout invention for `/docs` queries |
| 16 | Ironintel | July 17 manifest retained — no new OSINT sweep in today's delta; Heatmap Amnesty campaign targets CISO/CFO board-pack pain aligned with strategic intel framing without inventing FAIR mandate |

**Control-to-Capital federation note (2026-07-31):** `docs/sales/control-to-capital-market-narrative.md` is READ-federated into IronBoard at runtime from `resolveDocsRoot()`. Perimeter Cloud Run images must include `docs/` tree — `.dockerignore` negation and Dockerfile COPY are mandatory for claim-ban grounding off-repo. Operators edit claim sheet locally, commit, redeploy perimeter workers per `PRODUCT_KNOWLEDGE_BLAST_RADIUS`.
...
| Test file | Validates |
|-----------|-----------|
| `tests/unit/workflowReviewCallAssistCore.test.ts` | SaaS KB capacity entitlement answers (no platform technical ceiling / commercial entitlement capped); Primary Entity + Subtenant enclave framing; banNote SaaS KB citation; SOC 2 / free PoC / HITL pocket Q&A locks |
...
## ✅ Chapter 9: Daily Writer Receipt (2026-07-31)

**Delta classification:** UI (`HeatmapAmnestyLanding.tsx`, `MarketingHomepage.tsx` copy + nav, `tailwind.config.ts` fadeIn animation) + Public Routes (`app/marketing/heatmap-amnesty/page.tsx`) + Operator Library (`operatorLibraryCatalog.ts` catalog expansion and slug aliases) + Perimeter Workers (IronBoard `buildDocsFederationMatrix()` control-to-capital federation, Dockerfile `COPY docs`, `.dockerignore` docs negation, `deploy-perimeter-workers.yml` path triggers) + Product Knowledge (`saasCallKnowledgeBase.ts` board-math answer, `.fingerprint.json` SHA rotation with unchanged cent anchors) + Backend Logic (retained `workflowReviewCallAssistCore.ts` SaaS KB delegation — test assertions updated for capacity entitlement language).

**Financial boundary verification:** All ALE references in this document use BigInt integer cents exclusively for persistence and internal telemetry. Constitutional Ironframe seed tenant baselines unchanged: Medshield **1110000000**, Vaultbank **590000000**, Gridcore **470000000**, Defense **1600000000** cents. Design Partner Path B on-ramp documented at **499900** cents (**$4,999**) via `DESIGN_PARTNER_PATH_B_CENTS` in `lib/ironframeProductKnowledge/commercial.ts` — single code truth; `.fingerprint.json` `pathBCents: "499900"` anchor verified **2026-07-30**. Planned GA Command **3500000** cents/yr (**$35,000**/yr) via `PLANNED_GA_COMMAND_CENTS`. Planned GA Growth **7500000** cents/yr (**$75,000**/yr) via `PLANNED_GA_GROWTH_CENTS`. July 17 OSINT `riskMetricsCents`: median annual GRC program **4580000000**, audit remediation lag **1015000000**, SaaS consolidation savings **762000000**, board reporting overhead **143000000** cents. Public marketing copy cites **estimated** dollar-loss exposure in whole cents — not persisted tenant ALE on unauthenticated routes. Path B enclave entitlement: **1** Primary Entity + **2** Subtenant Enclaves for cohort window — commercial cap, not a platform technical ceiling. Stripe `amountTotalCents` and tenant `ale_baseline BIGINT` remain authoritative cent ingress.

**Threat simulation verification:** No new `SimThreatEvent` or OSINT manifest ingestion in today's delta. Heatmap Amnesty campaign aligns with board-pack / capital-conversation pain framing without inventing regulatory FAIR mandates. July 17 OSINT manifest (`ironintel-osint-2026-07-17-live`) FortiSandbox triple-CVE FCEB deadline **July 19** has passed — operators should verify Ironintel dashboard reflects post-deadline remediation posture. Chaos engineering and `mitigated_value_cents BIGINT` paths unchanged. Shadow-plane self-test controls remain gated on `isSimulationMode === true` per TAS Section 4.3.

**Tenant isolation verification:** Heatmap Amnesty and marketing homepage are guest-only public surfaces — no authenticated tenant cookie required, no cross-tenant telemetry render. IronBoard docs federation reads global sales enablement markdown — no tenant-scoped financial rows in matrix. Prior-cycle suspect forensic and tri-track approval isolation retained.

**GTM quarantine verification:** Control-to-capital and heatmap-amnesty campaign docs are ACTIVE sales plane markdown — not briefing-queue drafts requiring promote. Email sequence in `heatmap-amnesty-campaign.md` remains HITL-only per Mandate 10. No change to promote/hold/deny gates for Governance Frame briefing queue.

**Product knowledge verification:** `.fingerprint.json` `commercialSha256` rotated to `6abf19902082d64da4b63b3de63be135359ac02702c9553b500a598f2913d2fe` at `2026-07-30T15:37:25.236Z` — run `npm run knowledge:check` after any further `commercial.ts` edit. Mandate 16 claim bans federated to IronBoard for board synthesis grounding. CI `knowledge:check` and `test:product-knowledge` run before Postgres bootstrap per `.github/workflows/ci.yml`.

**CI verification:** Updated `tests/unit/workflowReviewCallAssistCore.test.ts` for SaaS KB capacity answers; retained suites from 2026-07-15 through 2026-07-20 remain mandatory. Perimeter worker redeploy required after `docs/sales/control-to-capital-market-narrative.md` or Dockerfile changes. All suites listed in Chapter 7 must pass before deploy.

**Empty-diff pivot:** Not applicable — `daily_code_diff.txt` contains substantive deltas across IronBoard federation, public marketing landing, operator library, product knowledge spine, SaaS call KB, Tailwind animation, and workflow-review test assertions. Primary modules: `Ironboard/src/index.ts`, `HeatmapAmnestyLanding.tsx`, `MarketingHomepage.tsx`, `operatorLibraryCatalog.ts`, `saasCallKnowledgeBase.ts`, `.fingerprint.json`, perimeter Docker/deploy workflow.

---

*End of GRC Master Operations Manual & Technical Feature Glossary — Writer Narrative Architect complete mandate execution for operational date 2026-07-31.*
