...
After completing this chapter, you will be able to navigate to `/cockpit`, execute the prescribed task flow, and verify AGENT-001 controls using tenant-scoped session context.
...
2. Identify active agent coordination panels
3. Map agent IDs 01–19 to glossary entries
4. Note IronBoard bridge status indicators when present

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Navigate to /cockpit | /cockpit |
| 2 | Identify active agent coordination panels | /cockpit |
| 3 | Map agent IDs 01–19 to glossary entries | /cockpit |
| 4 | Note IronBoard bridge status indicators when present | /cockpit |
## Reference screenshot

![Chapter 6 — Cockpit & 19-Agent Workforce Viewport](/docs/training/assets/level-1-06-cockpit-agent-viewport.png)

*Figure: Chapter 6 — Cockpit & 19-Agent Workforce Viewport — captured at `/cockpit`. Asset path: `/docs/training/assets/level-1-06-cockpit-agent-viewport.png`.*
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Map agent IDs 01–19 to glossary entries
3. Locate the feature in the SaaS UI at route `/cockpit`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Note IronBoard bridge status indicators when present
3. Locate the feature in the SaaS UI at route `/cockpit`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: AGENT-001

This procedure validates AGENT-001 against the live Ironframe workspace at `/cockpit`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Navigate to /cockpit
...
Cross-reference `docs/qa/complete-feature-glossary.md` for AGENT-001 before submitting the lab.

### Procedure 2: AGENT-001

This procedure validates AGENT-001 against the live Ironframe workspace at `/cockpit`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Identify active agent coordination panels
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for AGENT-001 before submitting the lab.

### Procedure 3: AGENT-001

This procedure validates AGENT-001 against the live Ironframe workspace at `/cockpit`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Map agent IDs 01–19 to glossary entries
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for AGENT-001 before submitting the lab.

### Procedure 4: AGENT-001

This procedure validates AGENT-001 against the live Ironframe workspace at `/cockpit`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Note IronBoard bridge status indicators when present
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for AGENT-001 before submitting the lab.

### Procedure 5: AGENT-001
...
A: Use primary route `/cockpit`, follow the navigation path table in this chapter, and archive screenshot `level-1-06-cockpit-agent-viewport.png`.
...
- [ ] AGENT-001 — procedure completed and screenshot archived
...
ref: config/training-corpus-manifest.json · slug=training/level-1/06-cockpit-agent-viewport