...
After completing this chapter, you will be able to navigate to `/dashboard/support`, execute the prescribed task flow, and verify SALES-001 / CS-001 controls using tenant-scoped session context.
...
| GRC function IDs | SALES-001, CS-001 |
...
1. Review /register/contact public intake
2. Open /sales-agent-portal for grounded pitch workflow
3. Open /dashboard/support for CS pending drafts
4. Trace SYSTEM_AGENT CRM metadata on contact records

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Review /register/contact public intake | /dashboard/support |
| 2 | Open /sales-agent-portal for grounded pitch workflow | /dashboard/support |
| 3 | Open /dashboard/support for CS pending drafts | /dashboard/support |
| 4 | Trace SYSTEM_AGENT CRM metadata on contact records | /dashboard/support |
...
2. Review /register/contact public intake
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Open /dashboard/support for CS pending drafts
3. Locate the feature in the SaaS UI at route `/dashboard/support`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Trace SYSTEM_AGENT CRM metadata on contact records
3. Locate the feature in the SaaS UI at route `/dashboard/support`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
Execute: Review /register/contact public intake
...
This procedure validates CS-001 against the live Ironframe workspace at `/dashboard/support`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Open /sales-agent-portal for grounded pitch workflow
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for CS-001 before submitting the lab.

### Procedure 3: SALES-001

This procedure validates SALES-001 against the live Ironframe workspace at `/dashboard/support`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Open /dashboard/support for CS pending drafts
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for SALES-001 before submitting the lab.
...
This procedure validates CS-001 against the live Ironframe workspace at `/dashboard/support`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Trace SYSTEM_AGENT CRM metadata on contact records
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for CS-001 before submitting the lab.
...
- [ ] CS-001 — procedure completed and screenshot archived
...
- source-file: Ironboard/src/agents/sales/playbook.ts
