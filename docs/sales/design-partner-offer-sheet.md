# Design-partner offer sheet (send-safe)

**Audience:** CISO / VP Infra / Compliance lead · co-builder cohort (3–5 seats)  
**Do not include:** demo tenant names (`medshield`, `vaultbank`, `gridcore`), “free pilot,” or “fastest SOC 2.”

---

## Offer (one breath)

Ironframe is recruiting a **small paid co-builder cohort** for the **Command Tier** quantitative GRC command post — board-grade dollar risk, tenant-isolated enclaves, auditor-ready exports. Not another checkbox automation shelf.

| Item | Term |
|------|------|
| **Commercial** | Path B / Command Tier on-ramp **$4,999** (one price, sales-assisted) |
| **Planned GA** | Ironframe Command **~$35,000/yr** (`FINTECH_SEED`) — say “planned GA” until commercial GA flag is on |
| **Window** | **90-day** default (floor 60 if scoped in writing) → convert or exit cleanly |
| **Convert credit** | If converting **in-window**, Path B **$4,999** credits year-1 Command at list (~$35k) — fixed credit, **not** a negotiated %. Exit = fee **non-refundable** |
| **Eng syncs** | Weekly **capped** (first 4–6 weeks), then async — protects scope freeze |
| **Success criteria** | Agree **2–3** up front (examples below) |
| **CTA** | **10–15 min workflow review** on their evidence / board-report pain — not a product preview ([protocol](./design-partner-workflow-review-protocol.md)) |

## Example success criteria (pick 2–3)

1. Produce a clean **Ironquery analyst export** usable in a board pack or audit binder.  
2. Cut vendor / security **questionnaire turnaround** with governed evidence paths.  
3. Stand up **BigInt ALE** for one primary entity the board can defend in dollars.  
4. Prove **tenant isolation** for multi-entity / MSSP-style client boundaries.

## Channel blurbs (copy-paste)

### Warm network / advisor ask

> Who do you know standing up SOC 2, ISO, or ESG ops who is stuck in spreadsheets or heatmap GRC — and would value a **paid co-builder seat** on a quantitative command post ($4,999 Path B, 90-day window, capped eng syncs)? Intro welcome; we run a 10–15 min workflow review first, not a demo circus.

### Auditor / consultant intro

> Looking for 1–2 mid-market operators where audit evidence and board dollar-risk still live in spreadsheets. Ironframe offers a **paid design-partner Command Tier** ($4,999) with bounded syncs and a clear convert-or-exit window — useful when you need a referenceable quantified stack, not another checklist tool. Happy to keep you in the loop after one clean pilot.

### Cold co-builder open (subject options)

- Co-builder seat: Command Tier ($4,999) — workflow review, not a demo  
- Locked pilot for quantified GRC — 90-day default window, roadmap influence  

## Path after “yes”

1. HITL outreach → `/dashboard/admin/approvals` DISPATCH  
2. Quick-provision → **client-owned** operator email + **Path B activation link** (never generic `/pricing` for PENDING)  
3. Partner pays → ACTIVE → `/get-started` (ALE + primary GRC company)  
4. Log success criteria on the order form

**Runbook:** [Design partner recruitment](./design-partner-recruitment.md) · [Order form](./design-partner-order-form.md) · [Outreach sequence](./design-partner-outreach-sequence.md)
