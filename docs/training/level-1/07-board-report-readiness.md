# Chapter 7 — Board Report & Executive Readiness Compile

> **Track:** LEVEL_1 · **Author agent:** board-trainer · **Release:** `v0.1.0-ga-epic17`
> **Target length:** ~1600 words · **Primary route:** `/board-report`

## Learning objectives

After completing this chapter, you will be able to navigate to `/board-report`, execute the prescribed task flow, and verify REPORT-001 controls using tenant-scoped session context.

## Feature location in the SaaS application

| Attribute | Value |
|-----------|-------|
| Primary route | `/board-report` |
| Capture route | `/board-report` |
| GRC function IDs | REPORT-001 |
...
1. Open /board-report
2. Review compiled platform posture sections
3. Compare DORA readiness percentage with /integrity signals
4. Export or snapshot findings for classroom submission

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Open /board-report | /board-report |
| 2 | Review compiled platform posture sections | /board-report |
| 3 | Compare DORA readiness percentage with /integrity signals | /board-report |
| 4 | Export or snapshot findings for classroom submission | /board-report |
## Reference screenshot

![Chapter 7 — Board Report & Executive Readiness Compile](/docs/training/assets/level-1-07-board-report-readiness.png)

*Figure: Chapter 7 — Board Report & Executive Readiness Compile — captured at `/board-report`. Asset path: `/docs/training/assets/level-1-07-board-report-readiness.png`.*

source-file: public/docs/training/assets/level-1-07-board-report-readiness.png
...
2. Open /board-report
3. Locate the feature in the SaaS UI at route `/board-report`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Compare DORA readiness percentage with /integrity signals
3. Locate the feature in the SaaS UI at route `/board-report`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Export or snapshot findings for classroom submission
3. Locate the feature in the SaaS UI at route `/board-report`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: REPORT-001

This procedure validates REPORT-001 against the live Ironframe workspace at `/board-report`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Open /board-report
...
Cross-reference `docs/qa/complete-feature-glossary.md` for REPORT-001 before submitting the lab.

### Procedure 2: REPORT-001

This procedure validates REPORT-001 against the live Ironframe workspace at `/board-report`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Review compiled platform posture sections
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for REPORT-001 before submitting the lab.

### Procedure 3: REPORT-001

This procedure validates REPORT-001 against the live Ironframe workspace at `/board-report`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Compare DORA readiness percentage with /integrity signals
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for REPORT-001 before submitting the lab.

### Procedure 4: REPORT-001

This procedure validates REPORT-001 against the live Ironframe workspace at `/board-report`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Export or snapshot findings for classroom submission
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for REPORT-001 before submitting the lab.

### Procedure 5: REPORT-001
...
A: Use primary route `/board-report`, follow the navigation path table in this chapter, and archive screenshot `level-1-07-board-report-readiness.png`.
...
- [ ] REPORT-001 — procedure completed and screenshot archived
...
- source-file: docs/stakeholders/technical-requirements.md
...
ref: config/training-corpus-manifest.json · slug=training/level-1/07-board-report-readiness