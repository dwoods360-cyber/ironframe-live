...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!resolveWriterApiKey()) {
      console.error("[Writer Agent] Missing Gemini orchestration credentials.");
      return NextResponse.json(
        { error: "Writer engine offline. Intelligence cluster keys unassigned." },
        { status: 503 },
      );
    }
    ...
    console.error("[Writer Agent] Critical routing boundary exception:", err);
    return NextResponse.json(
      { error: "Internal Writer Routing Error", details },
      { status: 500 },
    );
  }
}
