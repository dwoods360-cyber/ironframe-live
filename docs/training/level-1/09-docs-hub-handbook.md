...
1. Open /docs — content served from PostgreSQL app_documents
2. If Compilation Ingress Portal appears, run npx tsx scripts/seed-app-documents.ts
3. Browse user-manuals/quickstart from sidebar
4. Open /docs/training/level1-student-index
...
|------|--------|---------------|
| 1 | Open /docs — content served from PostgreSQL app_documents | /docs |
| 2 | If Compilation Ingress Portal appears, run npx tsx scripts/seed-app-documents.ts | /docs |
...
2. Open /docs — content served from PostgreSQL app_documents
...
Execute: Open /docs — content served from PostgreSQL app_documents
...
This procedure validates DOCS-001 against the live Ironframe workspace at `/docs`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: If Compilation Ingress Portal appears, run npx tsx scripts/seed-app-documents.ts
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for DOCS-001 before submitting the lab.
