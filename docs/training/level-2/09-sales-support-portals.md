...
> **Target length:** ~1600 words · **Primary route:** `/dashboard/support`

## Learning objectives

After completing this chapter, you will be able to navigate to `/dashboard/support`, execute the prescribed task flow, and verify SALES-001 / SUPPORT-001 controls using tenant-scoped session context.

## Feature location in the SaaS application

| Attribute | Value |
|-----------|-------|
| Primary route | `/dashboard/support` |
| Capture route | `/register/contact` |
| GRC function IDs | SALES-001, SUPPORT-001 |
...
1. Submit lead via /sales-agent-portal or POST /api/agents/sales
2. Verify API returns { status: "QUEUED" } — no public pitch text is rendered
3. Open /dashboard/support; submit operator query under Ironguard tenant scope
4. Verify CS console returns queued acknowledgment and CRM [PENDING DRAFT APPROVAL]

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Submit lead via /sales-agent-portal or POST /api/agents/sales | /dashboard/support |
| 2 | Verify API returns { status: "QUEUED" } — no public pitch text is rendered | /dashboard/support |
| 3 | Open /dashboard/support; submit operator query under Ironguard tenant scope | /dashboard/support |
| 4 | Verify CS console returns queued acknowledgment and CRM [PENDING DRAFT APPROVAL] | /dashboard/support |
## Reference screenshot

![Chapter 9 — Sales Agent Portal & Support Workflows](/docs/training/assets/level-2-09-sales-support-portals.png)

*Figure: Chapter 9 — Sales Agent Portal & Support Workflows — captured at `/register/contact`. Asset path: `/docs/training/assets/level-2-09-sales-support-portals.png`.*
...
2. Submit lead via /sales-agent-portal or POST /api/agents/sales
3. Locate the feature in the SaaS UI at route `/dashboard/support`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Open /dashboard/support; submit operator query under Ironguard tenant scope
3. Locate the feature in the SaaS UI at route `/dashboard/support`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Verify CS console returns queued acknowledgment and CRM [PENDING DRAFT APPROVAL]
3. Locate the feature in the SaaS UI at route `/dashboard/support`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: SALES-001

This procedure validates SALES-001 against the live Ironframe workspace at `/dashboard/support`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Submit lead via /sales-agent-portal or POST /api/agents/sales
...
Cross-reference `docs/qa/complete-feature-glossary.md` for SALES-001 before submitting the lab.

### Procedure 2: SUPPORT-001

This procedure validates SUPPORT-001 against the live Ironframe workspace at `/dashboard/support`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Verify API returns { status: "QUEUED" } — no public pitch text is rendered
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for SUPPORT-001 before submitting the lab.

### Procedure 3: SALES-001

This procedure validates SALES-001 against the live Ironframe workspace at `/dashboard/support`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Open /dashboard/support; submit operator query under Ironguard tenant scope
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for SALES-001 before submitting the lab.

### Procedure 4: SUPPORT-001

This procedure validates SUPPORT-001 against the live Ironframe workspace at `/dashboard/support`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Verify CS console returns queued acknowledgment and CRM [PENDING DRAFT APPROVAL]
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for SUPPORT-001 before submitting the lab.

### Procedure 5: SALES-001
...
A: Use primary route `/dashboard/support`, follow the navigation path table in this chapter, and archive screenshot `level-2-09-sales-support-portals.png`.
...
- [ ] SALES-001 — procedure completed and screenshot archived
- [ ] SUPPORT-001 — procedure completed and screenshot archived
...
- source-file: app/api/agents/sales/route.ts
- source-file: app/api/agents/customer-service/route.ts
- source-file: app/lib/server/salesAgentConsoleCore.ts
- source-file: app/lib/server/customerServiceConsoleCore.ts
...
ref: config/training-corpus-manifest.json · slug=training/level-2/09-sales-support-portals