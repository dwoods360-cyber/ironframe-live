...
function buildTrainerDraft(state: typeof BoardStateAnnotation.State): string {
  return [
    ...
    "",
    "Baseline cents (whole-integer): Medshield 1110000000, Vaultbank 590000000, Gridcore 470000000.",
  ].join("\n");
}
...
function buildWriterDraft(state: typeof BoardStateAnnotation.State): string {
  return [
    ...
    `<p>Financial integrity: amount_cents: 1110000000 (Medshield baseline)</p>`,
