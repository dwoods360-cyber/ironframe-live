...
> **Target length:** ~1600 words · **Primary route:** `/cockpit`

## Learning objectives

After completing this chapter, you will be able to navigate to `/cockpit`, execute the prescribed task flow, and verify BRIDGE-001 controls using tenant-scoped session context.

## Feature location in the SaaS application

| Attribute | Value |
|-----------|-------|
| Primary route | `/cockpit` |
| Capture route | `/cockpit` |
| GRC function IDs | BRIDGE-001 |
...
1. Ensure IronBoard listening on http://127.0.0.1:8082
2. Verify GET /api/board/shared-context on :3000
3. Trigger boardroom query requiring live telemetry
4. Confirm CORE_TELEMETRY_DISCONNECTED on bridge failure

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Ensure IronBoard listening on http://127.0.0.1:8082 | /cockpit |
| 2 | Verify GET /api/board/shared-context on :3000 | /cockpit |
| 3 | Trigger boardroom query requiring live telemetry | /cockpit |
| 4 | Confirm CORE_TELEMETRY_DISCONNECTED on bridge failure | /cockpit |
## Reference screenshot

![Chapter 6 — IronBoard Core Telemetry Bridge](/docs/training/assets/level-2-06-ironboard-telemetry-bridge.png)

*Figure: Chapter 6 — IronBoard Core Telemetry Bridge — captured at `/cockpit`. Asset path: `/docs/training/assets/level-2-06-ironboard-telemetry-bridge.png`.*
...
2. Ensure IronBoard listening on http://127.0.0.1:8082
3. Locate the feature in the SaaS UI at route `/cockpit`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Trigger boardroom query requiring live telemetry
3. Locate the feature in the SaaS UI at route `/cockpit`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Confirm CORE_TELEMETRY_DISCONNECTED on bridge failure
3. Locate the feature in the SaaS UI at route `/cockpit`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: BRIDGE-001

This procedure validates BRIDGE-001 against the live Ironframe workspace at `/cockpit`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Ensure IronBoard listening on http://127.0.0.1:8082
...
Cross-reference `docs/qa/complete-feature-glossary.md` for BRIDGE-001 before submitting the lab.

### Procedure 2: BRIDGE-001

This procedure validates BRIDGE-001 against the live Ironframe workspace at `/cockpit`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Verify GET /api/board/shared-context on :3000
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for BRIDGE-001 before submitting the lab.

### Procedure 3: BRIDGE-001

This procedure validates BRIDGE-001 against the live Ironframe workspace at `/cockpit`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Trigger boardroom query requiring live telemetry
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for BRIDGE-001 before submitting the lab.

### Procedure 4: BRIDGE-001

This procedure validates BRIDGE-001 against the live Ironframe workspace at `/cockpit`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Confirm CORE_TELEMETRY_DISCONNECTED on bridge failure
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for BRIDGE-001 before submitting the lab.

### Procedure 5: BRIDGE-001
...
A: Use primary route `/cockpit`, follow the navigation path table in this chapter, and archive screenshot `level-2-06-ironboard-telemetry-bridge.png`.
...
- [ ] BRIDGE-001 — procedure completed and screenshot archived
...
- source-file: Ironboard/src/services/coreTelemetryBridge.ts
...
ref: config/training-corpus-manifest.json · slug=training/level-2/06-ironboard-telemetry-bridge