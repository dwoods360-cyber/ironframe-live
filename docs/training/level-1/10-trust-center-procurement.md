# Chapter 10 — Trust Center & Procurement Materials

> **Track:** LEVEL_1 · **Author agent:** board-trainer · **Release:** `v0.1.0-ga-epic17`
> **Target length:** ~1600 words · **Primary route:** `/trust`

## Learning objectives

After completing this chapter, you will be able to navigate to `/trust`, execute the prescribed task flow, and verify TRUST-001 controls using tenant-scoped session context.
...
1. Navigate to /trust
2. Review subprocessor and residency charts
3. Locate enterprise procurement download links
4. Document subprocessors: Vercel, Supabase, Resend per DPA annex

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Navigate to /trust | /trust |
| 2 | Review subprocessor and residency charts | /trust |
| 3 | Locate enterprise procurement download links | /trust |
| 4 | Document subprocessors: Vercel, Supabase, Resend per DPA annex | /trust |
## Reference screenshot

![Chapter 10 — Trust Center & Procurement Materials](/docs/training/assets/level-1-10-trust-center-procurement.png)

*Figure: Chapter 10 — Trust Center & Procurement Materials — captured at `/trust`. Asset path: `/docs/training/assets/level-1-10-trust-center-procurement.png`.*

source-file: public/docs/training/assets/level-1-10-trust-center-procurement.png
...
2. Navigate to /trust
3. Locate the feature in the SaaS UI at route `/trust`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Locate enterprise procurement download links
3. Locate the feature in the SaaS UI at route `/trust`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Document subprocessors: Vercel, Supabase, Resend per DPA annex
3. Locate the feature in the SaaS UI at route `/trust`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: TRUST-001

This procedure validates TRUST-001 against the live Ironframe workspace at `/trust`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Navigate to /trust
...
Cross-reference `docs/qa/complete-feature-glossary.md` for TRUST-001 before submitting the lab.

### Procedure 2: TRUST-001

This procedure validates TRUST-001 against the live Ironframe workspace at `/trust`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Review subprocessor and residency charts
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for TRUST-001 before submitting the lab.

### Procedure 3: TRUST-001

This procedure validates TRUST-001 against the live Ironframe workspace at `/trust`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Locate enterprise procurement download links
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for TRUST-001 before submitting the lab.

### Procedure 4: TRUST-001

This procedure validates TRUST-001 against the live Ironframe workspace at `/trust`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Document subprocessors: Vercel, Supabase, Resend per DPA annex
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for TRUST-001 before submitting the lab.

### Procedure 5: TRUST-001
...
A: Use primary route `/trust`, follow the navigation path table in this chapter, and archive screenshot `level-1-10-trust-center-procurement.png`.
...
- [ ] TRUST-001 — procedure completed and screenshot archived
...
- source-file: docs/technical/security-and-compliance.md
...
ref: config/training-corpus-manifest.json · slug=training/level-1/10-trust-center-procurement