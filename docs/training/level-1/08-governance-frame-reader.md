...
> **Target length:** ~1600 words · **Primary route:** `/governance-frame`

## Learning objectives

After completing this chapter, you will be able to navigate to `/governance-frame`, execute the prescribed task flow, and verify BRIEF-001 controls using tenant-scoped session context.

## Feature location in the SaaS application

| Attribute | Value |
|-----------|-------|
| Primary route | `/governance-frame` |
| Capture route | `/governance-frame` |
| GRC function IDs | BRIEF-001 |
...
1. Navigate to /governance-frame (session required)
2. Open published briefing index
3. Select a slug under /governance-frame/[slug]
4. Contrast with APP_DOCS plane at /docs — never cross-write planes

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Navigate to /governance-frame (session required) | /governance-frame |
| 2 | Open published briefing index | /governance-frame |
| 3 | Select a slug under /governance-frame/[slug] | /governance-frame |
| 4 | Contrast with APP_DOCS plane at /docs — never cross-write planes | /governance-frame |
## Reference screenshot

![Chapter 8 — Governance Frame Briefing Reader](/docs/training/assets/level-1-08-governance-frame-reader.png)

*Figure: Chapter 8 — Governance Frame Briefing Reader — captured at `/governance-frame`. Asset path: `/docs/training/assets/level-1-08-governance-frame-reader.png`.*

source-file: public/docs/training/assets/level-1-08-governance-frame-reader.png
## Hands-on lab sequence

### Lab 1: BRIEF-001

1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Navigate to /governance-frame (session required)
3. Locate the feature in the SaaS UI at route `/governance-frame`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Select a slug under /governance-frame/[slug]
3. Locate the feature in the SaaS UI at route `/governance-frame`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Contrast with APP_DOCS plane at /docs — never cross-write planes
3. Locate the feature in the SaaS UI at route `/governance-frame`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: BRIEF-001

This procedure validates BRIEF-001 against the live Ironframe workspace at `/governance-frame`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Navigate to /governance-frame (session required)
...
Cross-reference `docs/qa/complete-feature-glossary.md` for BRIEF-001 before submitting the lab.

### Procedure 2: BRIEF-001

This procedure validates BRIEF-001 against the live Ironframe workspace at `/governance-frame`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Open published briefing index
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for BRIEF-001 before submitting the lab.

### Procedure 3: BRIEF-001

This procedure validates BRIEF-001 against the live Ironframe workspace at `/governance-frame`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Select a slug under /governance-frame/[slug]
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for BRIEF-001 before submitting the lab.

### Procedure 4: BRIEF-001

This procedure validates BRIEF-001 against the live Ironframe workspace at `/governance-frame`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Contrast with APP_DOCS plane at /docs — never cross-write planes
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for BRIEF-001 before submitting the lab.

### Procedure 5: BRIEF-001
...
A: Use primary route `/governance-frame`, follow the navigation path table in this chapter, and archive screenshot `level-1-08-governance-frame-reader.png`.
...
- [ ] BRIEF-001 — procedure completed and screenshot archived
...
- source-file: lib/documentationCorpusPlanes.ts
- source-file: docs/briefing-queue/README.md
...
ref: config/training-corpus-manifest.json · slug=training/level-1/08-governance-frame-reader