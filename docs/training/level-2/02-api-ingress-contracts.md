...
> **Target length:** ~1600 words · **Primary route:** `/docs/technical/architecture-and-api`

## Learning objectives

After completing this chapter, you will be able to navigate to `/docs/technical/architecture-and-api`, execute the prescribed task flow, and verify API-001 controls using tenant-scoped session context.

## Feature location in the SaaS application

| Attribute | Value |
|-----------|-------|
| Primary route | `/docs/technical/architecture-and-api` |
| Capture route | `/docs/technical/architecture-and-api` |
| GRC function IDs | API-001 |
...
2. Map POST /api/sustainability/ironbloom physical-unit gate
3. Review CRM SYSTEM_AGENT channel metadata schema
4. Confirm no float cents in API examples

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Open /docs/technical/architecture-and-api | /docs/technical/architecture-and-api |
| 2 | Map POST /api/sustainability/ironbloom physical-unit gate | /docs/technical/architecture-and-api |
| 3 | Review CRM SYSTEM_AGENT channel metadata schema | /docs/technical/architecture-and-api |
| 4 | Confirm no float cents in API examples | /docs/technical/architecture-and-api |
## Reference screenshot

![Chapter 2 — API Ingress Contracts & BigInt Schema](/docs/training/assets/level-2-02-api-ingress-contracts.png)

*Figure: Chapter 2 — API Ingress Contracts & BigInt Schema — captured at `/docs/technical/architecture-and-api`. Asset path: `/docs/training/assets/level-2-02-api-ingress-contracts.png`.*
...
2. Open /docs/technical/architecture-and-api
3. Locate the feature in the SaaS UI at route `/docs/technical/architecture-and-api`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Review CRM SYSTEM_AGENT channel metadata schema
3. Locate the feature in the SaaS UI at route `/docs/technical/architecture-and-api`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Confirm no float cents in API examples
3. Locate the feature in the SaaS UI at route `/docs/technical/architecture-and-api`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
This procedure validates API-001 against the live Ironframe workspace at `/docs/technical/architecture-and-api`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Open /docs/technical/architecture-and-api
...
Cross-reference `docs/qa/complete-feature-glossary.md` for API-001 before submitting the lab.

### Procedure 2: API-001

This procedure validates API-001 against the live Ironframe workspace at `/docs/technical/architecture-and-api`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Map POST /api/sustainability/ironbloom physical-unit gate
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for API-001 before submitting the lab.
...
This procedure validates API-001 against the live Ironframe workspace at `/docs/technical/architecture-and-api`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Review CRM SYSTEM_AGENT channel metadata schema
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for API-001 before submitting the lab.
...
This procedure validates API-001 against the live Ironframe workspace at `/docs/technical/architecture-and-api`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Confirm no float cents in API examples
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for API-001 before submitting the lab.
...
A: Use primary route `/docs/technical/architecture-and-api`, follow the navigation path table in this chapter, and archive screenshot `level-2-02-api-ingress-contracts.png`.
...
- [ ] API-001 — procedure completed and screenshot archived
...
ref: config/training-corpus-manifest.json · slug=training/level-2/02-api-ingress-contracts