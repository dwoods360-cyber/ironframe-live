...
After completing this chapter, you will be able to navigate to `/integrity`, execute the prescribed task flow, and verify FIN-001 / FIN-002 controls using tenant-scoped session context.
...
| GRC function IDs | FIN-001, FIN-002 |
...
1. Open /integrity
2. Locate sovereign pool baseline cards (Medshield, Vaultbank, Gridcore)
3. Read active exposure formatted string (never raw BigInt in UI copy)
4. Record maturity score and critical threat indicators in lab journal

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Open /integrity | /integrity |
| 2 | Locate sovereign pool baseline cards (Medshield, Vaultbank, Gridcore) | /integrity |
| 3 | Read active exposure formatted string (never raw BigInt in UI copy) | /integrity |
| 4 | Record maturity score and critical threat indicators in lab journal | /integrity |
## Reference screenshot

![Chapter 4 — Integrity Hub & ALE Financial Posture](/docs/training/assets/level-1-04-integrity-hub-ale.png)

*Figure: Chapter 4 — Integrity Hub & ALE Financial Posture — captured at `/integrity`. Asset path: `/docs/training/assets/level-1-04-integrity-hub-ale.png`.*
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Read active exposure formatted string (never raw BigInt in UI copy)
3. Locate the feature in the SaaS UI at route `/integrity`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Record maturity score and critical threat indicators in lab journal
3. Locate the feature in the SaaS UI at route `/integrity`.
4. Record observations in your lab journal with timestamp 2026-06-19T15:48:50.176Z.
5. Cross-check against source anchors before marking complete.
...
This procedure validates FIN-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Open /integrity
...
Cross-reference `docs/qa/complete-feature-glossary.md` for FIN-001 before submitting the lab.

### Procedure 2: FIN-002

This procedure validates FIN-002 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Locate sovereign pool baseline cards (Medshield, Vaultbank, Gridcore)
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for FIN-002 before submitting the lab.

### Procedure 3: FIN-001

This procedure validates FIN-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Read active exposure formatted string (never raw BigInt in UI copy)
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for FIN-001 before submitting the lab.
...
This procedure validates FIN-002 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Record maturity score and critical threat indicators in lab journal
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-19T15:48:50.176Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for FIN-002 before submitting the lab.
...
A: Use primary route `/integrity`, follow the navigation path table in this chapter, and archive screenshot `level-1-04-integrity-hub-ale.png`.
...
- [ ] FIN-001 — procedure completed and screenshot archived
- [ ] FIN-002 — procedure completed and screenshot archived
...
ref: config/training-corpus-manifest.json · slug=training/level-1/04-integrity-hub-ale