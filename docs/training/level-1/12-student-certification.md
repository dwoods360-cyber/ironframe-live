# Chapter 12 — Level 1 Student Certification & Lab Sign-Off

> **Track:** LEVEL_1 · **Author agent:** board-trainer · **Release:** `v0.1.0-ga-epic17`
> **Target length:** ~1600 words · **Primary route:** `/docs/training/level-1/12-student-certification`

## Learning objectives

After completing this chapter, you will be able to navigate to `/docs/training/level-1/12-student-certification`, execute the prescribed task flow, and verify CERT-L1 controls using tenant-scoped session context.

## Feature location in the SaaS application

| Attribute | Value |
|-----------|-------|
| Primary route | `/docs/training/level-1/12-student-certification` |
| Capture route | `/docs` |
| GRC function IDs | CERT-L1 |
...
1. Complete AUTH-001, TENANT-001, EXPORT-001 lab sequence
2. Review all Level 1 chapters in order
3. Submit classroom attestation with tenant UUID and timestamp
4. Archive screenshots from /docs/training/assets/

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Complete AUTH-001, TENANT-001, EXPORT-001 lab sequence | /docs/training/level-1/12-student-certification |
| 2 | Review all Level 1 chapters in order | /docs/training/level-1/12-student-certification |
| 3 | Submit classroom attestation with tenant UUID and timestamp | /docs/training/level-1/12-student-certification |
| 4 | Archive screenshots from /docs/training/assets/ | /docs/training/level-1/12-student-certification |
## Reference screenshot

![Chapter 12 — Level 1 Student Certification & Lab Sign-Off](/docs/training/assets/level-1-12-student-certification.png)

*Figure: Chapter 12 — Level 1 Student Certification & Lab Sign-Off — captured at `/docs`. Asset path: `/docs/training/assets/level-1-12-student-certification.png`.*

source-file: public/docs/training/assets/level-1-12-student-certification.png
## Hands-on lab sequence

### Lab 1: CERT-L1

1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Complete AUTH-001, TENANT-001, EXPORT-001 lab sequence
3. Locate the feature in the SaaS UI at route `/docs/training/level-1/12-student-certification`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Submit classroom attestation with tenant UUID and timestamp
3. Locate the feature in the SaaS UI at route `/docs/training/level-1/12-student-certification`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Archive screenshots from /docs/training/assets/
3. Locate the feature in the SaaS UI at route `/docs/training/level-1/12-student-certification`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
This procedure validates CERT-L1 against the live Ironframe workspace at `/docs/training/level-1/12-student-certification`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Complete AUTH-001, TENANT-001, EXPORT-001 lab sequence
...
Cross-reference `docs/qa/complete-feature-glossary.md` for CERT-L1 before submitting the lab.

### Procedure 2: CERT-L1

This procedure validates CERT-L1 against the live Ironframe workspace at `/docs/training/level-1/12-student-certification`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Review all Level 1 chapters in order
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for CERT-L1 before submitting the lab.
...
This procedure validates CERT-L1 against the live Ironframe workspace at `/docs/training/level-1/12-student-certification`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Submit classroom attestation with tenant UUID and timestamp
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for CERT-L1 before submitting the lab.
...
This procedure validates CERT-L1 against the live Ironframe workspace at `/docs/training/level-1/12-student-certification`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Archive screenshots from /docs/training/assets/
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for CERT-L1 before submitting the lab.
...
A: Use primary route `/docs/training/level-1/12-student-certification`, follow the navigation path table in this chapter, and archive screenshot `level-1-12-student-certification.png`.
...
- [ ] CERT-L1 — procedure completed and screenshot archived
...
- source-file: docs/qa/student-testing-protocol.md
...
ref: config/training-corpus-manifest.json · slug=training/level-1/12-student-certification