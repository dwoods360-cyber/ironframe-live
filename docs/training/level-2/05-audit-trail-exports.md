...
> **Target length:** ~1600 words · **Primary route:** `/reports/audit-trail`

## Learning objectives

After completing this chapter, you will be able to navigate to `/reports/audit-trail`, execute the prescribed task flow, and verify AUDIT-001 controls using tenant-scoped session context.

## Feature location in the SaaS application

| Attribute | Value |
|-----------|-------|
| Primary route | `/reports/audit-trail` |
| Capture route | `/reports/audit-trail` |
| GRC function IDs | AUDIT-001 |
...
2. Generate tenant-scoped CSV export
3. Verify CVE tokens sanitized in export prose
4. Cross-reference Ironscribe audit lineage anchors

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Open /reports/audit-trail | /reports/audit-trail |
| 2 | Generate tenant-scoped CSV export | /reports/audit-trail |
| 3 | Verify CVE tokens sanitized in export prose | /reports/audit-trail |
| 4 | Cross-reference Ironscribe audit lineage anchors | /reports/audit-trail |
## Reference screenshot

![Chapter 5 — Audit Trail Reports & Forensic Exports](/docs/training/assets/level-2-05-audit-trail-exports.png)

*Figure: Chapter 5 — Audit Trail Reports & Forensic Exports — captured at `/reports/audit-trail`. Asset path: `/docs/training/assets/level-2-05-audit-trail-exports.png`.*
...
3. Locate the feature in the SaaS UI at route `/reports/audit-trail`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Verify CVE tokens sanitized in export prose
3. Locate the feature in the SaaS UI at route `/reports/audit-trail`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Cross-reference Ironscribe audit lineage anchors
3. Locate the feature in the SaaS UI at route `/reports/audit-trail`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: AUDIT-001

This procedure validates AUDIT-001 against the live Ironframe workspace at `/reports/audit-trail`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Open /reports/audit-trail
...
Cross-reference `docs/qa/complete-feature-glossary.md` for AUDIT-001 before submitting the lab.

### Procedure 2: AUDIT-001

This procedure validates AUDIT-001 against the live Ironframe workspace at `/reports/audit-trail`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Generate tenant-scoped CSV export
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for AUDIT-001 before submitting the lab.

### Procedure 3: AUDIT-001

This procedure validates AUDIT-001 against the live Ironframe workspace at `/reports/audit-trail`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Verify CVE tokens sanitized in export prose
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for AUDIT-001 before submitting the lab.

### Procedure 4: AUDIT-001

This procedure validates AUDIT-001 against the live Ironframe workspace at `/reports/audit-trail`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Cross-reference Ironscribe audit lineage anchors
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for AUDIT-001 before submitting the lab.

### Procedure 5: AUDIT-001
...
A: Use primary route `/reports/audit-trail`, follow the navigation path table in this chapter, and archive screenshot `level-2-05-audit-trail-exports.png`.
...
- [ ] AUDIT-001 — procedure completed and screenshot archived
...
- source-file: app/lib/reports/governanceTriadSanitizer.ts
- source-file: docs/qa/manual-testing-protocol.md
...
ref: config/training-corpus-manifest.json · slug=training/level-2/05-audit-trail-exports