...
> **Target length:** ~1600 words · **Primary route:** `/admin/onboarding`

## Learning objectives

After completing this chapter, you will be able to navigate to `/admin/onboarding`, execute the prescribed task flow, and verify ADMIN-001 controls using tenant-scoped session context.

## Feature location in the SaaS application

| Attribute | Value |
|-----------|-------|
| Primary route | `/admin/onboarding` |
| Capture route | `/admin/onboarding` |
| GRC function IDs | ADMIN-001 |
...
3. Review corporate tenant provision flow
4. Validate billing gate exemption on this route

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Sign in as GLOBAL_ADMIN | /admin/onboarding |
| 2 | Navigate to /admin/onboarding | /admin/onboarding |
| 3 | Review corporate tenant provision flow | /admin/onboarding |
| 4 | Validate billing gate exemption on this route | /admin/onboarding |
## Reference screenshot

![Chapter 8 — B2B Admin Onboarding & Tenant Provisioning](/docs/training/assets/level-2-08-admin-onboarding-provisioning.png)

*Figure: Chapter 8 — B2B Admin Onboarding & Tenant Provisioning — captured at `/admin/onboarding`. Asset path: `/docs/training/assets/level-2-08-admin-onboarding-provisioning.png`.*
...
2. Sign in as GLOBAL_ADMIN
3. Locate the feature in the SaaS UI at route `/admin/onboarding`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Review corporate tenant provision flow
3. Locate the feature in the SaaS UI at route `/admin/onboarding`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Validate billing gate exemption on this route
3. Locate the feature in the SaaS UI at route `/admin/onboarding`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: ADMIN-001

This procedure validates ADMIN-001 against the live Ironframe workspace at `/admin/onboarding`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Sign in as GLOBAL_ADMIN
...
Cross-reference `docs/qa/complete-feature-glossary.md` for ADMIN-001 before submitting the lab.

### Procedure 2: ADMIN-001

This procedure validates ADMIN-001 against the live Ironframe workspace at `/admin/onboarding`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Navigate to /admin/onboarding
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for ADMIN-001 before submitting the lab.

### Procedure 3: ADMIN-001

This procedure validates ADMIN-001 against the live Ironframe workspace at `/admin/onboarding`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Review corporate tenant provision flow
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for ADMIN-001 before submitting the lab.

### Procedure 4: ADMIN-001

This procedure validates ADMIN-001 against the live Ironframe workspace at `/admin/onboarding`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Validate billing gate exemption on this route
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for ADMIN-001 before submitting the lab.

### Procedure 5: ADMIN-001
...
A: Use primary route `/admin/onboarding`, follow the navigation path table in this chapter, and archive screenshot `level-2-08-admin-onboarding-provisioning.png`.
...
- [ ] ADMIN-001 — procedure completed and screenshot archived
...
- source-file: app/lib/server/corporateTenantProvisionCore.ts
...
ref: config/training-corpus-manifest.json · slug=training/level-2/08-admin-onboarding-provisioning