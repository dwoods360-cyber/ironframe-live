...
After completing this chapter, you will be able to navigate to `/integrity`, execute the prescribed task flow, and verify ESG-001 controls using tenant-scoped session context.
...
1. Locate Sustainability Pulse panel (right rail, 30% column)
2. Read kWh and fluid liters from shared-context payload
3. Attempt rejected monetary-only sustainability packet in lab sandbox
4. Document Ironwatch heartbeat degradation flags

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Locate Sustainability Pulse panel (right rail, 30% column) | /integrity |
| 2 | Read kWh and fluid liters from shared-context payload | /integrity |
| 3 | Attempt rejected monetary-only sustainability packet in lab sandbox | /integrity |
| 4 | Document Ironwatch heartbeat degradation flags | /integrity |
...
*Figure: Chapter 7 — Sustainability Ingress (Ironbloom Physical Units) — captured at `/integrity`. Asset path: `/docs/training/assets/level-2-07-sustainability-ironbloom.png`.*
...
2. Locate Sustainability Pulse panel (right rail, 30% column)
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Attempt rejected monetary-only sustainability packet in lab sandbox
3. Locate the feature in the SaaS UI at route `/integrity`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Document Ironwatch heartbeat degradation flags
3. Locate the feature in the SaaS UI at route `/integrity`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
This procedure validates ESG-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Locate Sustainability Pulse panel (right rail, 30% column)
...
Cross-reference `docs/qa/complete-feature-glossary.md` for ESG-001 before submitting the lab.

### Procedure 2: ESG-001

This procedure validates ESG-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Read kWh and fluid liters from shared-context payload
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for ESG-001 before submitting the lab.
...
This procedure validates ESG-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Attempt rejected monetary-only sustainability packet in lab sandbox
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for ESG-001 before submitting the lab.
...
This procedure validates ESG-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Document Ironwatch heartbeat degradation flags
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for ESG-001 before submitting the lab.
...
A: Use primary route `/integrity`, follow the navigation path table in this chapter, and archive screenshot `level-2-07-sustainability-ironbloom.png`.
...
- [ ] ESG-001 — procedure completed and screenshot archived
...
ref: config/training-corpus-manifest.json · slug=training/level-2/07-sustainability-ironbloom