...
export type GovernanceNewsletterIssueSpec = {
  filename: string;
  issueLabel: string;
  focus: string;
};

export type RequestGovernanceNewsletterSeriesInput = {
  /** Operator prompt describing the Ironcast / Substack newsletter series. */
  requestPrompt: string;
  /** Default series title prefix. */
  seriesTitle: string;
  issues: GovernanceNewsletterIssueSpec[];
...
function buildSystemPrompt(): string {
  return `
You are the Ironframe Ironcast newsletter authorship plane (board-bot / board-cfo style).
You draft PUBLIC newsletter issues for quarantine in docs/briefing-queue/.
These drafts are later human-promoted to Governance Frame and compiled by Ironcast into email/Substack HTML.

${buildPublicBriefingAuthoringMandate(PUBLIC_BRIEFING_DECLASSIFICATION_MANDATE)}

NEWSLETTER-SPECIFIC CONSTRAINTS:
- Output ONE markdown document only for the requested issue.
- Include YAML frontmatter with title, date (ISO date), status QUARANTINED_DRAFT, classification Institutional Governance, category newsletter, tenantId and tenantSlug exactly as provided, activeExposureCents "0", requiresImmediatePromotion false.
- Voice: outbound subscriber newsletter — clear hook, short paragraphs, one pillar or problem per issue.
- Focus on why Ironframe services were needed and which business/control problems they resolve (value + intent).
- Do NOT reveal program code, file paths, APIs, schemas, ports, stack internals, agent implementation details, or step-by-step engineering.
...
export async function requestGovernanceNewsletterSeriesCore(
  input: RequestGovernanceNewsletterSeriesInput,
...
FOCUS: ${issue.focus}
OUTPUT FILENAME (do not invent another): ${issue.filename}
...
Write the complete quarantine-ready newsletter markdown for this issue only.
`.trim(),
      });
      drafts.push({
