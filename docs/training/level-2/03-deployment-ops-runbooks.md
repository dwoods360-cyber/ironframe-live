...
> **Target length:** ~1600 words · **Primary route:** `/opsupport`

## Learning objectives

After completing this chapter, you will be able to navigate to `/opsupport`, execute the prescribed task flow, and verify OPS-001 controls using tenant-scoped session context.

## Feature location in the SaaS application

| Attribute | Value |
|-----------|-------|
| Primary route | `/opsupport` |
| Capture route | `/opsupport` |
| GRC function IDs | OPS-001 |
...
2. Cross-read docs/technical/deployment-and-ops.md
3. Verify .env.example and local .env.local keys for Phase 1–3
4. Document cron secret gating for internal routes

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Open /opsupport infrastructure dashboard | /opsupport |
| 2 | Cross-read docs/technical/deployment-and-ops.md | /opsupport |
| 3 | Verify .env.example and local .env.local keys for Phase 1–3 | /opsupport |
| 4 | Document cron secret gating for internal routes | /opsupport |
## Reference screenshot

![Chapter 3 — Deployment, Environment & Ops Runbooks](/docs/training/assets/level-2-03-deployment-ops-runbooks.png)

*Figure: Chapter 3 — Deployment, Environment & Ops Runbooks — captured at `/opsupport`. Asset path: `/docs/training/assets/level-2-03-deployment-ops-runbooks.png`.*
...
2. Open /opsupport infrastructure dashboard
3. Locate the feature in the SaaS UI at route `/opsupport`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Verify .env.example and local .env.local keys for Phase 1–3
3. Locate the feature in the SaaS UI at route `/opsupport`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Document cron secret gating for internal routes
3. Locate the feature in the SaaS UI at route `/opsupport`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
This procedure validates OPS-001 against the live Ironframe workspace at `/opsupport`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Open /opsupport infrastructure dashboard
...
Cross-reference `docs/qa/complete-feature-glossary.md` for OPS-001 before submitting the lab.

### Procedure 2: OPS-001

This procedure validates OPS-001 against the live Ironframe workspace at `/opsupport`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Cross-read docs/technical/deployment-and-ops.md
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for OPS-001 before submitting the lab.
...
This procedure validates OPS-001 against the live Ironframe workspace at `/opsupport`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Verify .env.example and local .env.local keys for Phase 1–3
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for OPS-001 before submitting the lab.
...
This procedure validates OPS-001 against the live Ironframe workspace at `/opsupport`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Document cron secret gating for internal routes
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for OPS-001 before submitting the lab.
...
A: Use primary route `/opsupport`, follow the navigation path table in this chapter, and archive screenshot `level-2-03-deployment-ops-runbooks.png`.
...
- [ ] OPS-001 — procedure completed and screenshot archived
...
- source-file: docs/technical/deployment-and-ops.md
- source-file: docs/operations-support/nightly-cron-runbook.md
...
ref: config/training-corpus-manifest.json · slug=training/level-2/03-deployment-ops-runbooks