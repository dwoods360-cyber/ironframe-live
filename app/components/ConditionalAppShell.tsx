...
export default function ConditionalAppShell({ children }: { children: React.ReactNode }) {
  ...
  const isGovernanceFrame =
    pathname === "/governance-frame" || pathname.startsWith("/governance-frame/");
  const isPublicLegal =
  ...
  if (isAuthPublic || isPublicLanding || isMarketingPage || isPublicPricing || isGovernanceFrame || isPublicLegal || isPublicRegistration) {
