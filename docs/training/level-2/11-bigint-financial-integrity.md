...
After completing this chapter, you will be able to navigate to `/integrity`, execute the prescribed task flow, and verify FIN-001 controls using tenant-scoped session context.
...
2. Verify Medshield baseline 1110000000 cents
3. Verify Vaultbank 590000000 and Gridcore 470000000
4. Run tests/unit/financialIngressInvariant.test.ts in lab environment

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Open /integrity financial posture cards | /integrity |
| 2 | Verify Medshield baseline 1110000000 cents | /integrity |
| 3 | Verify Vaultbank 590000000 and Gridcore 470000000 | /integrity |
| 4 | Run tests/unit/financialIngressInvariant.test.ts in lab environment | /integrity |
...
*Figure: Chapter 11 — BigInt Financial Integrity & Baseline Verification — captured at `/integrity`. Asset path: `/docs/training/assets/level-2-11-bigint-financial-integrity.png`.*
...
2. Open /integrity financial posture cards
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Verify Vaultbank 590000000 and Gridcore 470000000
3. Locate the feature in the SaaS UI at route `/integrity`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Run tests/unit/financialIngressInvariant.test.ts in lab environment
3. Locate the feature in the SaaS UI at route `/integrity`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
Execute: Open /integrity financial posture cards
...
This procedure validates FIN-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Verify Medshield baseline 1110000000 cents
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for FIN-001 before submitting the lab.
...
This procedure validates FIN-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Verify Vaultbank 590000000 and Gridcore 470000000
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for FIN-001 before submitting the lab.
...
This procedure validates FIN-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Run tests/unit/financialIngressInvariant.test.ts in lab environment
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for FIN-001 before submitting the lab.
...
A: Use primary route `/integrity`, follow the navigation path table in this chapter, and archive screenshot `level-2-11-bigint-financial-integrity.png`.
...
- source-file: tests/unit/financialIngressInvariant.test.ts
...
ref: config/training-corpus-manifest.json · slug=training/level-2/11-bigint-financial-integrity