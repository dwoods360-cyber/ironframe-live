# 📖 Complete Platform Feature Glossary & Screen Guide
## Self-Paced Reference Manual for Independent Technical Learners

Welcome to the Master Feature Glossary. This guide documents every visible visual block, controller, ticker, and data matrix on the Ironframe command console deck. Use this to audit the interface behaviors independently online.
...
* **How to Use It:**
  1. Look at the top left of the header line to confirm the pulsing green indicator light next to `Ironframe Sovereign Platform` is active.
  2. Look at the far right of the line to find the button labeled `➔ BACK TO DASHBOARD`.
...
* **How to Use It:**
  1. Click the active dropdown text block displaying the current corporate tenant.
  2. Select an alternative company from the animated layout menu list.
  3. Watch the interface briefly flash a gray loading layout block to clear client memory.
  4. Verify that all center statistics metrics redrawing on your screen map strictly to the selected company profile.

---

## 🕹️ 2. The Left Panel: Control & Chaos Deck [22% Screen Width]

### 📊 Function Name: System Maturity Scoring Track (`UX-005`)
* **Screen Location:** Placed at the top section of the left-hand column panel. Look for the bold fraction readout showing **`4.5 / 10`**.
* **What It Is Used For:** Providing a high-level technical grading score of the selected company's defense posture based on active vulnerability checks.
* **How to Use It:**
  1. Read the large white numeric grading status value on the left panel block.
  2. Look right beneath the score to view the small green subtitle tracking metric labeled **`+1.2 MoM`** (Month-Over-Month improvement trend).
...
* **How to Use It:**
  1. Review the list of selectable simulation scenarios mapped out in the left panel.
  2. Click on a specific mock scenario row (e.g., *Ransomware Outbreak Mock Sync*).
  3. Watch the main system response feed update instantly, showing how the background agents detect, intercept, and isolate the threat vector in real time.

---

## 📈 3. The Center Panel: Core Workspace Canvas [48% Screen Width]

### ⚡ Function Name: Platform Integrity Synchronizer (`SYNC-001`)
* **Screen Location:** Pinned inside the top grid box of your center canvas column layout. Look for the message bar labeled **`ALL MODULES SECURE · ZERO DRIFT ENFORCED`**.
* **What It Is Used For:** Giving an instant visual confirmation that all background security, data validation, and multi-tenant perimeters are operating with 100% precision.
* **How to Use It:**
  1. View the horizontal status card resting at the crown of the center panel.
...
* **How to Use It:**
  1. Scan the list rows to check individual agent check-in times and statuses (e.g., Green `ACTIVE` markers).
  2. Click directly on any specific agent row item in the table layout.
...
* **How to Use It:**
  1. Scroll through the ledger block independently down the center column track.
  2. Read individual entry lines to trace execution stamps (e.g., `[AGENT-14] SANITIZATION PURGE COMPLETION`).
  3. Verify that every logged event contains a matching cryptographic verification string.
...
* **How to Use It:**
  1. Read the live intensity metric chart displaying the real-time grid load (e.g., **`382 gCO₂eq/kWh`**).
  2. Check the top-right corner of the box to verify if the orange **`FALLBACK ACTIVE`** badge is glowing.
...
* **How to Use It:**
  1. Click into the text entry input bar.
  2. Type a validation string (such as `VAULTBANK`) and hit Enter.
...
* **How to Use It:**
  1. View the card row layout to confirm all three metric containers share a uniform horizontal border height (`h-24`) across the screen width.
  2. Locate the button labeled **`Export Tabular Ledger Data (CSV)`** placed right within this panel group.
  3. Click the button to start an instantaneous CSV spreadsheet download file stream.
  4. Open the downloaded file to verify that all financial balances are formatted as pure BigInt integers representing whole cents (e.g., `$5,000.00` appears as `500000`), demonstrating banking-accuracy compliance.
