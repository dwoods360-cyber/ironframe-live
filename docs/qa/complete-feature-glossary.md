...
* **The App Reality:** In our platform, these rules are hardcoded into an electronic constitution known as the **TAS (Tenant Architecture Specifications)** file at `docs/TAS.md`. The software code is physically blocked from ever breaking these rules. Today's delta (2026-09-22) delivers **Ironintel September 21 live OSINT manifest refresh** — `Ironboard/src/knowledge/grcProfessionalResearch.manifest.json` rotates to `ironintel-osint-2026-09-21-live` (`generatedAt: 2026-09-21T08:00:00.000Z`) centering on **BOD 26-04 FCEB due-date closure** for Linux kernel KEVs **CVE-2025-39964** (AF_ALG race) and **CVE-2026-53266** (ebtables SNAT ARP out-of-bounds write) with **CVE-2025-39682** TLS receive-path carryover attestation on internet-facing Linux and container hosts; **Cisco Identity Services Engine CVE-2026-76460** (CVSS **10.0**; FCEB due **September 19**) enters **post-deadline forensic triage day two** on operational date **2026-09-22** — mandatory compromise assessment, iACL review, and attestation receipts before identity-plane ticket close; post-deadline triage continues for **Google Pixel CVE-2026-58704** and **Acronis Backup CVE-2026-87886** (FCEB due **September 19**). **CISA weekly Vulnerability Bulletin** seven-day countdown to **September 28** sunset — SOCs retune to KEV catalog, CISA alerts, and vendor PSIRT advisories under BOD 26-04 rather than generic weekly severity summaries. **Google Chromium V8 CVE-2026-87491** (September **9** KEV add) — out-of-bounds write in V8 enables sandbox escape via crafted HTML on Chromium-derived enterprise browsers; Technology Industry Profile enforces browser patch cadence and Irongate-sanitize third-party HTML ingress. **CMMC DARS Class Deviation 2026-O0025 Revision 3** (**September 3**) preserves NIST SP **800-171 Rev 2** under DFARS **252.204-7012** while Phase II C3PAO transition stays suspended; RIN **0790-AM01** Rev **3** interim final rule remains unpublished with no effective date. **Revolut** government-impersonation legal-request fraud (~**680** cryptocurrency customer records via compromised Italian government PEC workflow); **Metaencryptor** Beckman Coulter diagnostics carryover; **Akira** Manders subcontractor extortion (~**70GB** claim) elevates SPRS and DFARS flow-down verification; **FAR Case 2026-001 Rev 3** governmentwide CUI rule comment period closed **July 23** with finalization expected before end **2026**; **osint-08-post-deadline-carryover** requires forensic attestation for Minnesota water PLC **AA26-097A** and Cl0p Windchill **CVE-2026-12569** alongside kernel due-date closure; workday `riskMetricsCents`: median annual GRC program **5520000000**, audit remediation lag **1388000000**, SaaS consolidation savings **1032000000**, board reporting overhead **183200000** cents; SaaS disruption memorandum: **5160000000**, **1456000000**, **1614000000**, **149400000** cents; industry `saasDisruptionExposureIndex` — Finance **94**, Healthcare **88**, Defense **76**, Public Sector **81**, Manufacturing **91**; RAG chunks **osint-01-cisco-ise-deadline-day** through **osint-08-post-deadline-carryover** authored **2026-09-21**; **`saas-01-dmz-ingress`** binds **`ironintel-osint-2026-09-21-live`**. **Path B Sales touch cadence HITL desk** — `AdminApprovalDashboardClient.tsx` adds **`touch`** URL filter (**TOUCH1** / **TOUCH2** / **TOUCH3** / **ALL**) via `parseApprovalTouchFilter()` and extended `approvalsHref()` / `approvalsDraftHref()`; **`expectedTouch`** on DISPATCH preview; **`app/api/admin/approvals/[id]/route.ts`** computes **`expectedTouch`** from `inferSalesTouchStage()` and passes to `validateApprovalDispatch()`; **Touch 3 Value Drop** EMAIL perimeter in `approvalDispatchValidation.ts` — when **`expectedTouch === "TOUCH3"`**, forbid **`looksLikeTouchEconomicsBody()`** (Path B **499900** cents / Command Design Partner / workflow review language); require **`looksLikeTouch3ValueDropBody()`** numbered three-check body per `app/lib/salesTouch3Body.ts`; `lintSalesHumanVoice(body, { allowMissingPeerCta: true })` for Touch **3**; Touch **1**–**2** retain Mandate **17** C1 locks. **SalesTouch2QueuePanel** — card grid with Sales queue chrome; **`pendingDraftId`** / **`pendingSubject`** from `salesTouch2QueueCore.ts` pending TOUCH2 join; click opens shared Approvals editor via **`approvalsDraftHref(..., "TOUCH2")`**; HITL only — never auto-DISPATCH. **Prospect-pool CRM scope** — `withProspectPoolTenant()` on `approvalQueueCore.ts`, `approvalNeedsEnrichmentCore.ts`, Approvals DISPATCH mutations; **`PROSPECT_POOL_TENANT_UUID` `11111111-1111-4111-8111-111111111111`** in `salesAgentConsoleCore.ts` (Medshield fallback removed). **Account Research Brief resilience** — `resolveAccountResearchBrief()` rejects incomplete legacy metadata; `selectAccountResearchBriefForReport()` reason **`incomplete_persisted_gates`** when **`gates.buyer`** missing; optional **`outreach?.status`** in suspect report and operator edit panel; unit test Strong Security Brasil digest **350965605**. **Ops schedule video slate slip** — `opsScheduleSeedSpecs.ts` moves **When Risk Enters the Room** V1–V6 build/publish milestones to **October–November 2026** (V1 publish **2026-10-21**); companion story bank **2026-10-14**; LinkedIn cards **2026-08-31-nis2-iso-accountability**, **2026-09-04-evidence-provenance**, **2026-09-15-ma-security-debt** voice-pass **Ready for publish** **2026-09-21**. **Tests** — `tests/unit/publicAppUrl.test.ts` **`sanitizeAuthNextPath`**; `tests/unit/ironleadsAccountResearchBrief.test.ts` incomplete gates case. Prior-cycle (2026-09-21) delivers **Ironintel September 20 OSINT manifest** (`ironintel-osint-2026-09-20-live` — Feature **148** historical); prospect-pool RLS; Approvals sticky queue + Touch-2 panels below grid; Integrity Hub soft-fail; auth sanitize (Feature **147**); Prior-cycle (2026-08-11) delivers **Ops schedule GTM gate reschedule** — `opsScheduleSeedSpecs.ts` moves **Marketing — Cold outreach gate (after free marketing)** from **2026-08-10T15:00:00.000Z** to **2026-08-17T15:00:00.000Z** (wait for LinkedIn week-1 posts + Video Phase 0 before cold Path B DISPATCH) and **Rollout — First SalesTeam DISPATCH batch** from **2026-08-17T15:00:00.000Z** to **2026-08-24T15:00:00.000Z** (after cold outreach gate **August 17**); **LinkedIn Wed draft first-comment CTA refresh** — `linkedinDeskDraftCore.ts` rewrites **`wed-product-demo`** slot body to contrast feature-list/color-dashboard GRC tours against **Evidence → Scenario → Estimated Exposure** (whole cents, explicit assumptions) workflow copy with single primary link `https://ironframegrc.com/product-demo`; **10–15 minute workflow review** CTA moves to **first comment** (post immediately after publish — do not put in main body) per founder cadence one-primary-link rule; research block adds held alternate `docs/marketing-strategy/linkedin-drafts-hold-board-reporting-gap.md` (Option 2 board-lens — future Monday) and first-comment paste checklist item; **Ironintel August 10 live OSINT manifest refresh** — `grcProfessionalResearch.manifest.json` rotates to `ironintel-osint-2026-08-10-live` (active Strategic Intel binding `ironintel-osint-2026-08-10-live`, authored **2026-08-10**) — on operational date **2026-08-11** **Progress LoadMaster CVE-2026-8037** and **Fortinet FortiOS CVE-2025-68686** federal remediation deadlines **passed August 10** enter **post-deadline forensic triage day one** (manifest authorship was **dual BOD 26-04 KEV remediation deadline day**): **Progress LoadMaster CVE-2026-8037** (CVSS **9.6** unauthenticated OS command injection RCE; CISA KEV added August 7; eSentire TRU confirms exploitation attempts since June 29 with public PoC; patches GA **v7.2.63.2** and LTSF **v7.2.54.18**) and **Fortinet FortiOS CVE-2025-68686** (symbolic link persistency bypass on post-exploit perimeter appliances; CISA Forensics Triage Requirements mandatory on potentially compromised devices); **ChainDrop** self-propagating npm worm **day-seven containment** — Unit 42, Microsoft, StepSecurity, and Elastic Security Labs confirm **400**-plus unique packages with **keyv** exceeding **600** million monthly downloads and combined ecosystem exposure above **1.3** billion monthly downloads; Bun preinstall dropper, EtherHiding Ethereum dead-drop C2, and Claude Code or VS Code persistence injection remain under active secret-rotation triage; **FBI and CISA August 9 renewed urgent warning** on internet-connected PLC exploitation per **CISA AA26-097A** — Iranian-affiliated APT actors targeting Rockwell Automation, Allen-Bradley, Schneider Electric, and Siemens PLCs via ports **44818**, **2222**, **102**, **502**, and **22**; **BOD 26-04 post-deadline forensic triage day five** for IBM Langflow CVE-2026-9198, Apache Tomcat CVE-2026-34486, N-able N-central CVE-2026-18556, and JetBrains TeamCity CVE-2026-63077 (federal remediation deadlines passed **August 7–8**); **JetBrains TeamCity CVE-2026-63077** post-deadline forensic triage **day three**; agency BOD 26-04 vulnerability-management policy update deadline passed **August 7** — Public Sector Industry Profile enters policy attestation verification window; **UTS Unlimited Technology Systems** Ohio medical software provider breach affecting **3803750** individuals per HHS breach portal — largest HHS-reported healthcare breach of 2026; CMMC Phase II C3PAO suspension day **29** (RFI closes **August 14** with **3** days remaining); carryover: **PleaseFix Intent Collision** zero-click agentic browser class (Zenity Labs Black Hat USA 2026 — Claude in Chrome, Gemini in Chrome, Perplexity Comet, ChatGPT Atlas, Copilot Edge), **Windows Hello for Business** Entra ID WebAuthn persistence, Minnesota water PLC day **fifteen** (CISA AA26-097A), Cl0p Windchill **CVE-2026-12569** PLM extortion; updated `riskMetricsCents` on workday analysis doc: median annual GRC program **5360000000**, audit remediation lag **1346000000**, SaaS consolidation savings **1000000000**, board reporting overhead **178000000** cents; SaaS disruption memorandum doc: **5004000000**, **1420000000**, **1578000000**, **145800000** cents — all string-encoded BigInt integers through Irongate DMZ before CRM persistence. **Video campaign V1 production pack wiring** — `publishingDeskDraftKind.ts` adds **V1 kickoff — Phase 0 + shot list** and **V1 production script — shots + AI prompts** under `PUBLISHING_VIDEO_NARRATIVE_LINKS` Hub & budget group; `opsScheduleSeedSpecs.ts` **Video Phase 0 — Style lock complete** due **2026-08-09** cites working pack `/docs/marketing-strategy/video-series/v1-production-script` with calendar `href` deep link. **Ironleads Promote-ready email gate simplification** — removed `emailMatchesAccountDomain()` and `isPromoteReadyEmployerEmail()`; `accountDomain` removed from `BuildAccountResearchBriefInput`; Email gate PASS uses **`isPromoteReadyWorkEmail()`** only (blocks company intakes, not cross-domain Prospeo side-gig inboxes); `mergeNamedBuyerIntoBriefMembers()` merges buyer or contact email directly; `ironleadsSuspectReportCore.ts` stops passing `deal.accountDomain`; Kyber wrong-domain unit test removed. **Apex marketing redirect** — `app/page.tsx` sends unauthenticated apex guests to **`/marketing`** instead of **`/login`**; signed-in operators still route to Integrity Hub. Prior-cycle (2026-08-10) **Ironintel August 10 live OSINT manifest refresh** — `grcProfessionalResearch.manifest.json` rotates to `ironintel-osint-2026-08-10-live` with **dual BOD 26-04 KEV remediation deadline day August 10**: LoadMaster CVE-2026-8037 and FortiOS CVE-2025-68686 remediation due **August 10**; ChainDrop day-six containment; BOD 26-04 post-deadline triage day four; TeamCity post-deadline triage day two; CMMC day **28**; workday `riskMetricsCents` **5360000000**, **1346000000**, **1000000000**, **178000000** cents; SaaS disruption **5004000000**, **1420000000**, **1578000000**, **145800000** cents; Video V1 production pack links; Ironleads employer-domain gate removed; apex marketing redirect. Prior-cycle (2026-08-08) **Ironintel August 8 live OSINT manifest refresh** — `grcProfessionalResearch.manifest.json` rotates to `ironintel-osint-2026-08-08-live` with **JetBrains TeamCity CVE-2026-63077** federal remediation deadline day (**August 8** manifest authorship — CVSS **9.8** unauthenticated deserialization RCE via agent polling protocol; CISA KEV added August 5; patches TeamCity **2025.11.7** and **2026.1.3**); **ChainDrop** day-four containment; **PleaseFix Intent Collision** zero-click agentic browser; **UTS** **3803750**-individual healthcare breach; **LoadMaster CVE-2026-8037** KEV due **August 10**; BOD 26-04 post-deadline triage day two; CMMC day **26**; workday `riskMetricsCents` **5315000000**, **1332000000**, **988000000**, **175800000** cents; SaaS disruption **4958000000**, **1406000000**, **1562000000**, **143400000** cents. Prior-cycle (2026-08-07) **Ironintel August 7 live OSINT manifest refresh** — `grcProfessionalResearch.manifest.json` rotates to `ironintel-osint-2026-08-07-live` with converging **BOD 26-04 governance and KEV remediation deadline day**: agency vulnerability-management policy updates were due **August 7**; federal remediation for IBM Langflow CVE-2026-9198 (CVSS **9.8**), Apache Tomcat CVE-2026-34486 (CVSS **7.5**), and N-able N-central CVE-2026-18556 (CVSS **8.2**) due **August 7**; **ChainDrop** day-three containment; **JetBrains TeamCity CVE-2026-63077** active CI/CD RCE with KEV due **August 8**; **Zenity** zero-click AI browser indirect prompt injection; **Windows Hello for Business** Entra ID persistence; CMMC Phase II day **25**; workday `riskMetricsCents` **5270000000**, **1318000000**, **976000000**, **173600000** cents; SaaS disruption **4912000000**, **1391000000**, **1548000000**, **141200000** cents. Prior-cycle (2026-08-07) **Ironintel August 6 live OSINT manifest refresh** — `grcProfessionalResearch.manifest.json` rotates to `ironintel-osint-2026-08-06-live` with ChainDrop npm worm **day-two containment triage** converging on a KEV deadline cluster: Elastic Security Labs confirms **400**-plus unique npm packages compromised August 6 with **keyv** exceeding **600** million monthly downloads and combined ecosystem exposure exceeding **1.3** billion monthly downloads across flat-cache and cacheable-request chains; **N-able N-central CVE-2026-18577** BOD 26-04 three-day federal remediation deadline was **August 6** (manifest authorship deadline day); **JetBrains TeamCity CVE-2026-63077** active exploitation confirmed August 6 (CVSS **9.8**; KEV due **August 8** with **2** days remaining at authorship); **IBM Langflow CVE-2026-9198** and **Apache Tomcat CVE-2026-34486** federal remediation due **August 7** (**1** day remaining at authorship); **BOD 26-04** agency vulnerability management policy update deadline **August 7** (FedRAMP Notice **0014** VER-EVA-ELX four-variable risk model); CMMC Phase II C3PAO suspension day **24** (RFI closes **August 14** with **8** days remaining); **CHAOS** healthcare extortion carryover (Radia Inc **655** GB PHI, Healthcare Highways **235** GB); **Brown Health Medical Group-MA** breach **311000** individuals per SecurityWeek August 6 reporting; **CaptiveCrunch** Storm-2945 captive-portal carryover; Fortinet FortiOS **CVE-2025-68686** due **August 10** (**4** days); Minnesota water PLC day **ten** (CISA AA26-097A); Cl0p Windchill **CVE-2026-12569** PLM extortion carryover; updated `riskMetricsCents` on workday analysis doc: median annual GRC program **5228000000**, audit remediation lag **1306000000**, SaaS consolidation savings **972000000**, board reporting overhead **172400000** cents; SaaS disruption memorandum doc: **4865000000**, **1378000000**, **1536000000**, **139800000** cents — all string-encoded BigInt integers through Irongate DMZ before CRM persistence. **Ops Publishing LinkedIn three-slot catalog** — `linkedinDeskDraftCore.ts` defines `LINKEDIN_DRAFT_CATALOG` with **`mon-heatmap`**, **`wed-product-demo`**, **`fri-collection`** Mon/Wed/Fri slots; `GET /api/admin/operations-hub/linkedin/drafts` lists active vs posted-archive buckets keyed to Ops calendar `sourceRef` DONE status; `PublishingDeskClient.tsx` renders **Drafts awaiting paste** left and **Published** right (mirrors Briefings/Newsletters chrome); deep links `?desk=linkedin&li={id}`; default open slot **`fri-collection`** (Collection is not verification); Mon heatmap slot marked **DONE** **2026-08-06** moves to Published archive; **Reset to template** replaces legacy Load suggested seed. **Ops schedule calendar catch-up** — `opsScheduleSeedSpecs.ts` closes DORA supervision promotion (**DONE** **2026-08-06**), GF-2026-001 editorial and product-boundary reviews (**PASS** **2026-08-05**), Control-First newsletter outline + quarantine draft staged, EU AI Act claim alignment (**PASS** in `docs/ops/eu-ai-act-claim-alignment-2026-08.md`), board packet Aug AI Act posture (**PASS**), stakeholder benefit-map (**CANCELLED** — product-boundary heavy), partner provisioning walkthrough (**DEFERRED**), First SalesTeam DISPATCH rescheduled **August 17**, Stripe lifecycle rescheduled **September 28**, cold outreach gate **August 10**, LinkedIn Fri due **August 8**. **Ops queue review idempotency** — `ensureQueueReviewActivity()` skips Medshield demo-tenant narrate dumps (not public GF promote candidates) and does not reopen `sourceRef` rows already **DONE** or **CANCELLED**. **Apex marketing redirect** — `app/page.tsx` sends unauthenticated apex guests to **`/marketing`** instead of **`/login`**; signed-in operators still route to Integrity Hub. Prior-cycle (2026-08-06) **Ironintel August 5 live OSINT manifest refresh** — `grcProfessionalResearch.manifest.json` rotates to `ironintel-osint-2026-08-05-live` with tier-zero JavaScript supply-chain convergence: **ChainDrop** self-propagating npm worm (Shai-Hulud variant) poisoned **1300**-plus packages and **2212** versions across **444** publishers beginning with **keyv 6.0.0** August 4 (**150** million weekly downloads; combined monthly exposure exceeds **2** billion); Bun runtime preinstall dropper, EtherHiding Ethereum C2, Claude Code and VS Code persistence hooks beyond install-time scanning; credential harvest expanded **70** percent to AI-agent stores (Cursor, Codex, Gemini), Jenkins, Argo CD, Harbor, cloud CLI configs; WordPress Core **CVE-2026-60137** BOD 26-04 FCEB deadline **passed August 4** — post-deadline forensic attestation day **two** at authorship (EPSS **79.0** percent); **CHAOS** ransomware healthcare extortion claims Radia Inc **655** GB PHI August 3 and Healthcare Highways **235** GB August 4; Anubis pharmacy breach August 3; **BOD 26-04** agency policy update deadline **August 7** (**1** day remaining at authorship); CMMC Phase II C3PAO suspension day **23** (RFI closes **August 14** with **8** days remaining); **N-able N-central CVE-2026-18577** MSP RMM god-mode bypass carryover (Huntress nine-org confirmation; hotfix **2026.3.1.7**); **CaptiveCrunch** Storm-2945 captive-portal carryover; Fortinet FortiOS **CVE-2025-68686** due **August 10** (**4** days); Minnesota water PLC day **nine** (CISA AA26-097A); Cl0p Windchill **CVE-2026-12569** PLM extortion carryover; updated `riskMetricsCents` on workday analysis doc: median annual GRC program **5186000000**, audit remediation lag **1294000000**, SaaS consolidation savings **968000000**, board reporting overhead **171200000** cents; SaaS disruption memorandum doc: **4823000000**, **1367000000**, **1528000000**, **138900000** cents — all string-encoded BigInt integers through Irongate DMZ before CRM persistence. **Ops Publishing desk five-tab expansion** — `publishingDeskDraftKind.ts` adds **LinkedIn** founder paste workbench at `/dashboard/operations/publishing?desk=linkedin`; `linkedinDeskDraftCore.ts` + `GET|PUT /api/admin/operations-hub/linkedin/draft` persist title/body/research to APP_DOCS slug `marketing-strategy/linkedin-drafts-week-1` with Mandate **16**-safe suggested heatmap-vs-dollars copy and operator-only citation research block — manual LinkedIn paste only, never auto-post or GF promote. **Ironleads Promote-ready email gate** — `isPromoteReadyWorkEmail()` rejects company intakes (`info@`, `hello@`, `support@`, etc.); `selectAccountResearchBriefForReport()` merges Prospeo/operator email upgrades without clobbering rich scrape corpus; `pathBHold` forces Email/Buyer FAIL on operator `channel_competitor` HOLD; **mailto/directory email discovery** — `discoverPublishedEmails()`, `extractMailtoEmails()`, `publishedEmailMatchesPerson()`, trade-show booth signals via `extractTradeShowAndEventSignals()`; expanded `RESEARCH_PATHS` for legal/terms/event pages; `returnToQueue()` hard navigation after promote/hold/discard/restore with query params `?promoted|held|restored|discarded=`; duplicate Explorer **copy** artifacts purged from repo with `.cursorignore` and narrowed `tsconfig.json` include/exclude. Prior-cycle (2026-08-05) **Ironintel August 3 live OSINT manifest refresh** — `grcProfessionalResearch.manifest.json` rotates to `ironintel-osint-2026-08-03-live` with MSP supply-chain cascade convergence: **N-able N-central CVE-2026-18577** god-mode RMM authentication bypass after incomplete patch for CVE-2026-18556 (Huntress confirms exploitation across nine customer organizations August 3 under one self-hosted partner instance; hotfix **2026.3.1.7**; Take Control lateral movement and cloudflared tunnel persistence); WordPress Core **CVE-2026-60137** BOD 26-04 FCEB deadline **August 4** (**1** day remaining at authorship; EPSS **79.0** percent); **CaptiveCrunch** Storm-2945 captive-portal M365/Azure AD/WAM token theft carryover; **ShinyHunters** healthcare vishing-to-Entra-SSO extortion wave (Medtronic, iRhythm, OneMedical, DentaQuest, AdaptHealth, Him and Hers; Amgen SEC 8-K July 31 third-party cloud PHI exfiltration); **Arch AUR** openconnect-sso developer supply-chain freeze carryover; CMMC Phase II C3PAO suspension day **21** (RFI closes **August 14** with **11** days remaining); Fortinet FortiOS **CVE-2025-68686** due **August 10** (**7** days); Minnesota water PLC coordinated campaign day **seven** (CISA AA26-097A); Cl0p Windchill **CVE-2026-12569** PLM extortion carryover; BOD 26-04 post-deadline verification on Cisco Secure FMC CVE-2026-20316 (deadline passed **August 1**), Balbooa Forms CVE-2026-56291 (**July 31** passed), SharePoint machine-key cluster; updated `riskMetricsCents` on workday analysis doc: median annual GRC program **5042000000**, audit remediation lag **1248000000**, SaaS consolidation savings **932000000**, board reporting overhead **165700000** cents; SaaS disruption memorandum doc: **4675000000**, **1319000000**, **1482000000**, **132400000** cents — all string-encoded BigInt integers through Irongate DMZ before CRM persistence. **Ops Publishing desk four-tab expansion** — `publishingDeskDraftKind.ts` adds **Research papers** (`*-draft-research-*`) and **Video** (When Risk Enters the Room campaign hub) at `/dashboard/operations/publishing?desk=briefings|newsletters|research|video`; inline draft reader modal; `publishedLedgerKind.ts` splits Briefings index from newsletters and industry research on `research.ironframegrc.com`; `BriefingArchiveDirectory.tsx` vertical archive sidebar; ops schedule seed marks **2026-08-04** archive promotions (CPS 230 MSP contracts, AUSTRAC Tranche 2, CSRD Omnibus ESRS, SEC Item 105 filings, GRC evolution/pain research). **Ironleads leadership OSINT search** — `googleLeadershipSearchClient.ts` cascades **Brave → SerpAPI → Google CSE legacy** when site scrape yields zero plausible names; `ironleadsLeadershipSearchAllowlist.ts` press/cyber URL allowlist; `domainMailFootprint.ts` MX/SMTP footprint; `buildEmailPermutationCandidates()` with first.last → f.last → first@ test order; portal **Research** auto-continues up to **8** batches; queue decision banners (`promoted|held|restored|discarded`); suspect dossier mode `?dossier=1`; **Prospeo HITL enrich** warns when email remains `@ironleads.local`. Prior-cycle (2026-08-03) **Ironintel August 2 live OSINT manifest refresh** — `ironintel-osint-2026-08-02-live` with CaptiveCrunch identity-first convergence, Arch AUR supply-chain, ShinyHunters healthcare SSO extortion, WEF **75** percent credential-breach synthesis; updated workday `riskMetricsCents` **4985000000**, **1215000000**, **918000000**, **162300000** cents; SaaS disruption **4612000000**, **1286000000**, **1458000000**, **129100000** cents. **Ironleads SUSPECT readiness ranking** — `ironleadsSuspectReadiness.ts` scores named buyer, buying-committee emails, website, phone, executive sponsor, and Account Research Brief completeness; active queue sorts **named buyers and fullest dossiers first**; research batch prefers **thinnest dossiers first**. **MSSPProviders card-block paste import**, **client bundle drift recovery**, **Supabase middleware single-session refresh**, **founder pitch practice audio**, **Stripe `plan_sku: COMMAND_TIER` metadata**, **operator-only MSSP/Clutch directory collectors** retained from prior cycle. Prior-cycle (2026-08-02) **Ironintel August 1 live OSINT manifest refresh** — Cisco FMC BOD 26-04 deadline-day, Minnesota water PLC day five, Cl0p Windchill PLM extortion wave, Balbooa Forms post-deadline verification, CareCloud **345000**-individual carryover, CMMC day **19**; workday `riskMetricsCents` **4928000000**, **1182000000**, **905000000**, **159600000** cents. **Ops Hub Publishing desk split** (briefings + newsletters), **Ironleads Path B directory pipeline** (pending pool **20**, Apollo HITL, Account Research Brief, N/E demotion, Command Center enclave gates) retained. Prior-cycle (2026-08-01) **Sales DISPATCH perimeter hardening**, workflow-review grounded pocket assist, SaaS call KB packaging expansion retained. Prior-cycle (2026-07-31) **Control-to-Capital market narrative federation**, Heatmap Amnesty landing `/marketing/heatmap-amnesty`, Mandate 16 claim bans; Path B **499900** cents via `formatPathBUsd()`. Prior-cycle (2026-07-20) **Ironleads SUSPECT location enrichment**, tri-track HITL Approvals desk, Ops Calendar checklist expansion. Prior-cycle (2026-07-19) **Governance Frame Publication Desk** with six quarantine-only desk agents and `.desk-reviews` sidecars. Prior-cycle (2026-07-18) **Governance Frame Research Publication** at `https://research.ironframegrc.com`, Ops Hub Calendar, IronBoard transcript export, marketing funnel expansion. Constitutional seed tenant ALE baselines unchanged: Medshield **1110000000**, Vaultbank **590000000**, Gridcore **470000000**, Defense **1600000000** cents. **priorityScore**, `readinessScore`, and `aiFitnessScore` remain integer ICP/dossier tier scores — not USD cents.
...
* **Mandate 16: Control-to-Capital Claim Bans & Heatmap Amnesty Copy Lock:** All outbound marketing, IronBoard advice, SaaS call KB, and SalesTeam drafts must ground against `docs/sales/control-to-capital-market-narrative.md` and `docs/sales-enablement/message-constitution.md` federated in the IronBoard docs matrix. **Publishable headline:** connecting continuous compliance evidence to business risk, financial exposure, and portfolio-level decision-making across isolated entities — not “most urgent unmet need in GRC.” **Financial language:** prefer **estimated annual loss exposure** / **loss exposure range (P50–P90)**; storage remains **whole-cent / BigInt integer cents** — never “true ALE as accounting dollars,” never float theater. **Heatmap doctrine:** heatmaps may remain optional context; the decision layer is scenarios + estimated financial exposure + mitigation cost — never “replace all heatmaps” or “boards are rejecting heatmaps” as universal claims. **SEC accuracy:** Item 106 / 1.05 raise accountability; materiality includes quantitative **and** qualitative factors — quantification helps defensibility; FAIR/ALE is not mandated. **Forbidden auto-claims:** most urgent unmet need / proves PMF / competitors cannot fix without rewriting / true ALE / only Open FAIR produces real dollars / standard tools are single-entity only (universal) / exact CAGR bands without named source / competitor capabilities unverified this quarter. **Campaign order:** Heatmap Amnesty (CISO/CFO board packs) is Priority-1; PE operating-partner campaign deferred until §A named buyers live and Path B counsel D0 clears first paid signature. Public landing `/marketing/heatmap-amnesty` copy must match `docs/sales/heatmap-amnesty-campaign.md` — HITL-only email sequence; never auto-send. Path B price displays via `formatPathBUsd()` from spine (**499900** cents) only.

* **Mandate 17: Sales DISPATCH Perimeter Gates & Pocket Assist Grounding:** GLOBAL_ADMIN **DISPATCH** on `/dashboard/admin/approvals/[id]` must pass `validateApprovalDispatch()` with full perimeter context — never bypass server gates from client-only edits. **HOLD companies:** `SALES_DISPATCH_HOLD_COMPANIES` (initial key **blueradius**) blocks Path B cold DISPATCH until operator purges or re-qualifies with a named buyer — UI shows rose **HOLD** banner on matching `company` field. **Harvest placeholder emails:** `@ironleads.local` destinations are blocked on EMAIL channel — auto-switch to SMS when E.164 phone present via `preferredSalesDispatchChannel()`; operator must paste real buyer email for production EMAIL DISPATCH. **Operator dry-run inboxes:** `operatorDryRunEmails()` includes `dwoods360@gmail.com`, `dereck@ironframegrc.com`, plus comma-separated `OPERATOR_DRY_RUN_EMAILS` — live partner DISPATCH to these addresses requires explicit **Acknowledge dry-run to my inbox** checkbox (`acknowledgeOperatorSelfDispatch: true` on API body). **Sales EMAIL C1 locks (Touch 1 and Touch 2 only):** when `expectedTouch` is not **TOUCH3**, body must mention **$4,999** or Command Design Partner and include workflow review CTA; must not offer free pilot/PoC/trial. **Touch 3 Value Drop locks (2026-09-22):** when `expectedTouch === "TOUCH3"`, EMAIL must use the three-check Value Drop body validated by `looksLikeTouch3ValueDropBody()` in `app/lib/salesTouch3Body.ts` — **FORBIDDEN:** $4,999 / Command Design Partner / planned GA economics / workflow review CTA (`looksLikeTouchEconomicsBody()`); `lintSalesHumanVoice()` may omit peer workflow-review CTA via `allowMissingPeerCta: true`. Server derives `expectedTouch` from `inferSalesTouchStage()` on pending summary and client passes matching `expectedTouch` on DISPATCH. **Workflow-review pocket assist:** `assistWorkflowReviewQuestionAsync()` returns verbatim SaaS KB and pocket-lock answers — never let Gemini paraphrase prices or entity caps; `grounded_llm` source applies only to novel phrasing grounded in `buildPocketGroundingPack()`; refuse (`source: miss`) when API unavailable rather than invent packaging math. **Packaging truth:** Path B hard-caps **1** Primary + **2** Subtenant Enclaves (**499900** cents flat); Command Core planned GA includes **4** entities (**3500000** cents/yr) — fourth entity is inside Core, not Core + Paid Enclave (**350000** cents/yr list per enclave beyond Core's three included Subtenants starts at entity **5**); Command Multi flat **5500000** cents/yr annual list mask up to **10** entities; do not stack **499900** + **350000** cents for a fourth tenant on Path B. FORBIDDEN: DISPATCH to HOLD shortlist without re-qualification; EMAIL to `@ironleads.local`; operator dry-run EMAIL without acknowledge checkbox; pocket assist inventing soft max-client quotas or Partner book as holding-company shortcut; Touch **3** EMAIL carrying Touch **1**–**2** economics or missing three-check Value Drop shape.
...
* **Mandate 23: Ironleads Leadership OSINT Search — Research Only, Allowlisted URLs:** When company-site scrape yields zero plausible buying-committee names, `researchBuyingCommitteeForContact()` may invoke `searchCompanyLeadership()` in provider order: **Brave Search** (`BRAVE_SEARCH_API_KEY` or `BRAVE_API_KEY`) → **SerpAPI** (`SERPAPI_API_KEY`, also failover when Brave returns zero hits) → **Google Custom Search JSON API** (`GOOGLE_CSE_API_KEY` + `GOOGLE_CSE_CX` — legacy entitlement only; paste allowlisted domains from `docs/ops/google-cse-ironleads-sites.txt`). Hits must pass `isAllowlistedLeadershipUrl()` in `ironleadsLeadershipSearchAllowlist.ts` and `refineLeadershipHits()` before persisting on `contact.metadata.buyingCommittee.googleLeadershipSearch`. Never scrapes google.com/search HTML. FORBIDDEN: wiring leadership search to auto-DISPATCH; persisting unallowlisted press URLs; treating MX PASS on pattern-guess emails as ownership proof on catch-all domains.

* **Mandate 24: Prospeo HITL Enrichment Only — Never Auto-DISPATCH:** `PROSPEO_API_KEY` enables named-buyer + domain work-email match via `enrichIronleadsSuspectWithProspeo()` on SUSPECT-stage contacts only — parallel to Apollo per Mandate 20. Prospeo applies work email only when contact still holds `@ironleads.local` placeholder — never overwrites verified buyer channels. Snapshot persists on `contact.metadata.prospeoEnrichment`. UI must warn when enrich completes but email remains `@ironleads.local` — operator must paste verified work address before EMAIL DISPATCH. FORBIDDEN: Promote on EMAIL while placeholder persists; wiring Prospeo to SalesTeam poll auto-send; storing Prospeo API key in client bundles.

* **Mandate 25: Ops Publishing Desk Five-Tab Split — Research, Video, and LinkedIn Surfaces:** Governance Frame quarantine desks live at **`/dashboard/operations/publishing?desk=briefings|newsletters|research|video|linkedin`**. Research-paper queue drafts match `*-draft-research-*` or `draft-research-*` via `isResearchDeskDraft()` — mutually exclusive from briefings and newsletters lists. Video desk links **When Risk Enters the Room** campaign hub at `/docs/marketing-strategy/video-series/when-risk-enters-the-room` — narrative index only, not auto-publish. LinkedIn desk is a founder paste workbench per Mandate 27 — not GF quarantine. Draft deep links auto-route to correct desk tab via `publishingDeskTabForQueueDraft()`. Inline draft reader modal loads markdown from `GET /api/admin/operations-hub/briefings/draft?filename=` — Escape dismisses. Human **Approve** / **Hold** / **Deny** gates unchanged per Mandates 9 and 14 for GF queue desks only.

* **Mandate 26: Published Ledger Kind Classification — Briefings Index Integrity:** Promoted markdown rows classify via `classifyPublishedLedgerItem()` in `publishedLedgerKind.ts` as **`briefing`**, **`newsletter`**, or **`industry_research`**. Newsletters and industry research briefs share `/briefings/[slug]` URL paths on `research.ironframegrc.com` but must not appear under the Briefings index — only `isBriefingIndexItem()` rows render on `/gf-research/briefings` main column. `BriefingArchiveDirectory.tsx` surfaces vertical archive sidebar on briefing detail pages. FORBIDDEN: promoting industry-research frontmatter without `category` precision; listing newsletter slugs on Briefings index.

* **Mandate 27: Ops Publishing Desk LinkedIn Workbench — Manual Paste Only, Never Auto-Post:** Founder LinkedIn copy workbench lives at **`/dashboard/operations/publishing?desk=linkedin`**. Desk catalog is **dynamic** as of **2026-08-11**: static week-1 slots (**`mon-heatmap`**, **`wed-product-demo`**, **`fri-collection`**) **union** every Ops calendar card whose `sourceRef` starts with **`marketing/linkedin`** (`resolveLinkedInDeskCatalog()` / `linkedinDeskIds.ts`). **Creation path:** seeding or upserting a LinkedIn calendar activity calls `ensureLinkedInDeskDraftForOpsActivity()` and stores href **`/dashboard/operations/publishing?desk=linkedin&li={id}`** via `hrefForSeedSpec` / `hrefForOpsSourceRef` — upcoming posts (e.g. **`2026-08-11-ai-evidence`**, **`2026-08-14-residual`**) appear under **Drafts awaiting paste** without a separate hard-coded slot list. Per-slot APP_DOCS under `marketing-strategy/linkedin-drafts/*` via `GET|PUT /api/admin/operations-hub/linkedin/draft?id=`; list returns **activeDrafts** vs **postedArchive** keyed to calendar **DONE**/**CANCELLED**. UI: **Drafts awaiting paste** left, **Published** right; deep link **`?li={id}`**; default open slot is first active draft by due date. **Copy body**, **Save**, and citation checkbox disabled on Published slots. Research under `## Research & verification (operator only — do not paste to LinkedIn)` must never ship to LinkedIn — copy **body** only; first-comment CTAs stay in research / post-publish comment. Research citations: require ≥1 outside/independent URL (not *.ironframegrc.com); when a published GF briefing maps the theme, add research.ironframegrc.com/briefings/… as a secondary cite (never a substitute). Legacy slug **`marketing-strategy/linkedin-drafts-week-1`** mirrors Fri collection. Mandate **16** claim bans apply. FORBIDDEN: wiring LinkedIn desk to SalesTeam auto-send; promoting LinkedIn markdown to GF quarantine; auto-posting to LinkedIn API; pasting research/first-comment into main body; editing Published archive as open drafts; LinkedIn calendar hrefs that skip the desk `?li=` slot.

* **Mandate 28: Ironleads Promote-Ready Work Email Gate — Company Intakes Fail Email Gate:** Account Research Brief Email gate PASS requires **`isPromoteReadyWorkEmail()`** — a named-buyer work seat, not a company switchboard. Generic locals (`info`, `hello`, `support`, `contact`, `sales`, `privacy`, `noreply`, etc.) keep Email **UNKNOWN** even when `hasRealEmail` is true and the inbox is reachable. Placeholder `@ironleads.local` never qualifies. As of **2026-08-10** delta (retained **2026-08-11**), **`emailMatchesAccountDomain()`** and **`isPromoteReadyEmployerEmail()`** were removed — Email gate no longer blocks Prospeo hits on a different employer domain when the address is otherwise a promote-ready work seat; operators must still HITL-verify buyer identity before DISPATCH. `mergeNamedBuyerIntoBriefMembers()` merges `namedBuyer.email` or `contactEmail` directly without domain filter. `BuildAccountResearchBriefInput.accountDomain` removed. `selectAccountResearchBriefForReport()` persists gate upgrades (Prospeo/operator email PASS, buyer roster drift) without replacing rich scrape corpus with thin report rebuilds. `pathBHold: true` when operator HOLD classification is `channel_competitor` or `hold` forces Email and Buyer **FAIL** and outreach **hold** even with verified work email on file. Published email discovery ranks **mailto** hrefs above directory labels above body text via `discoverPublishedEmails()`; role inboxes must not drive personal schema inference per `isRoleLocalPart()`. FORBIDDEN: Promote on EMAIL when only `info@` or `hello@` is on file; treating MX PASS on pattern-guess emails as ownership proof; auto-DISPATCH after Email gate PASS without human Approvals click; assuming removed employer-domain gate replaces operator buyer verification.

* **Mandate 29: Ops Queue Review Idempotency & Medshield Narrate Dump Exclusion:** When markdown stages into `docs/briefing-queue/`, `ensureQueueReviewActivity()` in `opsScheduleCore.ts` may spawn a calendar **BRIEFING_REVIEW** or **NEWSLETTER_REVIEW** row — but must **skip** filenames matching **`/medshield/i`** (demo-tenant narrate output is illustrative only, not a public GF promote candidate). Lookup uses latest row per `sourceRef` regardless of open status: if the newest row is **DONE** or **CANCELLED**, do **not** spawn a duplicate review card on re-stage. Open rows reuse in-place. `synopsisForQueueDraft()` labels Medshield dumps explicitly — **not a public GF promote candidate (auto-calendar skipped)**. FORBIDDEN: auto-spawning promote review cards for Medshield narrate fixtures; reopening closed calendar rows on duplicate queue file writes; treating demo-tenant narrate output as Governance Frame quarantine candidates.

* **Mandate 30: Path B CRM Prospect-Pool Tenant Scope — Never Unscoped Prisma on Ironboard CRM:** All reads and writes to `ironboardCrmInteraction`, `ironboardCrmContact`, and related Path B sales queue tables from Approvals DISPATCH, pending draft fetch, Needs enrichment demotion, Touch-2 queue panels, and outreach reply receipts must execute inside **`withProspectPoolTenant()`** from `app/lib/server/ironleadsTenantScope.ts` so Postgres RLS `tenant_id` GUC binds to the air-gapped prospect-pool UUID. **`resolveProspectPoolTenantId()`** in `salesAgentConsoleCore.ts` returns **`PROSPECT_POOL_TENANT_UUID` `11111111-1111-4111-8111-111111111111`** when `IRONFRAME_PROSPECT_POOL_TENANT_UUID` is unset — **FORBIDDEN:** falling back to Medshield or any customer beachhead tenant UUID for sales agent console, unauthenticated sales ingress, or Approvals queue hydration (cross-tenant bleed and empty-queue false negatives under RLS). Unscoped `prisma.ironboardCrmInteraction.findMany` from GLOBAL_ADMIN session without prospect-pool GUC is a **Mandate 4** violation even when the operator is platform admin — tenant isolation is database-enforced, not role-bypassed.

* **Mandate 31: Academic Fellowship Database Isolation — Tier-2 Never Shares Path B Surface:** When Academic Fellowship modules are present (see Feature **145**), fellows data must use **`FELLOWS_DATABASE_URL`** targeting Postgres schema **`academic_fellows`** (prefer role **`ironframe_fellows_app`** per `prisma/fellows/ops/01_create_role_and_grants.sql`) and the dedicated Prisma client at `@/lib/prismaFellows` — **FORBIDDEN:** importing `@/lib/prisma` from fellows routes for fellow rows; sharing primary `DATABASE_URL` user/database without `schema=academic_fellows` isolation; persisting lab **`lossExposureCents`** or estimated exposure registers as float or Decimal — Mission **01** **`estimatedExposureCents(sleCents, aroMilli)`** uses **`(sleCents * aroMilli) / 1000n`** BigInt floor only; treating lab completion badge as industry certification; auto-promoting fellows rubric submissions to Path B PROSPECT without operator HITL. **`assertFellowsDatabaseIsolated()`** in `lib/fellowsDatabaseUrl.ts` throws when fellows URL matches primary URL without academic schema. Empty fellows file stubs in a working tree are **deploy-blocking** — restore from version control before production promotion.
...
### Layout Refactor Notes (2026-09-22 Delta)

Today's delta delivers **Ironintel September 21 OSINT manifest refresh** (`ironintel-osint-2026-09-21-live` — Linux kernel KEV **due-date closure September 21**, Cisco ISE **CVE-2026-76460** post-deadline triage **day two** on **2026-09-22**, **Chromium V8 CVE-2026-87491** browser-fleet priority, CISA bulletin retirement **September 28**, DARS **2026-O0025 Rev 3** CMMC continuity, workday `riskMetricsCents` **5520000000**, **1388000000**, **1032000000**, **183200000** cents; SaaS disruption **5160000000**, **1456000000**, **1614000000**, **149400000** cents), **Sales touch stage filters** on Approvals desk (`?touch=TOUCH1|TOUCH2|TOUCH3`), **Touch 3 Value Drop DISPATCH gates** (`expectedTouch`, `salesTouch3Body.ts`), **Touch 2 queue card grid** with pending-draft deep links into shared editor, **Account Research Brief legacy gate hardening**, **video campaign October–November 2026 reschedule**, and **LinkedIn voice-pass Ready for publish** calendar updates — without altering the constitutional 22/48/30 tripane geometry on authenticated cockpit routes:

* **Ironintel OSINT manifest:** `Ironboard/src/knowledge/grcProfessionalResearch.manifest.json` — `manifestId: ironintel-osint-2026-09-21-live`, `generatedAt: 2026-09-21T08:00:00.000Z`; title **GRC Professional Workday Analysis — September 21 OSINT Sweep**; executive summary centers kernel due-date closure, ISE/Pixel/Acronis post-deadline triage, bulletin sunset countdown, V8 browser fleet, CMMC Rev 2 continuity; RAG **osint-01-cisco-ise-deadline-day** (Ironwatch **2026-09-21** post-deadline framing); **osint-02-linux-kernel-kev-triple** (CISA **September 18** alert); **`saas-01-dmz-ingress`** binds **`ironintel-osint-2026-09-21-live`**; `riskMetricsCents` unchanged string-encoded BigInt on workday and SaaS disruption docs (values above); peer `peerAleBaselineCents` unchanged (Finance **1800000000**, Healthcare **1210000000**, Technology **950000000**, Defense **2500000000**, Public Sector **1500000000**, Manufacturing **1400000000** cents).
* **Approvals touch filters:** `approvalDraftKinds.ts` — `parseApprovalTouchFilter()`, `approvalsHref(..., touch)`, `approvalsDraftHref(..., touch)`; `AdminApprovalDashboardClient.tsx` — touch toggle row with per-stage counts; empty-state hint when **TOUCH2** filter has zero pending drafts.
* **Touch 3 DISPATCH:** `approvalDispatchValidation.ts` — `expectedTouch` branch; `app/api/admin/approvals/[id]/route.ts` — `inferSalesTouchStage()` + `expectedTouch` on validate; `salesHumanVoice.ts` — optional `allowMissingPeerCta`.
* **Touch 2 pipeline UI:** `salesTouch2QueueCore.ts` — `pendingDraftId`, `pendingSubject` from pending TOUCH2 interactions; `SalesTouch2QueuePanel.tsx` — card grid, `openDraft()` → `approvalsDraftHref`.
* **Ironleads brief resilience:** `ironleadsAccountResearchBrief.ts` — `resolveAccountResearchBrief()` null on incomplete gates; `selectAccountResearchBriefForReport()` **`incomplete_persisted_gates`**; suspect report + operator panel optional **`outreach?.status`**.
* **Ops schedule:** `opsScheduleSeedSpecs.ts` — video V1 build **2026-10-16**, V1 publish **2026-10-21**, V2–V6 October/November dates; companion story bank **2026-10-14**; LinkedIn **2026-08-31**, **2026-09-04**, **2026-09-15** cards marked voice-pass ready.
* **Lab validation:** Run `npx tsx scripts/ingest-strategic-intel-manifest.ts` for **`ironintel-osint-2026-09-21-live`**; open `/dashboard/admin/approvals?kind=SALES&touch=TOUCH2` — verify touch counts and Touch 2 card opens draft editor; queue Touch 3 draft — verify DISPATCH rejects $4,999 body and accepts three-check Value Drop; run `tests/unit/ironleadsAccountResearchBrief.test.ts` incomplete gates case; run `tests/unit/publicAppUrl.test.ts`.

### Layout Refactor Notes (2026-09-21 Delta)

Prior-cycle delta delivered **Ironintel September 20 OSINT manifest refresh** (`ironintel-osint-2026-09-20-live` — Feature **148** historical), **prospect-pool CRM RLS scoping**, **Admin Approvals** sticky queue + Touch-2 panels below grid, **Integrity Hub** schema-lag soft-fail (Feature **146**), and **auth next-path sanitize** (Feature **147**) — superseded for Strategic Intel active binding by **`ironintel-osint-2026-09-21-live`** per Feature **149**.

### Layout Refactor Notes (2026-09-20 Delta)

Prior-cycle delta delivered **Ironintel September 19 OSINT manifest authorship** (`ironintel-osint-2026-09-19-live` — Feature **143** historical) — superseded by Feature **148** then **149**.

### Layout Refactor Notes (2026-08-11 Delta)

Prior-cycle delta delivered **Ops schedule GTM gate reschedule** (cold outreach gate **August 17**, First SalesTeam DISPATCH **August 24**), **LinkedIn Wed draft first-comment CTA refresh** (`wed-product-demo` body + research block), **Ironintel August 10 OSINT manifest operational follow-through** (LoadMaster/FortiOS post-deadline triage day one, ChainDrop day-seven, TeamCity post-deadline triage day three, BOD post-deadline triage day five, CMMC day **29**, UTS **3803750**-individual healthcare breach carryover), **Video campaign V1 kickoff and production script narrative links**, **Ironleads Promote-ready email gate simplification** (employer-domain check removed), and **apex guest marketing redirect** (`/marketing` instead of `/login`) without altering the constitutional 22/48/30 tripane geometry on authenticated cockpit routes:

* **Ops schedule GTM gate reschedule:** `opsScheduleSeedSpecs.ts` — `preOutreachMarketing2026SeedSpecs()` **Marketing — Cold outreach gate (after free marketing)** (`sourceRef: marketing/cold-outreach-gate`) rescheduled from **2026-08-10T15:00:00.000Z** to **2026-08-17T15:00:00.000Z** — wait for LinkedIn week-1 posts + Video Phase 0 before cold Path B DISPATCH; `ironframeRollout2026SeedSpecs()` **Rollout — First SalesTeam DISPATCH batch** (`sourceRef: rollout/first-salesteam-dispatch`) rescheduled from **2026-08-17T15:00:00.000Z** to **2026-08-24T15:00:00.000Z** — after cold outreach gate (**August 17**).
* **LinkedIn Wed first-comment CTA:** `linkedinDeskDraftCore.ts` — `LINKEDIN_WED_DRAFT_BODY` contrasts feature-list/color-dashboard tours vs **Evidence → Scenario → Estimated Exposure** (whole cents); single primary link `https://ironframegrc.com/product-demo`; hashtags `#GRC #BoardRisk #CyberGovernance #RiskQuantification #RiskManagement`; `LINKEDIN_WED_DRAFT_RESEARCH` adds **First comment** section with workflow-review CTA at `https://ironframegrc.com/register/contact` (post immediately after publish — not in main body); claim map cites Mon heatmap qualitative-theater context; held alternate `docs/marketing-strategy/linkedin-drafts-hold-board-reporting-gap.md`; pre-post checklist includes first-comment paste step.
* **Ironintel OSINT manifest (operational follow-through):** `Ironboard/src/knowledge/grcProfessionalResearch.manifest.json` — `manifestId: ironintel-osint-2026-08-10-live`, `generatedAt: 2026-08-10T08:00:00.000Z`; on operational date **2026-08-11**: LoadMaster CVE-2026-8037 and FortiOS CVE-2025-68686 **post-deadline forensic triage day one** (deadlines passed **August 10**); ChainDrop npm worm **day-seven** containment (`osint-02-chaindrop-day-six` chunk authorship day-six); TeamCity CVE-2026-63077 post-deadline triage **day three**; BOD 26-04 Langflow/Tomcat/N-able post-deadline triage **day five**; CMMC Phase II day **29** (**3** days to RFI close **August 14**); Minnesota PLC day **fifteen**; workday `riskMetricsCents`: **5360000000**, **1346000000**, **1000000000**, **178000000** cents; SaaS disruption doc: **5004000000**, **1420000000**, **1578000000**, **145800000** cents.
* **Video V1 production pack:** `publishingDeskDraftKind.ts` — `PUBLISHING_VIDEO_NARRATIVE_LINKS` Hub & budget group adds **V1 kickoff — Phase 0 + shot list** and **V1 production script — shots + AI prompts**; `opsScheduleSeedSpecs.ts` **Video Phase 0 — Style lock complete** due **2026-08-09** cites working pack with `href` deep link.
* **Ironleads email gate simplification:** `ironleadsAccountResearchBrief.ts` — removed `emailMatchesAccountDomain()`, `isPromoteReadyEmployerEmail()`, and `accountDomain` input; Email PASS uses `isPromoteReadyWorkEmail()` only; `ironleadsSuspectReportCore.ts` stops passing `deal?.accountDomain`; Kyber wrong-domain unit test removed from `tests/unit/ironleadsAccountResearchBrief.test.ts`.
* **Apex marketing redirect:** `app/page.tsx` — unauthenticated apex guests `redirect("/marketing")` instead of `/login`; signed-in operators still route to Integrity Hub.
* **Lab validation:** Open Ops Calendar — verify cold outreach gate **August 17** and First DISPATCH **August 24**; open `/dashboard/operations/publishing?desk=linkedin&li=wed-product-demo` — verify Wed body has single demo link and research block documents first-comment CTA; run `npx tsx scripts/ingest-strategic-intel-manifest.ts` for `ironintel-osint-2026-08-10-live`; confirm LoadMaster/FortiOS post-deadline triage day one attestation receipts on Strategic Intel dashboard.

### Layout Refactor Notes (2026-08-10 Delta)

Prior-cycle delta delivered **Ironintel August 10 OSINT manifest refresh** (dual BOD 26-04 KEV deadline day — LoadMaster CVE-2026-8037 and FortiOS CVE-2025-68686 remediation due **August 10**, ChainDrop day-six containment, FBI-CISA PLC critical infrastructure alert renewal AA26-097A, BOD 26-04 post-deadline forensic triage day four, TeamCity post-deadline triage day two, UTS **3803750**-individual healthcare breach carryover, CMMC day **28**), **Video campaign V1 kickoff and production script narrative links** on Publishing desk video tab, **Ops schedule Phase 0 working-pack href**, **Ironleads Promote-ready email gate simplification** (employer-domain check removed), and **apex guest marketing redirect** (`/marketing` instead of `/login`) without altering the constitutional 22/48/30 tripane geometry on authenticated cockpit routes:

* **Ironintel OSINT manifest:** `Ironboard/src/knowledge/grcProfessionalResearch.manifest.json` — `manifestId: ironintel-osint-2026-08-10-live`, `generatedAt: 2026-08-10T08:00:00.000Z`; RAG chunks pivot to dual BOD 26-04 KEV remediation deadline day — LoadMaster CVE-2026-8037 and FortiOS CVE-2025-68686 (`osint-01-dual-kev-deadline-day`), ChainDrop npm worm day-six containment (`osint-02-chaindrop-day-six`), JetBrains TeamCity CVE-2026-63077 post-deadline forensic triage day two (`osint-03-teamcity-post-deadline-triage`), CISA AA26-097A PLC critical infrastructure alert renewal August 9 (`osint-04-plc-critical-infrastructure-alert`), BOD 26-04 post-deadline forensic triage day four — Langflow, Tomcat, N-able (`osint-05-post-deadline-triage-day-four`), CMMC Phase II day **28** (`osint-06-cmmc-phase2-suspension-day28`), PleaseFix Intent Collision zero-click agentic browser carryover (`osint-07-pleasefix-intent-collision-carryover`), UTS **3803750**-individual healthcare breach carryover (`osint-08-uts-healthcare-breach-carryover`); workday `riskMetricsCents`: **5360000000**, **1346000000**, **1000000000**, **178000000** cents; SaaS disruption doc: **5004000000**, **1420000000**, **1578000000**, **145800000** cents; industry profile bumps — Finance `saasDisruptionExposureIndex` **98**; Healthcare **92**; Technology `regulatoryPressureIndex` **99**, `saasDisruptionExposureIndex` **99**; Defense **78**; Public Sector **82**; Manufacturing `regulatoryPressureIndex` **98**, `saasDisruptionExposureIndex` **94** (workday) / **99** (SaaS doc).
* **Video V1 production pack:** `publishingDeskDraftKind.ts` — `PUBLISHING_VIDEO_NARRATIVE_LINKS` Hub & budget group adds **V1 kickoff — Phase 0 + shot list** (`/docs/marketing-strategy/video-series/v1-kickoff-phase0-shotlist`) and **V1 production script — shots + AI prompts** (`/docs/marketing-strategy/video-series/v1-production-script`); video desk at `/dashboard/operations/publishing?desk=video` renders links — narrative index only, not auto-publish per Mandate 25.
* **Ops schedule Phase 0 href:** `opsScheduleSeedSpecs.ts` — `videoCampaign2026SeedSpecs()` **Video Phase 0 — Style lock complete** due **2026-08-09T18:00:00.000Z** synopsis cites working pack `/docs/marketing-strategy/video-series/v1-production-script`; `href: "/docs/marketing-strategy/video-series/v1-production-script"` on calendar card.
* **Ironleads email gate simplification:** `ironleadsAccountResearchBrief.ts` — removed `emailMatchesAccountDomain()`, `isPromoteReadyEmployerEmail()`, and `accountDomain` input; Email PASS uses `isPromoteReadyWorkEmail()` only; `mergeNamedBuyerIntoBriefMembers()` merges buyer/contact email without employer-domain filter; `ironleadsSuspectReportCore.ts` stops passing `deal?.accountDomain`; `tests/unit/ironleadsAccountResearchBrief.test.ts` — Kyber wrong-domain rejection test removed.
* **Apex marketing redirect:** `app/page.tsx` — unauthenticated apex guests `redirect("/marketing")` instead of `/login`; signed-in operators still route to Integrity Hub.
* **Lab validation:** Run `npx tsx scripts/ingest-strategic-intel-manifest.ts` for `ironintel-osint-2026-08-10-live`; open `/dashboard/operations/publishing?desk=video` — verify V1 kickoff and production script links; rebuild Account Research Brief on active SUSPECT — verify Email gate uses work-seat check without domain rejection; confirm LoadMaster and FortiOS **August 10** deadline attestation receipts and TeamCity post-deadline triage day two on Strategic Intel dashboard; visit apex `/` as guest — verify marketing landing instead of login redirect.

### Layout Refactor Notes (2026-08-08 Delta)

Prior-cycle delta delivered **Ironintel August 8 OSINT manifest refresh** (TeamCity KEV deadline day, ChainDrop day-four containment, PleaseFix Intent Collision zero-click agentic browser, UTS **3803750**-individual healthcare breach, LoadMaster KEV due **August 10**, BOD 26-04 post-deadline triage day two), **Video campaign V1 kickoff and production script narrative links**, **Ops schedule Phase 0 working-pack href**, and **Ironleads Promote-ready email gate simplification** without altering the constitutional 22/48/30 tripane geometry:

* **Ironintel OSINT manifest:** `manifestId: ironintel-osint-2026-08-08-live`, `generatedAt: 2026-08-08T08:00:00.000Z`; RAG chunks: `osint-01-teamcity-deadline-day` through `osint-08-windows-hello-entra-carryover`; workday `riskMetricsCents`: **5315000000**, **1332000000**, **988000000**, **175800000** cents; SaaS disruption doc: **4958000000**, **1406000000**, **1562000000**, **143400000** cents.
* **Lab validation:** Run ingest for `ironintel-osint-2026-08-08-live`; verify TeamCity deadline day and LoadMaster/FortiOS approaching deadline receipts.

### Layout Refactor Notes (2026-08-07 Delta)

Prior-cycle delta delivered **Ironintel August 7 OSINT manifest refresh** (BOD 26-04 policy + KEV deadline convergence day, ChainDrop day-three containment, AI browser zero-click and Windows Hello Entra persistence), **Video campaign V1 kickoff and production script narrative links** on Publishing desk video tab, **Ops schedule Phase 0 working-pack href**, and **Ironleads Promote-ready email gate simplification** (employer-domain check removed) without altering the constitutional 22/48/30 tripane geometry on authenticated cockpit routes:

* **Ironintel OSINT manifest:** `Ironboard/src/knowledge/grcProfessionalResearch.manifest.json` — `manifestId: ironintel-osint-2026-08-07-live`, `generatedAt: 2026-08-07T08:00:00.000Z`; RAG chunks pivot to ChainDrop npm worm day-three containment (`osint-01-chaindrop-day-three`), BOD 26-04 KEV remediation deadline day Langflow/Tomcat/N-able (`osint-02-kev-deadline-day`), BOD 26-04 agency policy update deadline August 7 (`osint-03-bod26-04-policy-deadline-day`), JetBrains TeamCity CVE-2026-63077 active CI/CD RCE (`osint-04-teamcity-cicd-rce`), Zenity AI browser zero-click indirect prompt injection (`osint-05-ai-browser-zero-click`), Windows Hello for Business Entra ID persistence (`osint-06-windows-hello-entra`), CMMC Phase II day **25** (`osint-07-cmmc-phase2-suspension-day25`), post-deadline KEV cluster (`osint-08-post-deadline-carryover`); workday `riskMetricsCents`: **5270000000**, **1318000000**, **976000000**, **173600000** cents; SaaS disruption doc: **4912000000**, **1391000000**, **1548000000**, **141200000** cents; industry profile bumps — Finance `saasDisruptionExposureIndex` **96**; Healthcare **90**; Technology `regulatoryPressureIndex` **99**; Defense **78**; Public Sector **82**; Manufacturing **93**/**99**.
* **Video V1 production pack:** `publishingDeskDraftKind.ts` — `PUBLISHING_VIDEO_NARRATIVE_LINKS` Hub & budget group adds **V1 kickoff — Phase 0 + shot list** and **V1 production script — shots + AI prompts**; video desk at `/dashboard/operations/publishing?desk=video` renders links — narrative index only, not auto-publish per Mandate 25.
* **Ops schedule Phase 0 href:** `opsScheduleSeedSpecs.ts` — **Video Phase 0 — Style lock complete** due **2026-08-09T18:00:00.000Z** cites working pack `/docs/marketing-strategy/video-series/v1-production-script`; `href` deep link on calendar card.
* **Ironleads email gate simplification:** `ironleadsAccountResearchBrief.ts` — removed employer-domain ownership gate; Email PASS uses `isPromoteReadyWorkEmail()` only.
* **Apex routing comment:** `app/page.tsx` — comment clarifies apex guests → `/marketing`, authenticated apex → Integrity Hub.
* **Lab validation:** Run ingest for `ironintel-osint-2026-08-07-live`; verify V1 video links and Email gate work-seat check.

### Layout Refactor Notes (2026-08-06 Delta)

Prior-cycle delta delivered **Ironintel August 6 OSINT manifest refresh** (ChainDrop day-two containment + KEV deadline cluster), **LinkedIn Publishing desk three-slot catalog** with Drafts/Published two-column chrome, **Ops schedule calendar catch-up closures**, **queue review idempotency and Medshield narrate exclusion**, and **apex guest marketing redirect** without altering the constitutional 22/48/30 tripane geometry on authenticated cockpit routes:

* **Ironintel OSINT manifest:** `Ironboard/src/knowledge/grcProfessionalResearch.manifest.json` — `manifestId: ironintel-osint-2026-08-06-live`, `generatedAt: 2026-08-06T08:00:00.000Z`; RAG chunks pivot to ChainDrop npm worm day-two containment (`osint-01-chaindrop-day-two`), N-able KEV deadline day (`osint-02-n-able-kev-deadline-day`), TeamCity CI/CD active RCE (`osint-03-teamcity-cicd-rce`), Langflow and Tomcat deadline eve (`osint-04-langflow-tomcat-deadline-eve`), BOD 26-04 policy deadline August 7 (`osint-05-bod26-04-policy-deadline`), CMMC Phase II day **24** (`osint-06-cmmc-phase2-suspension-day24`), Fortinet approaching deadline (`osint-07-fortinet-deadline`), post-deadline KEV cluster (`osint-08-post-deadline-carryover`); workday `riskMetricsCents`: **5228000000**, **1306000000**, **972000000**, **172400000** cents; SaaS disruption doc: **4865000000**, **1378000000**, **1536000000**, **139800000** cents; industry profile bumps — Finance `saasDisruptionExposureIndex` **95**; Healthcare **89**; Technology `regulatoryPressureIndex` **99**; Defense **78**; Public Sector **82**; Manufacturing **93**/**99**.
* **LinkedIn three-slot catalog:** `linkedinDeskDraftCore.ts` — `LINKEDIN_DRAFT_CATALOG` (`mon-heatmap`, `wed-product-demo`, `fri-collection`); `listLinkedInDeskDraftsCore()` joins `prisma.opsActivity` by `opsSourceRef` for posted archive; per-slot APP_DOCS slugs + repo files (`linkedin-drafts-mon-heatmap.md`, `linkedin-drafts-wed-product-demo.md`, `linkedin-drafts-week-1.md` for Fri); Fri legacy slug mirror on save; `LINKEDIN_FRI_DRAFT_*`, `LINKEDIN_WED_DRAFT_*` default templates; `linkedInDraftCatalogEntry()` resolves legacy seed aliases (`suggested`, `friday`, `monday`, `heatmap`).
* **LinkedIn Publishing desk UI:** `PublishingDeskClient.tsx` — two-column **Drafts awaiting paste** / **Published** grid; `loadLinkedinDraftList()` via `GET /api/admin/operations-hub/linkedin/drafts`; `?li=` deep link with auto-land on active draft (not Published); **Reset to template** replaces Load suggested seed; Copy/Save/citation checkbox disabled on Published slots; `app/api/admin/operations-hub/linkedin/drafts/route.ts` — new list endpoint; draft route accepts `?id=` and `?reset=1`.
* **Ops schedule catch-up:** `opsScheduleSeedSpecs.ts` — DORA supervision **DONE** **2026-08-06** promoted to `research.ironframegrc.com/briefings/2026-07-16-research-dora-supervision`; GF-2026-001 editorial + product-boundary **PASS** **2026-08-05**; Control-First newsletter outline + quarantine draft `2026-08-06-draft-newsletter-control-first-evidence-theater.md`; EU AI Act alignment + board packet **PASS** in `docs/ops/eu-ai-act-claim-alignment-2026-08.md`; stakeholder benefit-map **CANCELLED**; partner provisioning **DEFERRED**; LinkedIn Mon **DONE** **2026-08-06** (`mon-heatmap` Published); LinkedIn Fri due **August 8**; cold outreach gate **August 10**; First DISPATCH **August 17**; Stripe lifecycle **September 28**.
* **Queue review idempotency:** `opsScheduleCore.ts` — `ensureQueueReviewActivity()` skips `/medshield/i` filenames; latest row per `sourceRef` — closed **DONE**/**CANCELLED** rows not reopened; `synopsisForQueueDraft()` Medshield label updated.
* **Apex marketing redirect:** `app/page.tsx` — unauthenticated apex guests `redirect("/marketing")` instead of `/login`; authenticated operators unchanged (Integrity Hub).
* **Lab validation:** Run `npx tsx scripts/ingest-strategic-intel-manifest.ts` for `ironintel-osint-2026-08-06-live`; open `/dashboard/operations/publishing?desk=linkedin&li=fri-collection` — verify Fri draft active while Mon in Published; stage Medshield narrate dump — verify no auto calendar review card.

### Layout Refactor Notes (2026-08-06 Delta)

Prior-cycle delta delivered **Ironintel August 5 OSINT manifest refresh** (ChainDrop npm worm tier-zero pivot), **Ops Publishing desk five-tab expansion** (LinkedIn founder workbench), **Ironleads Promote-ready email gate and brief selector**, **mailto/directory published-email discovery**, **trade-show event signal extraction**, **returnToQueue hard navigation**, **TypeScript project boundary hardening**, and **duplicate artifact purge** without altering the constitutional 22/48/30 tripane geometry on authenticated cockpit routes:

* **Ironintel OSINT manifest:** `Ironboard/src/knowledge/grcProfessionalResearch.manifest.json` — `manifestId: ironintel-osint-2026-08-05-live`, `generatedAt: 2026-08-05T08:00:00.000Z`; RAG chunks pivot to ChainDrop npm worm (`osint-01-chaindrop-npm-worm`), WordPress post-deadline attestation (`osint-02-wordpress-post-deadline`), CHAOS healthcare extortion (`osint-03-chaos-healthcare-extortion`), CMMC Phase II day **23** (`osint-04-cmmc-phase2-suspension-day23`), BOD 26-04 policy deadline August 7 (`osint-05-bod26-04-policy-deadline`), N-able RMM carryover (`osint-06-n-able-rmm-carryover`), Fortinet approaching deadline (`osint-07-fortinet-deadline`), post-deadline KEV cluster (`osint-08-post-deadline-carryover`); workday `riskMetricsCents`: **5186000000**, **1294000000**, **968000000**, **171200000** cents; SaaS disruption doc: **4823000000**, **1367000000**, **1528000000**, **138900000** cents; industry profile `saasDisruptionExposureIndex` bumps — Finance **94**, Healthcare **89**, Technology `regulatoryPressureIndex` **99**, Defense **78**, Public Sector **82**, Manufacturing **93**/**99**.
* **Publishing desk LinkedIn tab:** `publishingDeskDraftKind.ts` — `PublishingDeskTab` adds `linkedin`; `PUBLISHING_LINKEDIN_DRAFTS_HREF`, `PUBLISHING_LINKEDIN_CADENCE_HREF`; `PublishingDeskClient.tsx` — title/body/research editors, Save, Copy body, Load suggested seed; `linkedinDeskDraftCore.ts` — `composeLinkedInDeskMarkdown()`, `parseLinkedInDeskMarkdown()`, `seedSuggestedLinkedInDeskDraftCore()`, repo sync to `docs/marketing-strategy/linkedin-drafts-week-1.md`; `app/api/admin/operations-hub/linkedin/draft/route.ts` — perimeter workforce gate; `OperationsHubClient.tsx` quick link **LinkedIn drafts**; `opsScheduleLinks.ts` — `marketing/linkedin` → `?desk=linkedin`.
* **Ironleads email gate:** `ironleadsAccountResearchBrief.ts` — `isPromoteReadyWorkEmail()`, `selectAccountResearchBriefForReport()`, `GENERIC_COMPANY_INBOX_LOCALS`, `pathBHold` input; Email gate distinguishes promote-ready work seat from company intake; `ironleadsSuspectReportCore.ts` — auto-persist brief upgrades via selector; `scripts/dev/_remediate-brief-gates.ts` — batch rebuild Email/Buyer gates for legacy SUSPECT rows.
* **Ironleads published-email discovery:** `ironleadsBuyingCommitteeExtract.ts` — `extractMailtoEmails()`, `extractDirectoryEmailClues()`, `discoverPublishedEmails()`, `publishedEmailMatchesPerson()`, `isRoleLocalPart()`, `extractTradeShowAndEventSignals()`, expanded `RESEARCH_PATHS` (legal/terms/events/resources); single first.last publish sufficient for schema inference; `ironleadsBuyingCommitteeResearchCore.ts` — `publishedEmailDiscovery`, `tradeShowAndEventSignals` metadata on buying committee.
* **Ironleads portal UX:** `SuspectOperatorEditPanel.tsx` — `returnToQueue()` via `window.location.replace` (fixes soft-router HOLD stuck on dossier); `enrichResultMessage()` Prospeo/Apollo placeholder warnings; **Already PROSPECT** promote button guard; `ironleads/page.tsx` — `?promoted|held|restored|discarded=` + `company` query params; `[contactId]/page.tsx` — PROSPECT redirect unless `?dossier=1`; OSINT title noise discard encodes contactId in query.
* **Dev environment hygiene:** `.cursorignore` — excludes nested worker packages, `docs/briefing-queue/`, heavy trees from tsserver; `.gitignore` — blocks accidental ` copy` duplicates; `tsconfig.json` — explicit include paths for `app/`, `lib/`, `components/` only; excludes `scripts`, `tests`, `docs`, duplicate copy artifacts; mass deletion of Explorer **copy** duplicate files from repo root.
* **Unit tests:** expanded `ironleadsAccountResearchBrief.test.ts` (`isPromoteReadyWorkEmail`, `selectAccountResearchBriefForReport`, `pathBHold`, company intake UNKNOWN); expanded `ironleadsBuyingCommitteeExtract.test.ts` (mailto, directory labels, trade-show signals, `publishedEmailMatchesPerson`).

### Layout Refactor Notes (2026-08-05 Delta)

Prior-cycle delta delivered **Ironintel August 3 OSINT manifest refresh**, **Ops Publishing desk four-tab expansion** (research papers + video), **Ironleads leadership OSINT search cascade**, **domain mail footprint verification**, **buying-committee batch auto-continue**, **suspect dossier mode and queue decision banners**, **Prospeo enrich placeholder warnings**, **GF research-site ledger kind classification**, and **August 4 ops schedule archive promotions** without altering the constitutional 22/48/30 tripane geometry on authenticated cockpit routes:

* **Ironintel OSINT manifest:** `Ironboard/src/knowledge/grcProfessionalResearch.manifest.json` — `manifestId: ironintel-osint-2026-08-03-live`, `generatedAt: 2026-08-03T08:00:00.000Z`; RAG chunks pivot to N-able N-central RMM god-mode bypass (`osint-01-n-able-rmm-god-mode`), WordPress Core BOD 26-04 deadline day (`osint-02-wordpress-deadline-day`), CaptiveCrunch captive-portal carryover (`osint-03-captivecrunch-carryover`), ShinyHunters healthcare SSO extortion (`osint-04-shinyhunters-healthcare`), CMMC Phase II day **21** (`osint-05-cmmc-phase2-suspension-day21`), Fortinet FortiOS approaching deadline (`osint-06-fortinet-deadline`), Minnesota PLC day **seven** (`osint-07-minnesota-plc-day-seven`), BOD 26-04 post-deadline verification carryover (`osint-08-post-deadline-carryover`); workday `riskMetricsCents`: **5042000000**, **1248000000**, **932000000**, **165700000** cents; SaaS disruption doc: **4675000000**, **1319000000**, **1482000000**, **132400000** cents; industry profile `saasDisruptionExposureIndex` bumps — Finance **92**, Healthcare **87**, Technology `regulatoryPressureIndex` **97**, Defense **75**, Public Sector **80**, Manufacturing **90**/**98**.
* **Publishing desk four-tab:** `publishingDeskDraftKind.ts` — `PublishingDeskTab` `briefings | newsletters | research | video`; `isResearchDeskDraft()`, `publishingDeskTabForQueueDraft()`, `PUBLISHING_VIDEO_NARRATIVE_LINKS`; `PublishingDeskClient.tsx` draft reader modal (Escape dismiss, auto-open on `?draft=`); `publishedLedgerKind.ts` — `classifyPublishedLedgerItem()`; `briefingArchiveDirectory.ts` + `BriefingArchiveDirectory.tsx` vertical archive sidebar; `opsScheduleLinks.ts` routes research drafts to `?desk=research`.
* **Ironleads leadership search:** `googleLeadershipSearchClient.ts` — Brave → SerpAPI → Google CSE cascade; `ironleadsLeadershipSearchAllowlist.ts`, `ironleadsLeadershipSearchHitRefine.ts`; `domainMailFootprint.ts` MX/SMTP probes; `buildEmailPermutationCandidates()` + `inferEmailLocalPattern()` replace single initial-last guess; browser-like User-Agent on site scrape; `googleLeadershipSearch` metadata on buying committee when scrape thin.
* **Ironleads portal UX:** `IronleadsPortalClient.tsx` — queue decision banners (`promoted|held|restored|discarded`); Research button auto-continues up to **8** batches (`researchBatchLimit: 5`, `forceResearch` first click); active row links use `?dossier=1`; `SuspectOperatorEditPanel.tsx` — `returnToQueue()` after promote/hold/discard; `enrichResultMessage()` warns on remaining `@ironleads.local`; `[contactId]/page.tsx` — PROSPECT redirect unless `?dossier=1`.
* **Prospeo enrich:** `ironleadsProspeoEnrichCore.ts`, `prospeoEnrichmentClient.ts` — NO_MATCH copy guides LinkedIn URL or manual paste; `.env.example` documents `PROSPEO_API_KEY`.
* **GF research site:** `app/gf-research/briefings/[slug]/page.tsx` — archive directory column; `app/gf-research/research-papers/page.tsx` — industry research index; `ResearchSiteChrome.tsx` nav links; `public/rss.xml` syndication updates for August 4 promotions.
* **Ops schedule seed:** `opsScheduleSeedSpecs.ts` — CPS 230 MSP contracts, AUSTRAC Tranche 2, CSRD Omnibus ESRS, SEC Item 105 filings, GRC evolution/pain research marked **DONE + archived 2026-08-04** with canonical `research.ironframegrc.com/briefings/` URLs.
* **Unit tests:** `googleLeadershipSearchClient.test.ts`, `ironleadsLeadershipSearchHitRefine.test.ts`, `ironleadsResearchBatchSelect.test.ts`, `domainMailFootprint.test.ts`, `publishedLedgerKind.test.ts`, `publishingDeskDraftKind.test.ts`, `briefingArchiveDirectory.test.ts`, updated `ironleadsAccountResearchBrief.test.ts`, `ironleadsBuyingCommitteeExtract.test.ts`, `ironleadsAboutPageExtractSmoke.spec.ts` (e2e).

### Layout Refactor Notes (2026-08-03 Delta)

Prior-cycle delta delivered **Ironintel August 2 OSINT manifest refresh**, **Ironleads SUSPECT readiness ranking**, **MSSPProviders card-block paste import**, **client bundle drift recovery**, **Supabase middleware single-session refresh**, **founder pitch practice audio surfaces**, **Stripe COMMAND_TIER plan_sku metadata**, and **operator-only MSSP/Clutch directory collectors** without altering the constitutional 22/48/30 tripane geometry on authenticated cockpit routes:
...
* **Approvals API:** `app/api/admin/approvals/[id]/route.ts` — passes `company` and `acknowledgeOperatorSelfDispatch` into dispatch gate.
* **Workflow-review async assist:** `workflowReviewCallAssistCore.ts` — `CallAssistSource` type; SaaS KB first in `assistWorkflowReviewQuestion()`; `assistWorkflowReviewQuestionAsync()` / `runWorkflowReviewCallAssistAsync()`; narrowed multi-tenant pocket regex (architecture-only, capacity routes to SaaS KB).
* **Grounded pocket LLM:** `app/lib/server/workflowReviewPocketAssistLlm.ts` — `buildPocketGroundingPack()`, `groundedPocketAssistFromLlm()`, `pocketRefuseAnswer()`; Gemini Flash `generateObject` with `groundedAssistSchema`; imports full commercial spine constants including `COMMAND_MULTI_SUBTENANTS`, `PAID_ENCLAVES_TO_FILL_MULTI`.
* **Workflow-review call UI:** `WorkflowReviewCallClient.tsx` — default channel **zoom**; `localStorage` key `ironframe.live.channel`; select order Zoom (default) → Meet → Teams → Other.
* **Talk-track panel:** `WorkflowReviewTalkTrackPanel.tsx` — tenant capacity row (**1+2** Path B, Paid Enclave **350000** cents/yr list display, never invent soft maxes).
* **SaaS call KB:** `lib/ironframeProductKnowledge/saasCallKnowledgeBase.ts` — helper functions for entity stacking, Core→Paid→Multi counting, Multi eligibility, capacity, Partner book, fair-use WORM/ingest; `saasPocketTopicCatalog()`; **20+** regex entries; `collectSaasCallKnowledgeHits()`.
* **Commercial spine:** `lib/ironframeProductKnowledge/commercial.ts` — `COMMAND_MULTI_SUBTENANTS` (**9**), `PAID_ENCLAVES_TO_FILL_MULTI` (**6**).
* **Ops Calendar filters:** `OperationsHubClient.tsx` — `calendarDueFrom`, `calendarDueTo`, `calendarFilterActive`, **Today · WF review** preset (sets today bounds + search `WF review`), **Next 7 days** preset, **Clear filters** button; Done column slice respects date filter.
* **Ops schedule seeds:** `opsScheduleSeedSpecs.ts` — rollout pre-outreach gate, SMS smoke, ICP shortlist marked **DONE** with outcome receipts; marketing live-surfaces and Path B Stripe confirm **DONE**; warm-network asks **CANCELLED**.
* **Operator library:** `operatorLibraryCatalog.ts` — `docsRoot: "ops"` type; **windows-workstation-migration** entry; `loadOperatorLibraryMarkdown.ts` resolves `docs/ops/` path.
* **Unit tests:** `tests/unit/approvalDispatchValidation.test.ts` — HOLD, ironleads.local, dry-run ack, C1 locks; `tests/unit/saasCallKnowledgeBase.test.ts` — stack math pinned to commercial.ts; `tests/unit/workflowReviewPocketAssistLlm.test.ts` — grounding pack and refuse path; updated `workflowReviewCallAssistCore.test.ts`.

### Layout Refactor Notes (2026-07-31 Delta)

Today's delta delivers **Control-to-Capital docs federation**, **Heatmap Amnesty public landing**, **marketing homepage copy realignment**, and **operator library catalog expansion** without altering the constitutional 22/48/30 tripane geometry on authenticated cockpit routes:

* **IronBoard docs matrix:** `Ironboard/src/index.ts` `buildDocsFederationMatrix()` loads `docs/sales/control-to-capital-market-narrative.md` alongside TAS, TRD, pricing, competitive map, and message constitution; matrix section header **CONTROL-TO-CAPITAL MARKET NARRATIVE (CLAIM BANS + BUYER-SAFE WEDGE)**; loaded file count logged at startup.
* **Perimeter Docker federation:** `Ironboard/Dockerfile` `COPY docs ./docs`; `.dockerignore` negates blanket `*.md` ignore for `docs/` tree; `.github/workflows/deploy-perimeter-workers.yml` triggers on `docs/**` and `.dockerignore` changes.
* **Heatmap Amnesty route:** `app/marketing/heatmap-amnesty/page.tsx` — metadata title **Heatmap Amnesty | Ironframe**; renders `HeatmapAmnestyLanding` with `data-ironframe-surface="heatmap-amnesty"`.
* **Heatmap Amnesty landing sections:** Hero **Amnesty for color-coded guesswork**; **The decision gap** — stoplight tiles vs CFO capital conversation; **Control-to-capital loop** three-step grid (Estimate exposure → Link the chain → Keep walls hard); close **Grant yourself amnesty** with workflow review CTA to `SALES_CONTACT_PATH`.
* **Marketing homepage nav:** `MarketingHomepage.tsx` adds **Heatmap Amnesty** link to `/marketing/heatmap-amnesty` between Proof and product-demo links; hero and board-problem copy reframed to estimated dollar-loss exposure in whole cents with heatmaps as context.
* **Operator library catalog:** `operatorLibraryCatalog.ts` — Pre-outreach set adds **Ops surface map** slug `ops-surface-map`; Enablement set adds **Control-to-Capital narrative**, **Heatmap Amnesty campaign**, and live href **Heatmap Amnesty (live page)**; `MARKDOWN_SLUG_ALIASES` maps `design-partner-ops-surface-map` → `ops-surface-map`.
* **SaaS call KB:** `saasCallKnowledgeBase.ts` — `ale-reporting` and `capacity-clients-tenants` entries align with control-to-capital claim sheet; capacity answer cites Path B **1** Primary + **2** Subtenant enclave entitlement without inventing platform-wide client ceilings.
* **Tailwind animation:** `tailwind.config.ts` — `fadeIn` keyframe (`opacity 0→1`, `translateY 6px→0`); `animation.fadeIn` **0.55s ease-out both** for staggered public landing hero.
* **Unit tests:** `tests/unit/workflowReviewCallAssistCore.test.ts` — capacity/load questions match SaaS KB entitlement language; banNote cites SaaS KB on capacity hits.

### Layout Refactor Notes (2026-07-20 Delta)

Prior-cycle delta delivered **Ironleads SUSPECT location enrichment**, **tri-track HITL Approvals desk**, and **Ops Calendar checklist expansion** without altering the constitutional 22/48/30 tripane geometry on authenticated cockpit routes:
...
`shouldBlockProductionIngress` returns true when:
  ...
  4. Pathname is **not** a narrow public funnel path (`isPublicCloudIngressPath`: `/`, `/terms`, `/privacy`, `/pricing`, `/marketing`, `/marketing/*`, `/resources`, `/resources/*`, `/sales-agent-portal`, `/register/*`, auth surfaces, `/legal/accept`, `/account/billing-hold`, `/docs`, `/governance-frame`, `/api/auth/callback`)
...
* **Exact Screen Coordinates:** Root URL `/` — marketing redirect for apex guests; tripane Command Center for authenticated operators with RBAC clearance.
* **Operational Purpose:** Sends unauthenticated apex visitors to the public **`/marketing`** funnel while preserving the full 19-agent workforce grid for credentialed operators on the same route when signed in.
* **Technical Mechanics:** `app/page.tsx` resolves `resolveDashboardAccess()`:
  * Apex guest (no tenant subdomain) → `redirect("/marketing")` — public Control-to-Capital marketing hero, regulatory brief cards, and **CONSOLE INGRESS ──►** link to `/login`
  * Apex allowed operator → `DashboardHomeClient` inside `DashboardGroupShell` with `carbonMitigatedValueCents` passed as **BigInt** from `resolveDashboardMitigatedValueCents`
  * Tenant subdomain hosts continue to route to Command Post per existing envelope
...
* **Step-by-Step Lab Validation:**
  1. Open `/` on apex host in a private browser window — verify redirect to `/marketing` with **Ironframe: The Immutable Standard for AI-Driven GRC** hero.
  2. Click **CONSOLE INGRESS** — verify navigation to `/login`.
  3. Sign in with RBAC-cleared operator on apex — verify tripane Command Center replaces marketing layout on `/`.
...
* **Manifest Entry Example:** Current live manifest `ironintel-osint-2026-07-31-live` (July 31, 2026 OSINT sweep) — supersedes prior July 17/20 manifests after `npx tsx scripts/ingest-strategic-intel-manifest.ts`. All `riskMetricsCents` fields are string-encoded BigInt integers (e.g. median annual GRC program **4895000000** cents on workday analysis doc).
...
* **Technical Mechanics:** `app/lib/approvalDraftKinds.ts` — shared kind labels, filter parsing (`CS` alias → `CUSTOMER_SUCCESS`), `approvalsHref()`, card/badge/banner class helpers.
  ...
  * **2026-09-20 prospect-pool scope (Mandate 30):** `fetchPendingApprovalDrafts`, `countPendingApprovalDraftRows`, `parkHoldCompanyPendingSalesDrafts`, and `resolveIncomingQuery` execute inside **`withProspectPoolTenant()`** — unscoped Prisma against Ironboard CRM returns zero rows under RLS
  ...
  * `AdminApprovalDashboardClient.tsx` — `Suspense` wrapper; kind filter chips; sort order SALES → SUPPORT → CUSTOMER_SUCCESS; editable `proposedReply` textarea; dispatch button label reflects selected kind meta (`Approve & dispatch Sales outreach`, etc.); **2026-09-20 layout:** review queue column `max-h-[70vh] overflow-y-auto lg:sticky lg:top-0`; **`OutreachReplyReceiptPanel`** and **`SalesTouch2QueuePanel`** render in full-width footer block when `kindFilter === "SALES" || kindFilter === "ALL"` (always visible for sales operators, not only when a draft is selected); **2026-09-22 touch filters:** `?touch=TOUCH1|TOUCH2|TOUCH3` via `parseApprovalTouchFilter()` and `touchCounts` badges; `expectedTouch` on dispatch preview (Feature **150**)
  * **2026-08-01 dispatch perimeter (Mandate 17):** `approvalDispatchValidation.ts` — `validateApprovalDispatch()` receives `company`, `acknowledgeOperatorSelfDispatch`, and **`expectedTouch`**; blocks `SALES_DISPATCH_HOLD_COMPANIES` (e.g. **blueradius**); blocks `@ironleads.local` on EMAIL; requires dry-run acknowledge for operator inboxes; Sales EMAIL C1 locks on Touch **1**–**2** ($4,999/Command Design Partner + workflow review CTA, no free pilot); Touch **3** Value Drop three-check body via `salesTouch3Body.ts` (no economics pitch); `preferredSalesDispatchChannel()` auto-switches harvest placeholders to SMS when phone present; UI surfaces HOLD banner, placeholder warning, and dry-run checkbox
...
* **Step-by-Step Lab Validation:**
  ...
  5. Attempt DISPATCH to `@ironleads.local` on EMAIL — verify validation error blocks send.
  6. Select BlueRadius company draft — verify rose HOLD banner and validation error on DISPATCH.
  7. Set dry-run inbox without acknowledge checkbox — verify validation error; check box — verify DISPATCH proceeds for smoke test only.
  8. Run `tests/unit/approvalDispatchValidation.test.ts` — HOLD, C1 locks, and channel preference pass.
...
* **Technical Mechanics:**
  ...
  * **2026-08-01 date-range filters:** `OperationsHubClient.tsx` — `calendarDueFrom` / `calendarDueTo` YYYY-MM-DD inclusive filter on `dueAt` (fallback `completedAt`); `calendarFilterActive` combines text + date; **Today · WF review** preset sets today's bounds and search term; **Next 7 days** rolling window preset; **Clear filters** resets all; Done column shows full filtered set when any filter active (not only text search)
...
* **Technical Mechanics:**
  ...
  * **Async assist path (2026-08-01):** `assistWorkflowReviewQuestionAsync()` and `runWorkflowReviewCallAssistAsync()` — API awaits async assist on LIVE questions; `CallAssistSource`: `saas_kb` | `pocket_lock` | `miss` | `grounded_llm`
  * **Lookup order:** SaaS KB (`lookupSaasCallKnowledge`) runs **before** POCKET_QA pocket locks so capacity/packaging questions never fall through to generic diligence filler; multi-tenant pocket regex narrowed to architecture/isolation only
  * **Grounded LLM fallback:** `workflowReviewPocketAssistLlm.ts` — `buildPocketGroundingPack()` injects full commercial spine + curated SaaS cards; Gemini Flash `generateObject` at `temperature: 0`; exact `saas_kb` and `pocket_lock` hits never reach LLM; `pocketRefuseAnswer()` when API unavailable
  * SaaS KB delegation: **20+** entries covering entity stacking, Core→Paid→Multi (**6** Paid enclaves fill Multi from Core's **3** included), Multi/Enterprise flat books, Partner book, fair-use WORM/ingest; Path B **1** Primary + **2** Subtenant enclaves; Core **4** entities at **3500000** cents/yr planned GA
  * Default call channel **Zoom** (`ironframe.live.channel` localStorage); Teams Graph remains optional post-call transcript (Feature 107)
...
* **Step-by-Step Lab Validation:** Follow [pre-outreach run order R5–R6](../sales/design-partner-pre-outreach-run-order.md) — level bar moves, words in buffer, recap appears, Push to calendar creates `[WF review]` cards.
  1. Ask Live Q&A **"Can we load 4 tenants on Path B?"** — verify SaaS KB capacity answer cites **1+2** hard cap and **499900** cents flat, not unlimited upload.
  2. Ask **"So after the first 3 subtenants, $3,500 each?"** — verify `core-paid-to-multi-stack` id with **6** Paid enclaves and **5500000** cents/yr Multi quote.
  3. Confirm default channel is **Zoom** on fresh load; switch to Meet — reload — verify localStorage persistence.
  4. Run `tests/unit/saasCallKnowledgeBase.test.ts` and `tests/unit/workflowReviewPocketAssistLlm.test.ts` — stack math and grounding refuse path pass.
...
* **Operational Purpose:** Curated directory of design-partner GTM playbooks (run order, glossary, offer sheet, protocol, control-to-capital claim sheet, heatmap amnesty campaign) plus Ops tools links — not the partner `/docs` packet and not `docs/end-users/` “Extended operator library.”
* **Technical Mechanics:**
  * Catalog: `app/lib/operations/operatorLibraryCatalog.ts` (`OPERATOR_LIBRARY_SETS`)
  * Loader: `loadOperatorLibraryMarkdown.ts` reads `docs/sales/*`, `docs/qa/*`, and `docs/ops/*` by slug (`docsRoot` field)
  * **Pre-outreach set (2026-08-01):** `windows-workstation-migration` → `docs/ops/windows-workstation-migration.md` — pull, secrets, Task Scheduler, GTM smoke on Windows Cursor box
  * **Pre-outreach set (2026-07-31):** `ops-surface-map` → `design-partner-ops-surface-map.md` — one screen → one job surface map with daily **5**-minute operator check
  * **Enablement set (2026-07-31):** `control-to-capital` → `control-to-capital-market-narrative.md`; `heatmap-amnesty` → `heatmap-amnesty-campaign.md`; `heatmap-amnesty-live` href → `/marketing/heatmap-amnesty`
  * Slug alias: `design-partner-ops-surface-map` → `ops-surface-map` in `MARKDOWN_SLUG_ALIASES`
* **Agent Boundary:** Read-only markdown for operators; DISPATCH and Path B remain separate Approvals / onboarding surfaces.
* **Step-by-Step Lab Validation:** Open library → open **Ops surface map**, **Control-to-Capital narrative**, and **Heatmap Amnesty campaign** slugs → confirm markdown renders; click **Heatmap Amnesty (live page)** → verify `/marketing/heatmap-amnesty` loads without auth.

---

<a id="marketing-004"></a>

### 🔥 Feature 109: Heatmap Amnesty Public Landing (Priority-1 Path B Demand Gen)
* **GRC Function ID:** `MARKETING-004`
* **Exact Screen Coordinates:** `/marketing/heatmap-amnesty` — full-page public scroll; `PublicApexNav` chrome; secondary nav link from `/marketing` homepage **Heatmap Amnesty** anchor.
* **Operational Purpose:** Priority-1 outbound landing for CISO/CFO buyers who still defend color-coded risk heatmaps at board risk committees. Positions **estimated dollar-loss exposure in whole cents** — with visible assumptions — as the capital allocation decision layer while permitting heatmaps to remain optional context. CTA is **Schedule {10-15} min workflow review** to `SALES_CONTACT_PATH` — not a free pilot or product demo circus.
* **Technical Mechanics:**
  * Route: `app/marketing/heatmap-amnesty/page.tsx` — `Metadata.title: Heatmap Amnesty | Ironframe`; description cites whole-cent board decision layer and Command Design Partner workflow review
  * Component: `app/components/marketing/HeatmapAmnestyLanding.tsx` — `data-ironframe-surface="heatmap-amnesty"`; grid background radial gradient; staggered `animate-fadeIn` hero (eyebrow **Heatmap Amnesty**, headline **Amnesty for color-coded guesswork**)
  * Spine imports: `CUSTOMER_FACING_PATH_B_SKU`, `DESIGN_PARTNER_DEFAULT_WINDOW_DAYS`, `WORKFLOW_REVIEW_CTA_MINUTES`, `formatPathBUsd()` from `lib/ironframeProductKnowledge/commercial.ts`
  * Copy lock: `docs/sales/heatmap-amnesty-campaign.md` + `docs/sales/control-to-capital-market-narrative.md` (Mandate 16)
  * Control-to-capital loop steps: **01 Estimate exposure** (whole-cent ranges, not float theater or single “true ALE”); **02 Link the chain** (controls, evidence, owners, remediation); **03 Keep walls hard** (subsidiary/client isolation at platform boundary)
  * Footer SKU line: `{Command Design Partner}: {formatPathBUsd()} · {90}-day co-builder window — not a free pilot`
* **Agent Boundary:** **Ironcore** marketing surface; **Irontrust** (Agent 3) whole-cent language only in copy — no persisted ALE on public route; SalesTeam drafts for email sequence remain HITL-only per Mandate 10.
* **Financial note:** Path B on-ramp displays **499900** cents via `formatPathBUsd()` (**$4,999** display) — internal storage and Stripe metadata use `DESIGN_PARTNER_PATH_B_CENTS` string **499900**; never float.
* **Step-by-Step Lab Validation:**
  1. Open `/marketing/heatmap-amnesty` in private browser — verify hero headline and primary CTA link to contact/workflow review path.
  2. Confirm **The decision gap** section cites CFO conversation vs stoplight tiles.
  3. Confirm control-to-capital loop three-step grid renders on sm+ breakpoints.
  4. From `/marketing` — click **Heatmap Amnesty** nav link — verify navigation without auth redirect.
  5. Cross-check copy against `docs/sales/heatmap-amnesty-campaign.md` — no forbidden claims from Mandate 16 ban list.

---

<a id="marketing-005"></a>

### 📊 Feature 110: Control-to-Capital Market Narrative Federation (IronBoard + Operator Library)
* **GRC Function ID:** `MARKETING-005`
* **Exact Screen Coordinates:** No public UI — federated into IronBoard boardroom docs matrix; operator-readable at `/dashboard/operations/library/control-to-capital` and in-app slug `control-to-capital`.
* **Operational Purpose:** Canonical buyer-safe GTM claim sheet governing outbound drafts, call assist, Heatmap Amnesty copy, and IronBoard advice. Defines the **Control-to-Capital GRC** wedge: Controls → Evidence → Risks → Business scenarios → Estimated financial exposure (ranges) → Mitigation cost → Residual exposure — running at entity → business unit → subsidiary → portfolio → fund/holding/MSSP command with hard isolation and authorized roll-up.
* **Technical Mechanics:**
  * Source doc: `docs/sales/control-to-capital-market-narrative.md` — status ACTIVE canonical (2026-07-30)
  * IronBoard ingress: `Ironboard/src/index.ts` `readDoc(path.join(docsRoot, 'sales', 'control-to-capital-market-narrative.md'))` appended to federation matrix after message constitution block
  * Docker: `Ironboard/Dockerfile` copies full `docs/` tree; `.dockerignore` permits `docs/**/*.md` through federation root
  * SaaS KB alignment: `lib/ironframeProductKnowledge/saasCallKnowledgeBase.ts` `ale-reporting` id matches board-math doctrine — exposure ranges, SEC materiality nuance, heatmaps as non-decision-layer context
  * Claim ban table: forbidden phrases include “most urgent unmet need,” “true ALE,” “competitors cannot,” unverified CAGR bands, universal single-entity competitor claims
  * Campaign order lock: Heatmap Amnesty first; PE operating-partner deferred until §A buyers live and counsel D0 clears first paid signature
* **Agent Boundary:** **Ironlogic** (Agent 4) board synthesis must cite federated claim sheet; **gf-product-boundary** desk pass blocks sales language violating claim bans on quarantined manuscripts; narrative agents must not invent dollar exposure.
* **Financial note:** Talk-track cites flat **499900** cents Path B on-ramp; prefer **estimated annual loss exposure** / **P50–P90 ranges** in prose — constitutional tenant ALE baselines (Medshield **1110000000**, Vaultbank **590000000**, Gridcore **470000000**, Defense **1600000000** cents) remain Irontrust verification anchors only, never customer logos.
* **Step-by-Step Lab Validation:**
  1. Start IronBoard :8082 with docs tree present — verify startup log reports loaded markdown count includes control-to-capital file.
  2. Ask boardroom “what is our heatmap vs dollars positioning?” — verify answer aligns with claim sheet, not banned superlatives.
  3. Open operator library slug **control-to-capital** — verify full markdown renders.
  4. Run `npm run knowledge:check` — fingerprint SHA matches `.fingerprint.json` after commercial spine edits.

---

<a id="marketing-006"></a>

### 🏠 Feature 111: Marketing Homepage Control-to-Capital Copy Realignment
* **GRC Function ID:** `MARKETING-006`
* **Exact Screen Coordinates:** `/` (guest), `/marketing` — `MarketingHomepage.tsx` hero, committee defensibles grid, board-problem section, sticky subnav.
* **Operational Purpose:** Aligns public Command overview copy with Control-to-Capital doctrine — estimated exposure becomes the decision layer beside connected evidence and zero-trust isolation; heatmaps explicitly permitted as context rather than positioned as the sole artifact to eliminate.
* **Technical Mechanics:**
  * `COMMITTEE_DEFENSIBLES[0]` title **Whole-cent financial exposure** — desc: documented estimated dollar-loss exposure in whole cents; heatmaps can stay as context; they should not be the only board decision layer
  * Hero support copy: **Put estimated exposure, connected evidence, and board-ready output under zero-trust isolation — so color charts are context, not the capital conversation**
  * Board problem h3: **Spreadsheets and color charts alone fail the capital conversation**
  * Board problem body: estimated dollar-loss exposure — whole cents, assumptions visible — as decision layer beside connected evidence and isolation; heatmaps can remain context
  * Subnav: new `Link` to `/marketing/heatmap-amnesty` labeled **Heatmap Amnesty** between Proof and product-demo links
* **Agent Boundary:** **Ironcore** public funnel; no tenant telemetry on guest render path.
* **Financial note:** Path B and planned GA prices continue to resolve from `lib/ironframeProductKnowledge/commercial.ts` only — **499900** cents Path B, **3500000** cents/yr planned GA Command, **7500000** cents/yr planned GA Growth.
* **Step-by-Step Lab Validation:**
  1. Open `/marketing` as guest — verify **Heatmap Amnesty** appears in sticky subnav.
  2. Read committee defensibles first tile — confirm heatmap-as-context language, not “replace all heatmaps.”
  3. Compare board-problem paragraph against Mandate 16 — no “true ALE” or FAIR mandate claims.

---

<a id="intel-009"></a>

### 🛰️ Feature 140: August 10 Live Strategic Intel OSINT Manifest Refresh (Active Binding — Operational Follow-Through 2026-08-11)
* **GRC Function ID:** `INTEL-009`
* **Exact Screen Coordinates:** IronBoard Strategic Intel dashboard — rows in `ironboard_crm_interactions` with manifest `ironintel-osint-2026-08-10-live`.
* **Operational Purpose:** Delivers fresh external OSINT authored **August 10, 2026** through Irongate-sanitized CRM persistence. Manifest authorship converged on **dual BOD 26-04 KEV remediation deadline day August 10** — **Progress LoadMaster CVE-2026-8037** (CVSS **9.6**; patches GA **v7.2.63.2** and LTSF **v7.2.54.18**) and **Fortinet FortiOS CVE-2025-68686** (symbolic link persistency bypass). On operational date **2026-08-11**, both enter **post-deadline forensic triage day one**. Active containment: **ChainDrop npm worm day-seven**, **FBI and CISA August 9 PLC alert renewal** per **CISA AA26-097A**, **BOD 26-04 post-deadline forensic triage day five** (Langflow, Tomcat, N-able — deadlines passed **August 7**), **TeamCity post-deadline triage day three** (deadline passed **August 8**), **UTS** healthcare breach **3803750** individuals, **PleaseFix Intent Collision** carryover, CMMC day **29** (**3** days to RFI close **August 14**), Minnesota PLC day **fifteen**, Cl0p Windchill PLM extortion carryover.
* **Technical Mechanics:** `Ironboard/src/knowledge/grcProfessionalResearch.manifest.json`:
  * `manifestId`: `ironintel-osint-2026-08-10-live`
  * `generatedAt`: `2026-08-10T08:00:00.000Z`
  * **Post-deadline triage day one (operational 2026-08-11):** Progress LoadMaster CVE-2026-8037 and Fortinet FortiOS CVE-2025-68686 — federal remediation deadlines **passed August 10**; Healthcare, Public Sector, and Manufacturing Industry Profiles must produce BOD 26-04 forensic triage attestation and compromise assessment before Ironwatch closes tickets
  * **Active threat vector (tier-zero day-seven on operational 2026-08-11):** ChainDrop npm worm (Shai-Hulud CHAINDROP variant) — manifest chunk authorship day-six; Unit 42, Microsoft, StepSecurity, and Elastic Security Labs confirm **400**-plus unique npm packages; **keyv** exceeds **600**M monthly downloads; combined ecosystem exposure exceeds **1.3** billion monthly downloads; Bun preinstall dropper, EtherHiding Ethereum dead-drop C2, and Claude Code or VS Code persistence hooks remain active; CERT-SE advises treating compromised CI/CD runners as fully breached — secret rotation and lockfile audit mandatory
  * **Critical infrastructure alert renewal:** FBI and CISA August 9, 2026 renewed urgent warning per **CISA AA26-097A** — Iranian-affiliated APT actors targeting Rockwell Automation, Allen-Bradley, Schneider Electric, and Siemens PLCs via ports **44818**, **2222**, **102**, **502**, and **22**; malicious project file interactions and HMI/SCADA display manipulation causing operational disruption; Manufacturing and Public Sector Industry Profiles must remove OT devices from direct internet exposure and audit reusable logic modules for supply-chain tampering
  * **Post-deadline triage day five (operational 2026-08-11):** IBM Langflow CVE-2026-9198, Apache Tomcat CVE-2026-34486, and N-able N-central CVE-2026-18556 federal remediation deadlines passed **August 7** — forensic triage and compromise assessment attestation required before Ironwatch closes tickets; agency BOD 26-04 vulnerability-management policy update deadline also passed **August 7** — Public Sector Industry Profile enters policy attestation verification window
  * **Post-deadline triage day three (operational 2026-08-11):** JetBrains TeamCity CVE-2026-63077 (CVSS **9.8**) — federal remediation deadline passed **August 8**; Technology, Defense, and Manufacturing Industry Profiles must produce BOD 26-04 forensic triage attestation, hunt stored credentials on compromised CI/CD servers, and verify build artifact integrity
  * **Healthcare sector incident carryover:** UTS Unlimited Technology Systems Ohio medical software provider — breach affecting **3803750** individuals per HHS breach portal; largest healthcare breach reported to regulators in 2026; Healthcare Industry Profile must expand third-party medical software vendor continuous audit scope and correlate LoadMaster perimeter post-deadline triage
  * **Emerging agentic browser threat carryover:** PleaseFix Intent Collision — Zenity Labs Black Hat USA 2026 zero-click hijack of Claude in Chrome, Gemini in Chrome, Perplexity Comet, ChatGPT Atlas, and Copilot Edge via hidden instructions in emails, calendar invites, and web pages; Technology and Finance Industry Profiles must restrict agent permissions
  * **OT carryover:** Minnesota water PLC coordinated campaign day **fifteen** — CISA AA26-097A nationwide alert on internet-exposed PLCs across Rockwell Allen-Bradley, Schneider Modicon M340, and Siemens S7-1200 estates
  * **Regulatory horizon:** CMMC Phase II suspension day **29** of 60-day Reform Task Force review — report due **September 11**; RFI window closes **August 14** (**3** days remaining on operational date **2026-08-11**); Phase I self-assessments and NIST SP 800-171 Rev 2 remain active per DFARS 252.204-7012; Coalfire DIB Threat Report confirms only **2** percent Level 2 certification rate among estimated **80000** DIB organizations
  * **PLM extortion carryover:** Cl0p Windchill/FlexPLM CVE-2026-12569 hex-named JSP webshell deployment on internet-exposed engineering IP repositories
  * Updated `riskMetricsCents` (workday analysis): `medianAnnualGrcProgramCents` **5360000000**, `medianAuditRemediationLagCents` **1346000000**, `saasConsolidationSavingsOpportunityCents` **1000000000**, `boardReportingOverheadCents` **178000000**
  * Updated `riskMetricsCents` (SaaS disruption memorandum): **5004000000**, **1420000000**, **1578000000**, **145800000**
  * Industry profile narratives updated — Finance `saasDisruptionExposureIndex` **98**; Healthcare **92**; Technology `regulatoryPressureIndex` **99**, `saasDisruptionExposureIndex` **99**; Defense **78**; Public Sector **82**; Manufacturing `regulatoryPressureIndex` **98**, `saasDisruptionExposureIndex` **94** (workday) / **99** (SaaS doc)
  * RAG chunks: `osint-01-dual-kev-deadline-day` through `osint-08-uts-healthcare-breach-carryover`; `saas-01-dmz-ingress` binds manifest `ironintel-osint-2026-08-10-live`
  * Ingestion: `npx tsx scripts/ingest-strategic-intel-manifest.ts` after Irongate `validateStrategicIntelManifest`
* **Constitutional tenant ALE baselines (Ironframe seed tenants — unchanged):** Medshield **1110000000**, Vaultbank **590000000**, Gridcore **470000000**, Defense **1600000000** cents.
* **Agent Boundary:** **Ironintel** (Agent 16) OSINT correlation; **Ironwatch** (Agent 13) KEV deadline receipts and BOD 26-04 post-deadline attestation; **Irongate** (Agent 14) DMZ sanitization before CRM persistence; **Ironlogic** (Agent 4) board briefing synthesis must cite BIGINT-cent industry narratives without float drift.
* **Step-by-Step Lab Validation:**
  1. Run ingest script — verify manifest schema validation passes for `ironintel-osint-2026-08-10-live`.
  2. Query Strategic Intel dashboard — confirm LoadMaster/FortiOS post-deadline triage day one, ChainDrop day-seven, PLC AA26-097A alert renewal, TeamCity post-deadline triage day three, post-deadline triage day five, UTS breach, PleaseFix carryover, and CMMC day **29** entries visible under tenant scope.
  3. Ask board-CISO persona about perimeter appliance remediation — verify response references LoadMaster/FortiOS post-deadline attestation and ChainDrop secret rotation before deferring to TeamCity post-deadline attestation.
  4. Verify August 10 `riskMetricsCents` display as string-encoded BigInt — never float conversion on persist path.
...
### 🛰️ Feature 139: August 8 Live Strategic Intel OSINT Manifest Refresh (Prior Cycle)
* **GRC Function ID:** `INTEL-008`
* **Exact Screen Coordinates:** Historical CRM rows bound to `ironintel-osint-2026-08-08-live` — superseded by Feature 140 August 10 manifest.
* **Operational Purpose:** Prior-cycle August 8, 2026 OSINT — TeamCity CVE-2026-63077 federal remediation deadline day, ChainDrop npm worm day-four containment, PleaseFix Intent Collision zero-click agentic browser, UTS **3803750**-individual healthcare breach, LoadMaster CVE-2026-8037 KEV due **August 10**, BOD 26-04 post-deadline triage day two, Windows Hello Entra ID persistence carryover, CMMC day **26**, Fortinet **August 10** approaching deadline, Minnesota PLC day **twelve**, Cl0p Windchill PLM extortion. Retained for audit trail comparison only.
* **Technical Mechanics:** `Ironboard/src/knowledge/grcProfessionalResearch.manifest.json`:
  * `manifestId`: `ironintel-osint-2026-08-08-live`
  * `generatedAt`: `2026-08-08T08:00:00.000Z`
  * Updated `riskMetricsCents` (workday analysis): **5315000000**, **1332000000**, **988000000**, **175800000** cents
  * Updated `riskMetricsCents` (SaaS disruption memorandum): **4958000000**, **1406000000**, **1562000000**, **143400000** cents
  * RAG chunks: `osint-01-teamcity-deadline-day` through `osint-08-windows-hello-entra-carryover`; `saas-01-dmz-ingress` binds manifest `ironintel-osint-2026-08-08-live`
* **Agent Boundary:** Superseded by **Ironintel** August 10 manifest (`ironintel-osint-2026-08-10-live`) — re-ingest required after deploy.

---

<a id="intel-007"></a>

### 🛰️ Feature 137: August 7 Live Strategic Intel OSINT Manifest Refresh (Prior Cycle)
* **GRC Function ID:** `INTEL-007`
* **Exact Screen Coordinates:** Historical CRM rows bound to `ironintel-osint-2026-08-07-live` — superseded by Feature 140 August 10 manifest.
* **Operational Purpose:** Prior-cycle August 7, 2026 OSINT — BOD 26-04 governance and KEV remediation deadline convergence day: agency vulnerability-management policy update deadline **August 7**, federal remediation deadline **August 7** for IBM Langflow CVE-2026-9198, Apache Tomcat CVE-2026-34486, and N-able N-central CVE-2026-18556, **ChainDrop npm worm day-three containment**, **JetBrains TeamCity CVE-2026-63077** active CI/CD RCE with KEV due **August 8**, **Zenity** zero-click AI browser indirect prompt injection, **Windows Hello for Business** Entra ID persistence disclosure, CMMC reform RFI countdown day **25**, CHAOS healthcare extortion carryover, Minnesota PLC day **eleven**, Fortinet approaching deadline, and post-deadline WordPress attestation. Retained for audit trail comparison only.
* **Technical Mechanics:** `Ironboard/src/knowledge/grcProfessionalResearch.manifest.json`:
  * `manifestId`: `ironintel-osint-2026-08-07-live`
  * `generatedAt`: `2026-08-07T08:00:00.000Z`
  * Updated `riskMetricsCents` (workday analysis): **5270000000**, **1318000000**, **976000000**, **173600000** cents
  * Updated `riskMetricsCents` (SaaS disruption memorandum): **4912000000**, **1391000000**, **1548000000**, **141200000** cents
  * RAG chunks: `osint-01-chaindrop-day-three` through `osint-08-post-deadline-carryover`; `saas-01-dmz-ingress` binds manifest `ironintel-osint-2026-08-07-live`
* **Agent Boundary:** Superseded by **Ironintel** August 10 manifest (`ironintel-osint-2026-08-10-live`) — re-ingest required after deploy.
...
### 🛰️ Feature 135: August 6 Live Strategic Intel OSINT Manifest Refresh (Prior Cycle)
* **GRC Function ID:** `INTEL-006`
* **Exact Screen Coordinates:** Historical CRM rows bound to `ironintel-osint-2026-08-06-live` — superseded by Feature 140 August 10 manifest.
* **Operational Purpose:** Prior-cycle August 6, 2026 OSINT — ChainDrop npm worm day-two containment triage converging on KEV deadline cluster: N-able N-central CVE-2026-18577 federal remediation deadline day, JetBrains TeamCity CVE-2026-63077 active CI/CD RCE exploitation, IBM Langflow CVE-2026-9198 and Apache Tomcat CVE-2026-34486 deadline eve (**August 7**), BOD 26-04 agency policy update deadline (**August 7**), CMMC reform RFI countdown, CHAOS healthcare extortion carryover, Brown Health **311000**-individual breach reporting, CaptiveCrunch identity theft, Fortinet approaching deadline, Minnesota PLC day **ten**, and post-deadline WordPress attestation. Retained for audit trail comparison only.
* **Technical Mechanics:** `riskMetricsCents` (workday): **5228000000**, **1306000000**, **972000000**, **172400000** cents; SaaS disruption: **4865000000**, **1378000000**, **1536000000**, **139800000** cents. RAG chunks: `osint-01-chaindrop-day-two` through `osint-08-post-deadline-carryover`.
* **Agent Boundary:** Superseded by **Ironintel** August 10 manifest — re-ingest required after deploy.

---

<a id="intel-005"></a>

### 🛰️ Feature 134: August 5 Live Strategic Intel OSINT Manifest Refresh (Prior Cycle)
* **GRC Function ID:** `INTEL-005`
* **Exact Screen Coordinates:** Historical CRM rows bound to `ironintel-osint-2026-08-05-live` — superseded by Feature 140 August 10 manifest.
* **Operational Purpose:** Prior-cycle August 5, 2026 OSINT convergence — ChainDrop npm worm day-one cascade, WordPress CVE-2026-60137 post-deadline attestation, CHAOS healthcare extortion, CMMC day **23**, BOD 26-04 policy deadline August 7, N-able RMM carryover, Fortinet **August 10** approaching deadline, Minnesota PLC day **nine**, post-deadline KEV cluster. Retained for audit trail comparison only.
* **Technical Mechanics:** `riskMetricsCents` (workday): **5186000000**, **1294000000**, **968000000**, **171200000** cents; SaaS disruption: **4823000000**, **1367000000**, **1528000000**, **138900000** cents. RAG chunks: `osint-01-chaindrop-npm-worm` through `osint-08-post-deadline-carryover`.
* **Agent Boundary:** Superseded by **Ironintel** August 10 manifest — re-ingest required after deploy.
...
### 🛰️ Feature 112: August 3 Live Strategic Intel OSINT Manifest Refresh (Prior Cycle)
* **GRC Function ID:** `INTEL-004`
* **Exact Screen Coordinates:** Historical CRM rows bound to `ironintel-osint-2026-08-03-live` — superseded by Feature 140 August 10 manifest.
* **Operational Purpose:** Prior-cycle August 3, 2026 OSINT convergence — N-able N-central CVE-2026-18577 MSP RMM god-mode bypass, WordPress August 4 BOD 26-04 deadline day, CaptiveCrunch traveler identity theft, ShinyHunters healthcare SSO extortion, Arch AUR supply-chain carryover, CMMC day **21**, Fortinet **August 10** approaching deadline, Minnesota PLC day **seven**, post-deadline KEV cluster. Retained for audit trail comparison only.
* **Technical Mechanics:** `riskMetricsCents` (workday): **5042000000**, **1248000000**, **932000000**, **165700000** cents; SaaS disruption: **4675000000**, **1319000000**, **1482000000**, **132400000** cents. RAG chunks: `osint-01-n-able-rmm-god-mode` through `osint-08-post-deadline-carryover`.
* **Agent Boundary:** Superseded by **Ironintel** August 10 manifest — re-ingest required after deploy.
...
### 🛰️ Feature 128: August 2 Live Strategic Intel OSINT Manifest Refresh (Prior Cycle)
* **GRC Function ID:** `INTEL-004-AUG2`
* **Exact Screen Coordinates:** Historical CRM rows bound to `ironintel-osint-2026-08-02-live` — superseded by Feature 140 August 10 manifest.
* **Operational Purpose:** Prior-cycle August 2, 2026 OSINT convergence — CaptiveCrunch identity-first threat convergence, Arch AUR developer supply-chain, ShinyHunters healthcare SSO extortion, WEF **75** percent credential-breach synthesis, BOD 26-04 post-deadline verification carryover, WordPress **August 4** and Fortinet **August 10** approaching deadlines, Minnesota PLC day **six**, CMMC day **20**. Retained for audit trail comparison only — superseded by Feature 134 August 5 manifest.
* **Technical Mechanics:** `riskMetricsCents` (workday): **4985000000**, **1215000000**, **918000000**, **162300000** cents; SaaS disruption: **4612000000**, **1286000000**, **1458000000**, **129100000** cents.
* **Agent Boundary:** Superseded by **Ironintel** August 10 manifest — re-ingest required after deploy.
...
* **Exact Screen Coordinates:** Historical CRM rows bound to `ironintel-osint-2026-08-01-live` — superseded by Feature 112 August 3 manifest.
...
* **Agent Boundary:** Superseded by **Ironintel** August 3 manifest — re-ingest required after deploy.

---

<a id="hitl-003"></a>

### 🚦 Feature 113: Sales DISPATCH Perimeter Validation Gates
* **GRC Function ID:** `HITL-003`
* **Exact Screen Coordinates:** `/dashboard/admin/approvals` — SALES draft dispatch panel; server gate on `POST /api/admin/approvals/[id]` with `action: DISPATCH`.
* **Operational Purpose:** Prevents operator error on live partner outreach — blocks harvest placeholder emails, shortlist HOLD companies, unacknowledged dry-run inboxes, and Sales EMAIL bodies that violate C1 commercial locks before Resend or Textbelt sends fire.
* **Technical Mechanics:**
  * `app/lib/approvalDispatchValidation.ts` — `validateApprovalDispatch()` extended input: `company`, `acknowledgeOperatorSelfDispatch`
  * `SALES_DISPATCH_HOLD_COMPANIES`: readonly `["blueradius"]` — company name contains match (case-insensitive)
  * `operatorDryRunEmails()`: `dwoods360@gmail.com`, `dereck@ironframegrc.com`, plus `OPERATOR_DRY_RUN_EMAILS` env comma list
  * `isIronleadsLocalEmail()`: suffix `@ironleads.local` — EMAIL blocked; `preferredSalesDispatchChannel()` returns SMS when phone valid
  * Sales EMAIL C1 locks: requires `/4,?999|\$4k|command\s+design\s+partner/i` and `/workflow\s*review/i`; rejects `/free\s*(trial|poc|pilot)|proof\s*of\s*concept/i`
  * `AdminApprovalDashboardClient.tsx`: rose HOLD banner for hold companies; amber `@ironleads.local` notice; cyan dry-run acknowledge checkbox; `useEffect` auto channel swap preserves email body in `emailBodyByDraftId`
  * `app/api/admin/approvals/[id]/route.ts`: passes `contact.company` and `acknowledgeOperatorSelfDispatch` from request body
* **Agent Boundary:** **Ironwatch** (Agent 13) audit on successful DISPATCH; gate runs server-side — client validation is advisory mirror only.
* **Financial note:** C1 lock enforces Path B **499900** cents (**$4,999** display) mention on Sales EMAIL — aligns with `DESIGN_PARTNER_PATH_B_CENTS` spine truth.
* **Step-by-Step Lab Validation:**
  1. Queue SALES draft with company **BlueRadius Cyber** — verify DISPATCH blocked with HOLD error.
  2. Set EMAIL to `suspect@ironleads.local` with valid phone — verify auto SMS channel or EMAIL validation error.
  3. Set EMAIL to operator dry-run inbox without checkbox — verify blocked; check acknowledge — verify smoke DISPATCH allowed.
  4. Remove $4,999 from EMAIL body — verify C1 lock error.
  5. Run `tests/unit/approvalDispatchValidation.test.ts` — full matrix pass.

---

<a id="gtm-kb-001"></a>

### 📦 Feature 114: SaaS Call Knowledge Base — Command Packaging Expansion
* **GRC Function ID:** `GTM-KB-001`
* **Exact Screen Coordinates:** No dedicated UI — consumed by workflow-review Live Q&A, talk-track panel, and `lookupSaasCallKnowledge()` across ops surfaces.
* **Operational Purpose:** Curates spoken, short answers for recurring prospect diligence on entity caps, Paid Enclave stacking math, Command Multi/Enterprise flat books, Partner MSSP tiers, and fair-use WORM/ingest limits — grounded exclusively in `commercial.ts` and `productFacts.ts` with no invented soft max-client quotas or free pilots.
* **Technical Mechanics:**
  * `lib/ironframeProductKnowledge/saasCallKnowledgeBase.ts` — `SAAS_CALL_KNOWLEDGE_BASE` readonly array with **20+** regex-ordered entries
  * Helper exports: `saasEntityStackingCostAnswer()`, `saasCorePaidToMultiStackAnswer()`, `saasCommandMultiEligibilityAnswer()`, `saasCapacityClientsTenantsAnswer()`, `saasCommandCorePackagingAnswer()`, `saasPaidEnclaveAnswer()`, `saasEntityRangePricingAnswer()`, `saasCommandMultiEnterpriseAnswer()`, `saasPartnerBookAnswer()`, `saasFairUseWormIngestAnswer()`, `saasPocketTopicCatalog()`
  * Key packaging locks: Path B **1** Primary + **2** Subtenants (**499900** cents flat); Core **4** entities (**3500000** cents/yr planned GA) — fourth entity inside Core not Core + Paid; Paid Enclave **350000** cents/yr list per enclave beyond Core's **3** included (entity **5**+); **6** Paid enclaves fill Multi from Core (`PAID_ENCLAVES_TO_FILL_MULTI`); Command Multi **5500000** cents/yr flat up to **10** entities; Command Enterprise **9500000** cents/yr up to **25** entities
  * `collectSaasCallKnowledgeHits()`, `listSaasCallKnowledgeTopics()` for grounded LLM priority cards
  * `commercial.ts` additions: `COMMAND_MULTI_SUBTENANTS` (**9**), `PAID_ENCLAVES_TO_FILL_MULTI` (**6**)
  * Unit tests: `tests/unit/saasCallKnowledgeBase.test.ts` — pins stack math to spine constants; `assistWorkflowReviewQuestion()` returns verbatim KB answers for capacity questions
* **Agent Boundary:** **Ironlogic** (Agent 4) and workflow-review pocket assist READ this KB — never invent pricing outside spine; **Irontrust** (Agent 3) Path B **499900** cents anchor unchanged.
* **Step-by-Step Lab Validation:**
  1. `lookupSaasCallKnowledge("4,999 + 3,500 for four tenants")` — verify `entity-stacking-cost` id refutes stack.
  2. `lookupSaasCallKnowledge("after the first 3 subtenants $3,500 each")` — verify `core-paid-to-multi-stack` with **6** Paid count.
  3. Run full `tests/unit/saasCallKnowledgeBase.test.ts` — all packaging assertions pass against `commercial.ts` constants.

---

<a id="gtm-live-002"></a>

### 🤖 Feature 115: Workflow-Review Grounded Pocket Assist LLM
* **GRC Function ID:** `GTM-LIVE-002`
* **Exact Screen Coordinates:** Server-only module — invoked from `/api/admin/operations-hub/workflow-review-call` `action: assist` and `action: session` via `assistWorkflowReviewQuestionAsync()`.
* **Operational Purpose:** Grounds novel prospect question phrasing in the authoritative product pack when regex SaaS KB and pocket locks miss — while refusing to paraphrase exact commercial cards or invent numbers when Gemini is unavailable.
* **Technical Mechanics:**
  * `app/lib/server/workflowReviewPocketAssistLlm.ts` — server-only; `@ai-sdk/google` + `generateObject` with `groundedAssistSchema` (`grounded`, `determination`, `answer`, `missingFact`)
  * `buildPocketGroundingPack()` — injects `PRODUCT_NAME`, `PRODUCT_POSITIONING`, `PRODUCT_DIFFERENTIATORS`, full Path B / planned GA / Partner book constants from `commercial.ts`, canonical packaging determination paragraphs, `FORBIDDEN_PRODUCT_CLAIMS`, and full `SAAS_CALL_KNOWLEDGE_BASE` card text
  * `groundedPocketAssistFromLlm(question, { priorityCards, priorityNotes })` — temperature **0** via Gemini Flash (`resolveGeminiFlashModel()`); returns `CallAssistAnswer` with banNote guardrails
  * `pocketRefuseAnswer(question)` — explicit refuse when `GOOGLE_API_KEY` / `GOOGLE_GENERATIVE_AI_API_KEY` missing or model returns `grounded: false`
  * `assistWorkflowReviewQuestionAsync()` flow: empty → miss; exact `saas_kb` / `pocket_lock` → return verbatim (never LLM); else grounded LLM; else refuse
  * API keys: `GOOGLE_GENERATIVE_AI_API_KEY` or `GOOGLE_API_KEY`
* **Agent Boundary:** Sidecar to operator-hosted workflow review only — not SalesTeam auto-send; not boardroom synthesis; **Irongate** (Agent 14) principle: refuse rather than invent on packaging asks.
* **Financial note:** Grounding pack locks Path B at **499900** cents, Core at **3500000** cents/yr, Paid Enclave at **350000** cents/yr list, Multi at **5500000** cents/yr — LLM prompt forbids inventing beyond LOCKED FACTS block.
* **Step-by-Step Lab Validation:**
  1. Ask novel phrasing on capacity not matching regex — verify `source: grounded_llm` or `miss` with refuse, never float prices.
  2. Ask exact Path B price question — verify `source: pocket_lock` or `saas_kb` with no LLM rewrite.
  3. Unset `GOOGLE_API_KEY` — verify `pocketRefuseAnswer` path, not hallucinated packaging.
  4. Run `tests/unit/workflowReviewPocketAssistLlm.test.ts` — grounding pack contains spine constants.
...
### 🧭 Feature 108: Ironleads Buying-Committee Research, Leadership OSINT Search & Apollo/Prospeo HITL Enrichment
* **GRC Function ID:** `GTM-BC-001`
* **Exact Screen Coordinates:** Ironleads SUSPECT enrichment path — harvest auto-chains `ironleadsBuyingCommitteeResearchCore.ts`; portal **Research** button with batch auto-continue; suspect report page **Enrich with Apollo** and **Enrich with Prospeo** actions; results persist on contact metadata for operator scan before PROSPECT drafts.
* **Operational Purpose:** Map likely buying-committee roles (CISO/CFO/sponsor) and switchboard phones so outreach targets a real buyer — not title noise. When company-site scrape is thin, leadership OSINT search fills gaps via allowlisted press URLs. Apollo and Prospeo HITL enrich add org firmographics and named-buyer work email when placeholders remain — never auto-DISPATCH.
* **Technical Mechanics:**
  * `ironleadsBuyingCommitteeResearchCore.ts` / `ironleadsBuyingCommitteeExtract.ts` — `researchBuyingCommitteeForContact` / `researchBuyingCommitteeForAllSuspects`; active batch sorted **thinnest dossier first**; browser-like User-Agent on site scrape; live About/team parse wins over playbook when names present; `buildEmailPermutationCandidates()` with first.last → f.last → first@ order; MX PASS does not prove ownership on catch-all domains
  * `googleLeadershipSearchClient.ts` — provider cascade Brave → SerpAPI → Google CSE legacy; `searchCompanyLeadership()` only when zero plausible names from scrape; persists `googleLeadershipSearch` metadata on buying committee
  * `ironleadsLeadershipSearchAllowlist.ts`, `ironleadsLeadershipSearchHitRefine.ts` — press/cyber URL filtering and name extraction
  * `domainMailFootprint.ts` — MX and SMTP footprint probes for buyer email verification in research brief
  * Portal Research button — auto-continues up to **8** rounds (`researchBatchLimit: 5`, `forceResearch: true` on first click) under Vercel **120**s limit
  * `apolloEnrichmentClient.ts` / `ironleadsApolloEnrichCore.ts` — org-by-domain + named-buyer people match via `APOLLO_API_KEY`
  * `prospeoEnrichmentClient.ts` / `ironleadsProspeoEnrichCore.ts` — named-buyer + domain work email via `PROSPEO_API_KEY`; NO_MATCH copy guides LinkedIn URL or manual paste
  * `SuspectOperatorEditPanel.tsx` — `enrichResultMessage()` warns when email remains `@ironleads.local` after enrich
  * `.env.example` — `BRAVE_SEARCH_API_KEY`, `SERPAPI_API_KEY`, `GOOGLE_CSE_API_KEY`, `GOOGLE_CSE_CX`, `APOLLO_API_KEY`, `PROSPEO_API_KEY` documented
* **Agent Boundary:** Enrichment only — never DISPATCH; operator still HITL Approvals; **Irongate** (Agent 14) validates ingress before CRM write per Mandates 20, 23, 24.
* **Financial note:** Apollo `estimatedNumEmployees` and Prospeo headcount fields are headcount — not USD cents. Path B **499900** cents cited only in outreach brief `claimsToAvoid` grounding — not in search API payloads.
* **Step-by-Step Lab Validation:**
  1. Run **Research** on Ironleads portal with thin dossiers — verify multi-batch summary message and remaining count hint.
  2. Set `BRAVE_SEARCH_API_KEY` — research contact with empty About page — verify `googleLeadershipSearch` metadata on suspect report.
  3. Set `PROSPEO_API_KEY` — **Enrich with Prospeo** — verify placeholder warning when email stays `@ironleads.local`.
  4. Run `tests/unit/googleLeadershipSearchClient.test.ts`, `ironleadsResearchBatchSelect.test.ts`, `domainMailFootprint.test.ts`, `ironleadsLeadershipSearchHitRefine.test.ts` — all pass.

---

<a id="ops-pub-001"></a>

### 📰 Feature 116: Ops Publishing Desk Split (Briefings, Newsletters, Research Papers & Video)
* **GRC Function ID:** `OPS-PUB-001`
* **Exact Screen Coordinates:** `/dashboard/operations/publishing?desk=briefings|newsletters|research|video`; page shell `app/(dashboard)/dashboard/operations/publishing/page.tsx`.
* **Operational Purpose:** Separates Governance Frame quarantine publication workflow from Ops Today daily operator inbox — four mutually exclusive draft lists with deep-linkable `?draft=` URLs, inline draft reader modal, and video campaign narrative index without auto-publish.
* **Technical Mechanics:**
  * `publishingDeskDraftKind.ts` — `PublishingDeskTab` type; `isResearchDeskDraft()` matches `*-draft-research-*`; `isNewslettersDeskDraft()` for Ironcast and market-grc patterns; `publishingDeskTabForQueueDraft()` auto-routes deep links; `PUBLISHING_VIDEO_NARRATIVE_LINKS` Hub & budget group includes **V1 kickoff — Phase 0 + shot list** (`v1-kickoff-phase0-shotlist`) and **V1 production script — shots + AI prompts** (`v1-production-script`) alongside campaign hub and budget-and-production links; `PUBLISHING_VIDEOS_PAGE_HREF` for When Risk Enters the Room campaign
  * `PublishingDeskClient.tsx` — four tab chrome; draft reader modal loads `GET /api/admin/operations-hub/briefings/draft?filename=`; Escape dismisses; auto-opens reader on `?draft=` focus; promote panel scrolls for briefings and research desks; video desk skips queue snapshot load
  * `publishedLedgerKind.ts` — `classifyPublishedLedgerItem()` splits briefing index from newsletters and industry research on public research site
  * `OperationsHubClient.tsx` — links to Research papers desk and Video desk in publishing quick links
  * Legacy `/dashboard/operations?tab=briefings|newsletters` redirects preserving draft param
* **Agent Boundary:** Unchanged quarantine gates per Mandates 9, 12, 14, 25 — desk agents never promote; human operator only.
* **Step-by-Step Lab Validation:**
  1. Navigate `/dashboard/operations/publishing?desk=research` — verify only `*-draft-research-*` filenames appear.
  2. Open queue draft — verify reader modal auto-opens with markdown body and validation flag.
  3. Navigate `?desk=video` — verify campaign hub links render without quarantine promote panel.
  4. Run `tests/unit/publishingDeskDraftKind.test.ts` and `publishedLedgerKind.test.ts` — classification matrix pass.
...
* **Technical Mechanics:**
  * `approvalNeedsEnrichmentCore.ts` — `processNeedsEnrichmentAction()` returns `SUCCESS_NEEDS_ENRICHMENT`; `needsEnrichmentPlaceholderEmail()` assigns `needs-enrichment+{shortId}@ironleads.local`; `buildNeedsEnrichmentSummary()` prepends `[NEEDS_ENRICHMENT]` tag; **2026-09-20:** demotion transaction uses **`withProspectPoolTenant()`** per Mandate 30
...
<a id="leads-dossier-001"></a>

### 📂 Feature 129: Ironleads Suspect Dossier Mode & Queue Decision Banners
* **GRC Function ID:** `LEADS-DOSSIER-001`
* **Exact Screen Coordinates:** `/dashboard/operations/ironleads/suspects/[contactId]?dossier=1`; Ironleads portal queue decision banners after promote/hold/discard/restore.
* **Operational Purpose:** Keeps promoted PROSPECT contacts out of stale SUSPECT intake URLs while preserving operator access to full intake dossier for SalesTeam handoff review. Queue decisions return to Ironleads portal with contextual next-step banners instead of silent navigation.
* **Technical Mechanics:**
  * `[contactId]/page.tsx` — redirects PROSPECT-stage contacts to `/dashboard/operations/ironleads?promoted={contactId}&company={company}` unless `?dossier=1`; PROSPECT header shows SalesTeam → Approvals next-step banner
  * `SuspectOperatorEditPanel.tsx` — `returnToQueue()` sets query params `promoted|held|restored|discarded` + `company` on full page navigation after queue decisions
  * `IronleadsPortalClient.tsx` — `QueueDecision` type; emerald/amber/cyan/rose banners with links to SalesTeam and dossier; active row links default to `?dossier=1`
  * `ironleads/page.tsx` — parses queue decision query params into portal props
  * OSINT title noise discard redirects with encoded `contactId` and `company` params
* **Agent Boundary:** CRM stage transition only — no auto-DISPATCH; **Ironwatch** (Agent 13) logs promote/hold/discard actions.
* **Step-by-Step Lab Validation:**
  1. Promote SUSPECT to PROSPECT — verify portal shows emerald banner with SalesTeam link.
  2. Navigate suspect URL without `?dossier=1` on PROSPECT — verify redirect to portal with promoted banner.
  3. Open `?dossier=1` on PROSPECT — verify full intake dossier renders with PROSPECT header.

---

<a id="gf-ledger-001"></a>

### 📚 Feature 130: Published Ledger Kind Classification & GF Research Site Index Split
* **GRC Function ID:** `GF-LEDGER-001`
* **Exact Screen Coordinates:** `https://research.ironframegrc.com/briefings`, `/gf-research/briefings`, `/gf-research/newsletters`, `/gf-research/research-papers`; briefing detail pages with archive sidebar.
* **Operational Purpose:** Prevents newsletter and industry-research slugs from polluting the Briefings index while sharing canonical `/briefings/[slug]` URL paths on the research publication origin.
* **Technical Mechanics:**
  * `publishedLedgerKind.ts` — `classifyPublishedLedgerItem()` returns `briefing | newsletter | industry_research` from frontmatter `category`, slug patterns, and title heuristics
  * `publishedBriefingLedgerCards.ts` — uses `isBriefingIndexItem()` and `isNewsletterLedgerItem()` for index filtering
  * `briefingArchiveDirectory.ts` + `BriefingArchiveDirectory.tsx` — vertical timeline archive on briefing detail pages excluding active slug from main column
  * `app/gf-research/briefings/page.tsx` — briefings index only; `app/gf-research/newsletters/page.tsx` and `research-papers/page.tsx` — separate indexes
  * `ResearchSiteChrome.tsx` — nav links to newsletters and research-papers indexes
  * `public/rss.xml` — syndication entries for August 4 promoted briefings
* **Agent Boundary:** READ federation for agents; **Ironscribe** (Agent 5) frontmatter category precision required at promote time per Mandate 26.
* **Step-by-Step Lab Validation:**
  1. Promote industry-research draft with `category: Industry Research` — verify slug appears on `/gf-research/research-papers` but not Briefings index.
  2. Open promoted briefing detail — verify archive sidebar lists prior briefings with synopsis.
  3. Run `tests/unit/publishedLedgerKind.test.ts` and `briefingArchiveDirectory.test.ts` — classification pass.

---

<a id="ops-pub-002"></a>

### 📖 Feature 131: Publishing Desk Inline Draft Reader Modal
* **GRC Function ID:** `OPS-PUB-002`
* **Exact Screen Coordinates:** `/dashboard/operations/publishing?desk=briefings|newsletters|research&draft={filename}` — overlay reader panel.
* **Operational Purpose:** Lets operators read full quarantined markdown without leaving the publishing desk — reducing mis-clicks on promote/deny before reviewing body content.
* **Technical Mechanics:**
  * `PublishingDeskClient.tsx` — `openDraftReader()` fetches `GET /api/admin/operations-hub/briefings/draft?filename=`; displays title, markdown body, `validationOk` flag; Escape key closes; auto-opens on focused `?draft=` deep link once per session
  * Promote/deny success closes reader when filename matches active draft
  * Draft tab auto-correction via `publishingDeskTabForQueueDraft()` when `?draft=` filename belongs to research or newsletter desk
* **Agent Boundary:** Read-only queue inspection — promote/deny APIs unchanged per Mandate 9.
* **Step-by-Step Lab Validation:**
  1. Deep-link `?desk=briefings&draft={filename}` — verify reader auto-opens with markdown body.
  2. Press Escape — verify reader closes without clearing draft selection.
  3. Approve draft from reader context — verify reader closes and snapshot refreshes.

---

<a id="ops-pub-003"></a>

### 📱 Feature 132: Publishing Desk LinkedIn Founder Workbench (Three-Slot Catalog)
* **GRC Function ID:** `OPS-PUB-003`
* **Exact Screen Coordinates:** `/dashboard/operations/publishing?desk=linkedin` — two-column **Drafts awaiting paste** / **Published** chrome plus title, body, and research editors with Save, Copy body, and Reset to template actions.
* **Operational Purpose:** Gives operators a Mandate **16**-safe Mon/Wed/Fri LinkedIn paste workbench with separated operator-only citation research — manual founder publish only, never Governance Frame quarantine or SalesTeam auto-send. Posted slots (Ops calendar **DONE**) move to Published archive for read/reuse only.
* **Technical Mechanics:**
  * `publishingDeskDraftKind.ts` — `PublishingDeskTab` includes `linkedin`; `PUBLISHING_LINKEDIN_DRAFTS_HREF`, `PUBLISHING_LINKEDIN_CADENCE_HREF`
  * `linkedinDeskDraftCore.ts` — `LINKEDIN_DRAFT_CATALOG` with **`mon-heatmap`** (Mon heatmap vs whole-cent exposure), **`wed-product-demo`** (Wed `/product-demo` — body: feature-list vs evidence→exposure workflow; **first-comment CTA** for workflow review per **2026-08-11** template), **`fri-collection`** (Fri collection ≠ verification); `LinkedInDraftId` type; `listLinkedInDeskDraftsCore()` joins `prisma.opsActivity` by `opsSourceRef` for posted status; per-slot APP_DOCS slugs (`marketing-strategy/linkedin-drafts/mon-heatmap`, `wed-product-demo`, `fri-collection`); legacy `LINKEDIN_DRAFTS_APP_DOC_SLUG` mirrors Fri; `LINKEDIN_FRI_DRAFT_*`, `LINKEDIN_WED_DRAFT_*` (includes `LINKEDIN_WED_DRAFT_RESEARCH` first-comment section), `LINKEDIN_SUGGESTED_DRAFT_*` default templates; `loadLinkedInDeskDraftCore({ id, resetTemplate })`, `saveLinkedInDeskDraftCore({ id })`, `ensureCatalogDraft()`; repo sync per `entry.repoFile`
  * `app/api/admin/operations-hub/linkedin/draft/route.ts` — `GET ?id=&reset=1` and legacy `?seed=suggested|friday|monday`; `PUT` persists by catalog `id`
  * `app/api/admin/operations-hub/linkedin/drafts/route.ts` — `GET` returns `activeDrafts`, `postedArchive`, `defaultId`, `counts`
  * `PublishingDeskClient.tsx` — draft list left, published archive right; `?li=` deep link; lands on active draft not Published; Copy/Save/citation checkbox disabled on Published slots; **Reset to template** replaces Load suggested seed
  * `opsScheduleLinks.ts` — calendar refs under `marketing/linkedin-*` route to `?desk=linkedin&li=`
* **Agent Boundary:** **Ironlogic** (Agent 4) claim bans enforced in all slot templates; no agent auto-posts to LinkedIn. **Ironscribe** (Agent 5) does not promote LinkedIn markdown to GF publication plane. **Ironwatch** (Agent 13) tracks Ops calendar **DONE** outcomes for slot archive state.
* **Step-by-Step Lab Validation:**
  1. Open `/dashboard/operations/publishing?desk=linkedin` — verify Mon slot in Published after **2026-08-06** calendar **DONE**; Wed and Fri under Drafts.
  2. Select **`fri-collection`** — verify body loads **Collection is not verification** with research citations panel.
  3. Edit body, Save — verify APP_DOCS slug `marketing-strategy/linkedin-drafts/fri-collection` updates and Fri legacy slug mirrors.
  4. Copy body on active draft — verify clipboard contains post text only, not research block; verify Copy disabled on Published slot.
  5. Confirm post body cites **whole cents** estimated exposure language without Mandate **16** ban phrases.
  6. Select **`wed-product-demo`** — verify body has single `/product-demo` link; verify research block documents first-comment workflow-review CTA at `https://ironframegrc.com/register/contact`.

---

<a id="ops-cal-005"></a>

### 📅 Feature 141: Ops Schedule GTM Gate Reschedule — Cold Outreach & First DISPATCH Slip
* **GRC Function ID:** `OPS-CAL-005`
* **Exact Screen Coordinates:** `/dashboard/operations?tab=calendar` — **Marketing — Cold outreach gate (after free marketing)** and **Rollout — First SalesTeam DISPATCH batch** cards.
* **Operational Purpose:** Defers Path B cold outbound until LinkedIn week-1 posts and Video Phase 0 complete, then sequences First SalesTeam DISPATCH batch after the cold outreach gate — preventing premature cold DISPATCH before free-marketing surfaces and founder cadence posts are live.
* **Technical Mechanics:**
  * `opsScheduleSeedSpecs.ts` — `preOutreachMarketing2026SeedSpecs()` **Marketing — Cold outreach gate (after free marketing)** (`sourceRef: marketing/cold-outreach-gate`, `priorityHint: 11`): `dueAt` moved from **2026-08-10T15:00:00.000Z** to **2026-08-17T15:00:00.000Z**; synopsis `[2026-08-10] Rescheduled to Mon Aug 17 — wait for LinkedIn week-1 posts + Video Phase 0 before cold Path B DISPATCH`
  * `ironframeRollout2026SeedSpecs()` **Rollout — First SalesTeam DISPATCH batch** (`sourceRef: rollout/first-salesteam-dispatch`): `dueAt` moved from **2026-08-17T15:00:00.000Z** to **2026-08-24T15:00:00.000Z**; synopsis `[2026-08-10] Rescheduled to Mon Aug 24 — after cold outreach gate (Aug 17)`
  * Operators re-seed calendar via `POST /api/admin/operations-hub/schedule` `{ action: "seed-all-projects" }` to pick up rescheduled due dates on existing rows
* **Agent Boundary:** **Ironlogic** (Agent 4) GTM sequencing only — no auto-DISPATCH; SalesTeam poll remains HITL per Mandates 10 and 17.
* **Step-by-Step Lab Validation:**
  1. Open Ops Calendar — locate cold outreach gate card — verify due **August 17, 2026**.
  2. Locate First SalesTeam DISPATCH batch — verify due **August 24, 2026**.
  3. Confirm synopsis strings cite LinkedIn week-1 + Video Phase 0 dependency on cold gate card.
  4. Verify no SalesTeam auto-send fires before operator DISPATCH click.

---

<a id="ops-pub-004"></a>

### 📱 Feature 142: LinkedIn Wed Draft First-Comment CTA Refresh
* **GRC Function ID:** `OPS-PUB-004`
* **Exact Screen Coordinates:** `/dashboard/operations/publishing?desk=linkedin&li=wed-product-demo` — title, body, and research editors for Wed slot.
* **Operational Purpose:** Aligns Wed founder post with one-primary-link cadence — demo URL in body, workflow-review CTA in first comment — while sharpening copy to contrast feature-list GRC tours against evidence→scenario→estimated exposure (whole cents) workflow.
* **Technical Mechanics:**
  * `linkedinDeskDraftCore.ts` — `LINKEDIN_WED_DRAFT_TITLE`: **LinkedIn Wed — point to /product-demo (pre-video)**
  * `LINKEDIN_WED_DRAFT_BODY`: opens with feature-list/color-dashboard contrast; **Evidence → Scenario → Estimated Exposure** (whole cents, explicit assumptions); board shift from compliance checkbox to defendable financial exposure; single link `https://ironframegrc.com/product-demo`; hashtags `#GRC #BoardRisk #CyberGovernance #RiskQuantification #RiskManagement` (removed `#ProductDemo` from body)
  * `LINKEDIN_WED_DRAFT_RESEARCH`: **First comment (post immediately after publish — do not put in main body)** section with workflow-review CTA `https://ironframegrc.com/register/contact`; claim map cites Mon heatmap qualitative-theater context and one-primary-link rule; held alternate `docs/marketing-strategy/linkedin-drafts-hold-board-reporting-gap.md`; pre-post checklist includes **Paste first-comment CTA immediately after publish**
  * Reset to template on Wed slot reloads these defaults via `loadLinkedInDeskDraftCore({ id: "wed-product-demo", resetTemplate: true })`
* **Agent Boundary:** **Ironlogic** (Agent 4) Mandate **16** claim bans enforced; **Ironscribe** (Agent 5) does not promote LinkedIn copy to GF quarantine.
* **Step-by-Step Lab Validation:**
  1. Open Wed slot — verify body contains exactly one `ironframegrc.com/product-demo` link.
  2. Verify body does not contain `register/contact` URL.
  3. Open research block — verify first-comment section with workflow-review CTA.
  4. Reset to template — verify body and research revert to code defaults.
  5. After publish — paste first-comment CTA; mark Ops calendar Wed source ref **DONE** with post URL.

---

<a id="ironleads-013"></a>

### 📧 Feature 133: Ironleads Promote-Ready Email Gate & Published-Email Discovery
* **GRC Function ID:** `IRONLEADS-013`
* **Exact Screen Coordinates:** `/dashboard/operations/ironleads/suspects/[contactId]` — Account Research Brief Email gate row; buying-committee member email badges after Research.
* **Operational Purpose:** Prevents Path B Promote on company switchboard inboxes (`info@`, `hello@`) while discovering person-specific mailto and directory-labeled emails before pattern guessing; merges Prospeo/operator upgrades without dropping rich scrape findings. As of **2026-08-10**, employer-domain ownership check removed — Email gate uses work-seat classification only; operators must HITL-verify buyer identity when Prospeo returns a different-domain address.
* **Technical Mechanics:**
  * `ironleadsAccountResearchBrief.ts` — `isPromoteReadyWorkEmail()`, `GENERIC_COMPANY_INBOX_LOCALS`, `selectAccountResearchBriefForReport()`, `pathBHold` forces Email/Buyer FAIL and outreach hold on operator channel-competitor archive; **`emailMatchesAccountDomain()`** and **`isPromoteReadyEmployerEmail()`** removed; **`accountDomain`** removed from `BuildAccountResearchBriefInput`; `mergeNamedBuyerIntoBriefMembers()` merges `namedBuyer.email` or `contactEmail` directly without domain filter
  * `ironleadsBuyingCommitteeExtract.ts` — `extractMailtoEmails()`, `extractDirectoryEmailClues()`, `discoverPublishedEmails()` ranks mailto > directory_label > body_text; `publishedEmailMatchesPerson()` for first.last / f.last / first@ matching; `isRoleLocalPart()` excludes role inboxes from schema inference; `extractTradeShowAndEventSignals()` for booth/speaker/exhibitor clues (Black Hat, RSA, DEF CON, etc.); expanded `RESEARCH_PATHS` for legal/terms/events
  * `ironleadsBuyingCommitteeResearchCore.ts` — persists `publishedEmailDiscovery` and `tradeShowAndEventSignals` on buying committee metadata
  * `ironleadsSuspectReportCore.ts` — auto-persists brief when selector detects gate upgrade without clobbering corpus
  * `SuspectOperatorEditPanel.tsx` — `returnToQueue()` hard navigation; `enrichResultMessage()` warns on remaining `@ironleads.local`
  * `scripts/dev/_remediate-brief-gates.ts` — batch rebuild for legacy rows missing Email gate
* **Agent Boundary:** **Ironleads** harvest and research only — DISPATCH remains HITL per Mandates 17, 20, 24. **Irongate** (Agent 14) validates external scrape bytes before CRM write.
* **Step-by-Step Lab Validation:**
  1. Research SUSPECT with published `info@` on contact page — verify Email gate **UNKNOWN** with company-intake finding; Buyer may still **PASS**.
  2. Prospeo enrich named buyer to work email — verify `selectAccountResearchBriefForReport` upgrades Email to **PASS** while scrape source ledger retained.
  3. Move account to HOLD `channel_competitor` — verify `pathBHold` forces outreach **hold** even with verified email.
  4. Run `tests/unit/ironleadsAccountResearchBrief.test.ts` and `ironleadsBuyingCommitteeExtract.test.ts` — mailto and promote-ready gates pass; verify no `emailMatchesAccountDomain` export remains.

---

<a id="ops-vid-001"></a>

### 🎬 Feature 138: Video Campaign V1 Kickoff & Production Script Narrative Links
* **GRC Function ID:** `OPS-VID-001`
* **Exact Screen Coordinates:** `/dashboard/operations/publishing?desk=video` — Hub & budget link group under **When Risk Enters the Room** campaign index.
* **Operational Purpose:** Surfaces Phase 0 style-lock and V1 episode production working packs on the Publishing desk video tab so operators reach shot lists and AI prompt scripts without hunting the docs tree — narrative index only, not auto-publish or GF quarantine.
* **Technical Mechanics:**
  * `publishingDeskDraftKind.ts` — `PUBLISHING_VIDEO_NARRATIVE_LINKS` Hub & budget group adds `{ label: "V1 kickoff — Phase 0 + shot list", href: "/docs/marketing-strategy/video-series/v1-kickoff-phase0-shotlist" }` and `{ label: "V1 production script — shots + AI prompts", href: "/docs/marketing-strategy/video-series/v1-production-script" }`
  * `opsScheduleSeedSpecs.ts` — `videoCampaign2026SeedSpecs()` **Video Phase 0 — Style lock complete** (`sourceRef: video-series/when-risk-enters-the-room#phase-0`, due **2026-08-09T18:00:00.000Z`) synopsis cites working pack `/docs/marketing-strategy/video-series/v1-production-script`; `href` deep link on calendar card
  * Working pack markdown lives at `docs/marketing-strategy/video-series/v1-kickoff-phase0-shotlist.md` and `docs/marketing-strategy/video-series/v1-production-script.md` — operator-authored, not GF quarantine
* **Agent Boundary:** **Ironscribe** (Agent 5) docs-plane content only — no agent auto-renders video assets; **Ironlogic** (Agent 4) Mandate **16** claim bans apply to any public copy derived from scripts.
* **Step-by-Step Lab Validation:**
  1. Open `/dashboard/operations/publishing?desk=video` — verify **V1 kickoff** and **V1 production script** links appear under Hub & budget.
  2. Click production script link — verify docs reader loads shot list and AI prompt blocks.
  3. Open Ops Calendar — locate **Video Phase 0 — Style lock complete** — verify synopsis and `href` point to production script pack.
  4. Confirm video desk has no Approve/Deny promote panel — narrative links only per Mandate 25.

---

<a id="ops-cal-004"></a>

### 📅 Feature 136: Ops Queue Review Idempotency & Medshield Narrate Exclusion
* **GRC Function ID:** `OPS-CAL-004`
* **Exact Screen Coordinates:** No dedicated UI — server behavior when briefing-queue drafts stage and when cron or operator requests auto-spawn calendar review cards.
* **Operational Purpose:** Prevents duplicate **BRIEFING_REVIEW** / **NEWSLETTER_REVIEW** calendar rows when queue files re-stage after operator closure, and excludes Medshield demo-tenant narrate dumps from auto-promote review workflows.
* **Technical Mechanics:**
  * `app/lib/server/opsScheduleCore.ts` — `ensureQueueReviewActivity()` early-returns `null` when filename matches `/medshield/i`
  * Lookup changed from open-status-only to latest row per `sourceRef` ordered by `updatedAt` desc — if newest row is **DONE** or **CANCELLED**, do not spawn duplicate review card
  * Open rows reuse in-place (same as prior open-only behavior for active reviews)
  * `app/lib/opsScheduleSeedSpecs.ts` — `synopsisForQueueDraft()` Medshield label: **Medshield demo-tenant narrate dump — not a public GF promote candidate (auto-calendar skipped)**
* **Agent Boundary:** **Ironwatch** (Agent 13) calendar integrity; **Ironscribe** (Agent 5) briefing-queue staging triggers review spawn — Medshield narrate never reaches GF promote path per Mandate 29.
* **Step-by-Step Lab Validation:**
  1. Stage a quarantined briefing draft — verify one **IN_REVIEW** calendar row appears with matching `sourceRef`.
  2. Mark that row **DONE** with outcome — re-stage same filename — verify no second review card spawns.
  3. Stage filename containing `medshield` — verify `ensureQueueReviewActivity` returns null and no calendar row created.
  4. Confirm legitimate non-Medshield drafts still receive review cards on first stage.

---

<a id="intel-009"></a>

### 🛰️ Feature 143: September 19 Live Strategic Intel OSINT Manifest Refresh (Historical Binding — Superseded by Feature 149)
* **GRC Function ID:** `INTEL-009`
* **Exact Screen Coordinates:** Historical CRM rows and archived RAG chunks bound to `ironintel-osint-2026-09-19-live` — no longer the active Strategic Intel dashboard binding after **2026-09-21** manifest refresh.
* **Operational Purpose:** Preserved audit reference for operational date **2026-09-19** authorship — dual BOD 26-04 deadline convergence on Cisco ISE **CVE-2026-76460** FCEB **September 19** and Linux kernel triple-KEV countdown to **September 21** — superseded by Feature **149** September 21 live binding.
* **Technical Mechanics:** `manifestId: ironintel-osint-2026-09-19-live`, `generatedAt: 2026-09-19T08:00:00.000Z`; same `riskMetricsCents` registers as Feature **149** workday analysis (**5520000000**, **1388000000**, **1032000000**, **183200000** cents).
* **Agent Boundary:** Historical **Ironintel** / **Ironwatch** ingest only — operators must ingest Feature **149** manifest for live dashboard truth.

---

<a id="intel-010"></a>

### 🛰️ Feature 148: September 20 Live Strategic Intel OSINT Manifest Refresh (Historical Binding — Superseded by Feature 149)
* **GRC Function ID:** `INTEL-010`
* **Exact Screen Coordinates:** Historical CRM rows and archived RAG chunks bound to `ironintel-osint-2026-09-20-live` — superseded by **`ironintel-osint-2026-09-21-live`** on operational date **2026-09-22**.
* **Operational Purpose:** Preserved audit reference for **2026-09-20** authorship — post-deadline ISE/Pixel triage day one framing, Linux kernel sprint to **September 21**, **Chromium V8 CVE-2026-85046** and **Cisco SEG CVE-2026-76461** emphasis in that revision — active dashboard must ingest Feature **149** manifest for live truth.
* **Technical Mechanics:** `manifestId: ironintel-osint-2026-09-20-live`, `generatedAt: 2026-09-20T08:00:00.000Z`; same `riskMetricsCents` registers as Feature **149** (**5520000000**, **1388000000**, **1032000000**, **183200000** cents workday; SaaS disruption **5160000000**, **1456000000**, **1614000000**, **149400000** cents).
* **Agent Boundary:** Historical **Ironintel** / **Ironwatch** ingest only.

---

<a id="intel-011"></a>

### 🛰️ Feature 149: September 21 Live Strategic Intel OSINT Manifest Refresh (Active Binding)
* **GRC Function ID:** `INTEL-011`
* **Exact Screen Coordinates:** Strategic Intel dashboard (IronBoard knowledge ingest); CRM-bound OSINT after `scripts/ingest-strategic-intel-manifest.ts`; IronBoard GRC analyst briefing hooks reading `grcProfessionalResearch.manifest.json`.
* **Operational Purpose:** Binds Ironintel and Ironwatch live external research for operational date **2026-09-21** (follow-through operational date **2026-09-22**) to **BOD 26-04 Linux kernel due-date closure**, **Cisco ISE CVE-2026-76460** post-deadline forensic triage **day two**, post-deadline **Google Pixel CVE-2026-58704** and **Acronis Backup CVE-2026-87886** attestation, **CISA weekly bulletin retirement September 28** KEV-first pivot, **Chromium V8 CVE-2026-87491** enterprise browser orchestration, and CMMC **DARS 2026-O0025 Rev 3** continuity — with whole-cent `riskMetricsCents` board packet registers unchanged from prior manifest revision.
* **Technical Mechanics:**
  * `Ironboard/src/knowledge/grcProfessionalResearch.manifest.json` — `manifestId: ironintel-osint-2026-09-21-live`, `generatedAt: 2026-09-21T08:00:00.000Z`
  * Workday analysis — title **GRC Professional Workday Analysis — September 21 OSINT Sweep**; executive summary: kernel due-date closure, ISE/Pixel/Acronis post-deadline triage, bulletin countdown, V8 browser fleet, Revolut/Metaencryptor/Akira carryovers, CMMC Rev 2 binding
  * Key findings: Linux **CVE-2025-39964** / **CVE-2026-53266** FCEB **September 21**; **CVE-2025-39682** carryover attestation; ISE **CVE-2026-76460** post-deadline day two; CISA bulletin sunset **September 28**; Chromium **CVE-2026-87491** V8 KEV; CMMC DARS Rev 3 continuity
  * `riskMetricsCents` (workday): **5520000000**, **1388000000**, **1032000000**, **183200000** cents
  * SaaS disruption memorandum — title **SaaS Disruption Memorandum — September 21 OSINT**; `riskMetricsCents`: **5160000000**, **1456000000**, **1614000000**, **149400000** cents; key findings: kernel due-date orchestration, ISE NAC root-compromise triage, KEV-first feed retuning, V8 browser fleet, DARS Rev 3 contract-language clarity
  * RAG chunks: **osint-01-cisco-ise-deadline-day** (Ironwatch **2026-09-21** — PSIRT cisco-sa-ISE-ABP-VNSW7Tn5 post-deadline); **osint-02-linux-kernel-kev-triple** (CISA **September 18** alert, due **September 21**); **osint-03-revolut-legal-request-fraud** (~**680** records); **osint-04-cmmc-dars-rev3**; **osint-05-beckman-metaencryptor**; **osint-06-akira-manders** (~**70GB** claim); **osint-07-far-rev3-pending**; **osint-08-post-deadline-carryover** (Pixel, Acronis **CVE-2026-87886**, Minnesota PLC **AA26-097A**, Cl0p Windchill); **saas-01-dmz-ingress** binds **`ironintel-osint-2026-09-21-live`**
  * Industry profile `saasDisruptionExposureIndex`: Finance **94**, Healthcare **88**, Defense **76**, Public Sector **81**, Manufacturing **91**
* **Agent Boundary:** **Ironintel** (Agent 16) authorship; **Ironwatch** (Agent 13) continuous-audit triage receipts; **Irongate** (Agent 14) DMZ sanitization before CRM persistence; **Ironscribe** (Agent 5) external GF copy uses formatted exposure strings — internal manifest storage remains string-encoded BigInt cents only.
* **Financial note:** All `riskMetricsCents` and `peerAleBaselineCents` remain string-encoded BigInt integers; constitutional seed tenants unchanged (Medshield **1110000000**, Vaultbank **590000000**, Gridcore **470000000**, Defense **1600000000** cents).
* **Step-by-Step Lab Validation:**
  1. Run `npx tsx scripts/ingest-strategic-intel-manifest.ts` — verify active binding **`ironintel-osint-2026-09-21-live`**.
  2. On operational date **2026-09-22** — confirm ISE post-deadline triage **day two** receipts; verify kernel due-date attestation closed for **CVE-2025-39964** / **CVE-2026-53266** with **CVE-2025-39682** carryover documented.
  3. Verify SaaS disruption memorandum cites kernel orchestration, ISE identity-plane triage, KEV-first SOC retuning, and V8 browser fleet as seat-based GRC SaaS choke points.
  4. Run `tests/unit/strategicIntelIngress.test.ts` — Irongate path rejects float money in ingest payloads.

---

<a id="hitl-003"></a>

### 🎯 Feature 144: Prospect-Pool CRM Tenant Scope for Approvals & Sales Touch History
* **GRC Function ID:** `HITL-003`
* **Exact Screen Coordinates:** `/dashboard/admin/approvals` — pending queue hydration; **OutreachReplyReceiptPanel** and **SalesTouch2QueuePanel** (SALES / ALL filters); server-only — no separate UI chrome.
* **Operational Purpose:** Ensures GLOBAL_ADMIN Approvals desk, DISPATCH handler, Needs enrichment demotion, Touch-2 queue, and outreach reply receipt queries bind Postgres RLS to the air-gapped **prospect-pool** tenant so Path B CRM rows remain visible and mutable under tenant isolation — fixing empty-queue false negatives when operators used unscoped Prisma against Medshield or default connection GUC.
* **Technical Mechanics:**
  * `app/lib/server/ironleadsTenantScope.ts` — **`withProspectPoolTenant(callback)`** sets tenant GUC for transaction scope
  * `approvalQueueCore.ts` — all pending draft fetch/count/park/incoming-query paths scoped
  * `approvalNeedsEnrichmentCore.ts` — N/E transaction scoped
  * `app/api/admin/approvals/[id]/route.ts` — findUnique, contact phone update, summary update, purge paths scoped
  * `salesTouch2QueueCore.ts`, `salesTouchHistoryCore.ts`, `outreachReplyReceiptCore.ts` — scoped reads; Touch-2 queue joins pending TOUCH2 drafts by `contactId` (Feature **150**)
  * `salesAgentConsoleCore.ts` — **`PROSPECT_POOL_TENANT_UUID` `11111111-1111-4111-8111-111111111111`**; **`resolveProspectPoolTenantId()`** honors `IRONFRAME_PROSPECT_POOL_TENANT_UUID` — **removed Medshield fallback**
* **Agent Boundary:** **Ironguard** (Agent 12) RLS GUC enforcement; **Ironleads** (perimeter worker) prospect-pool only; **Ironwatch** (Agent 13) DISPATCH audit unchanged.
* **Financial note:** CRM scope fix does not alter tenant `ale_baseline BIGINT` — Path B **499900** cents checkout metadata unchanged.
* **Step-by-Step Lab Validation:**
  1. Seed pending SALES draft on prospect-pool contact — open Approvals — verify row appears.
  2. DISPATCH SMS draft — verify contact phone update persists under scoped transaction.
  3. Run `tests/architecture/tenantBoundRequestSurfaces.test.ts` — markers include `withProspectPoolTenant` on approvals routes.
  4. Run `tests/unit/agentPerimeter.test.ts` with `IRONFRAME_PROSPECT_POOL_TENANT_UUID` set — sales agent scopes prospect pool.

---

<a id="hitl-004"></a>

### 🎯 Feature 150: Sales Touch Stage Filters, Touch 2 Card Pipeline & Touch 3 Value Drop DISPATCH Gates
* **GRC Function ID:** `HITL-004`
* **Exact Screen Coordinates:** `/dashboard/admin/approvals?kind=SALES&touch=TOUCH1|TOUCH2|TOUCH3` — touch toggle row under geo/sort controls; **SalesTouch2QueuePanel** full-width below grid when kind **SALES** or **ALL**; shared draft editor for Touch 1–3.
* **Operational Purpose:** Separates Path B outreach cadence stages in the HITL queue so operators review Touch 1 cold, Touch 2 re-anchor follow-ups, and Touch 3 Value Drop drafts without mixed-stage noise; unifies Touch 2 due cards with pending HITL drafts via deep link into the same Approvals editor as Touch 1; enforces distinct EMAIL DISPATCH perimeter for Touch 3 (no Path B economics pitch).
* **Technical Mechanics:**
  * `approvalDraftKinds.ts` — `parseApprovalTouchFilter()`, `approvalsHref(..., touch)`, `approvalsDraftHref(..., touch)`
  * `AdminApprovalDashboardClient.tsx` — client filter on `salesTouchStage`; `touchCounts` memo; `expectedTouch` on dispatch preview payload
  * `SalesTouch2QueuePanel.tsx` — card grid (`draftKindCardClass("SALES")`); `openDraft()` → `approvalsDraftHref(id, "SALES", "US", "GEO", "TOUCH2")`
  * `salesTouch2QueueCore.ts` — `pendingDraftId`, `pendingSubject` from `PENDING_SALES_DRAFT_TAG` rows where `inferSalesTouchStage()` === **TOUCH2**
  * `approvalDispatchValidation.ts` — `expectedTouch` input; Touch **3** branch uses `looksLikeTouch3ValueDropBody()` / `looksLikeTouchEconomicsBody()` from `app/lib/salesTouch3Body.ts`
  * `app/api/admin/approvals/[id]/route.ts` — server-side `expectedTouch` from `inferSalesTouchStage()` + body `salesTouch`
  * `salesHumanVoice.ts` — `lintSalesHumanVoice(body, { allowMissingPeerCta?: boolean })`
* **Agent Boundary:** **SalesTeam** (perimeter) drafts only — GLOBAL_ADMIN DISPATCH after gates; **Ironguard** (Agent 12) tenant scope unchanged (Mandate 30); **Ironwatch** (Agent 13) logs dispatch closure.
* **Financial note:** Touch **1**–**2** EMAIL must still cite Command Design Partner **499900** cents positioning per Mandate **17**; Touch **3** must not persist or transmit Path B price literals in EMAIL body — Value Drop is diagnostic-only copy.
* **Step-by-Step Lab Validation:**
  1. Open `/dashboard/admin/approvals?kind=SALES&touch=TOUCH2` — verify counts match pending TOUCH2 drafts only.
  2. From Touch 2 queue panel, click card with pending draft — verify editor opens with `draft=` param and Touch **2** filter preserved.
  3. Attempt DISPATCH on Touch **3** draft with $4,999 body — verify HTTP **400** validation errors from `validateApprovalDispatch`.
  4. DISPATCH valid Touch **3** three-check body — verify send succeeds and human-voice lint accepts missing workflow-review CTA.

---

<a id="ironleads-012"></a>

### 🔬 Feature 151: Account Research Brief Legacy Metadata Hardening (Ops Hub Digest 350965605)
* **GRC Function ID:** `IRONLEADS-012`
* **Exact Screen Coordinates:** `/dashboard/operations/ironleads/suspects/[contactId]` — suspect report and **SuspectOperatorEditPanel**; server `buildIronleadsSuspectReport()` digest paths.
* **Operational Purpose:** Prevents Ops Hub and suspect dossier crashes when CRM `contact.metadata.accountResearchBrief` holds partial legacy shapes missing `gates.buyer`, `snapshot.status`, or `outreach.status` — treats incomplete blobs as missing and prefers rebuilt brief gates without wiping rich scrape corpus on thin rebuilds.
* **Technical Mechanics:**
  * `resolveAccountResearchBrief()` — returns `null` when gates/snapshot/outreach incomplete
  * `selectAccountResearchBriefForReport()` — reason **`incomplete_persisted_gates`**; optional chaining on `outreach?.status`, `buyerMap ?? []`, `snapshot?.status`
  * `ironleadsSuspectReportCore.ts` — optional **`accountResearchBrief?.outreach?.status`**
  * `SuspectOperatorEditPanel.tsx` — **`report.accountResearchBrief?.outreach?.status`**
  * `tests/unit/ironleadsAccountResearchBrief.test.ts` — Strong Security Brasil incomplete gates case
* **Agent Boundary:** **Ironleads** server core only — no auto-DISPATCH; **Ironwatch** (Agent 13) audit on brief persist unchanged.
* **Financial note:** Brief gates remain non-monetary PASS/FAIL/UNKNOWN — ALE and Path B **499900** cents live in product knowledge spine only.
* **Step-by-Step Lab Validation:**
  1. Run `tests/unit/ironleadsAccountResearchBrief.test.ts` — incomplete persisted gates test passes.
  2. Open suspect with legacy metadata missing buyer gate — verify report renders and rebuild persists full gates.

---

<a id="ops-012"></a>

### 📅 Feature 152: Video Campaign Slate Reschedule — October–November 2026 Window
* **GRC Function ID:** `OPS-012`
* **Exact Screen Coordinates:** Ops Calendar — **When Risk Enters the Room** seed cards from `videoCampaign2026SeedSpecs()`; Publishing desk video tab deep links unchanged.
* **Operational Purpose:** Moves V1–V6 build/publish milestones to **October–November 2026** so Path B outreach and LinkedIn voice-pass posts precede video production spend; companion story bank aligns to **2026-10-14**.
* **Technical Mechanics:** `opsScheduleSeedSpecs.ts` — V1 build **2026-10-16T18:00:00.000Z**, V1 publish **2026-10-21T16:00:00.000Z**, V2 build **2026-10-30**, V2 publish **2026-10-28**, V3 build **2026-10-29**, phase-2 window **2026-10-02**, V3–V5 publish **2026-10-11** through **2026-10-25**, V6 build **2026-11-11**, V6 publish **2026-11-16**; companion story bank **2026-10-14T17:00:00.000Z**; LinkedIn calendar synopses note voice-pass **2026-09-21** ready for publish on **2026-08-31-nis2-iso-accountability**, **2026-09-04-evidence-provenance**, **2026-09-15-ma-security-debt**
* **Agent Boundary:** Ops Hub schedule seed only — Mandate **25** video desk remains narrative index, never auto-publish.
* **Step-by-Step Lab Validation:**
  1. Re-seed calendar via `POST /api/admin/operations-hub/schedule` `{ action: "seed-all-projects" }` on staging.
  2. Open Ops Calendar — verify V1 publish due **October 21**, not **September 21**.
  3. Open `/dashboard/operations/publishing?desk=linkedin&li=2026-09-04-evidence-provenance` — verify voice-pass ready copy in calendar synopsis matches desk draft.

---

<a id="fellows-001"></a>

### 🎓 Feature 145: IRONFRAMEGRC // FELLOWS — Academic Tier-2 Lab (Architectural Specification)
* **GRC Function ID:** `FELLOWS-001`
* **Exact Screen Coordinates:** Public hosts `fellows.ironframegrc.com`, `lab.ironframegrc.com`, `fellows.localhost` (via `isFellowsPortalHost()`); internal App Router prefix **`/fellows`**; lab console after email verification session cookie **`ironframe_fellow_session`**.
* **Operational Purpose:** Provides WGU Cybersecurity BS / MSCSIA candidates an independent academic sandbox — approximately **45–60** minutes in-browser guided missions plus a **60**-day access window for capstone writing — producing tamper-evident exports for portfolio appendices without requiring real client data or industry certification claims.
* **Deploy status (2026-09-21 delta):** `daily_code_diff.txt` shows working-tree **module body evacuation** (public pages, config, Prisma schema, APIs, math, tests stubbed). **Restore fellows blast-radius from git HEAD before production** — empty stubs violate Mandate 31 and break academic lab ingress. This feature entry documents the **intended restored architecture** parsed from the diff removal hunks.
* **Technical Mechanics (restored module):**
  * **Portal config** — `config/fellowsPortal.ts`: `FELLOWS_PORTAL_ORIGIN`, `FELLOWS_INTERNAL_PREFIX`, `FELLOWS_ACADEMIC_SANDBOX_ID` **`ironframe-academic-sandbox`**, lab clients **`mssp-client-001`** / **`mssp-client-002`**, session cookie name
  * **Database isolation** — `prisma/fellows/schema.prisma` on schema **`academic_fellows`**; models **`Fellow`**, **`FellowMission`**, **`FellowMissionReceipt`**, **`FellowRubricSubmission`**; enums for track, employer type, mission codes; **`FELLOWS_DATABASE_URL`** + **`FELLOWS_DIRECT_URL`**; `lib/fellowsDatabaseUrl.ts` **`assertFellowsDatabaseIsolated()`**; dedicated client `@/lib/prismaFellows`
  * **Apply flow** — `POST /api/fellows/apply` Zod schema (LinkedIn URL required, employer **OTHER** requires `employmentContext`); creates **`PENDING_VERIFY`** fellow; **`issueFellowAccessVerification`** email link; never mutates existing fellow from public endpoint until mailbox proof
  * **Session** — `GET /api/fellows/me` extends **`sandboxExpiresAt`** on activity via **`extendFellowsSandboxExpiry()`**; returns mission progress and rubric unlock when **4** missions **PASSED**
  * **Mission 01 exposure math** — `app/lib/fellows/exposureMath.ts`: **`estimatedExposureCents(sleCents, aroMilli)`** = **`(sleCents * aroMilli) / 1000n`**; ARO stored as milli-occurrences per year (**1000** = **1.0**/yr); **`floatTheaterExposureDollars`** demo only — never pass gate; lab registers use **`lossExposureCents`** BigInt in `labFixtures.ts` (e.g. **1250000**, **420000**, **89000** cents examples)
  * **Mission 02 ingest** — `POST /api/fellows/sandbox/untrusted-ingest` quarantine-before-trust on **`LAB_UNTRUSTED_VENDOR_ARTIFACT`**
  * **Mission 03 boundary** — cross-enclave probe returns **403** + receipt (zero bleed) between **`FELLOWS_LAB_CLIENT_A`** and **`FELLOWS_LAB_CLIENT_B`**
  * **Mission 04 lineage** — `lib/capstone/exportPackage.ts` server-issued SHA-256 export pack; **`buildLabAuditRegister(fellowShortId)`** with operator sign-off **`fellow:{shortId}`**
  * **Receipts** — `app/lib/fellows/receipts.ts` **15**-minute TTL mission receipts; **`consumeFellowMissionReceipt`** single-use; telemetry **`POST /api/fellows/missions/telemetry`** marks mission **PASSED**
  * **Notes** — `POST /api/fellows/missions/notes` requires **`productImprovementConsent`** to persist methodology notes (export + anonymized product Eyes — not sales outreach)
  * **Rubric** — `POST /api/fellows/rubric` after four passes; integer scores only — not USD cents
  * **Public landing** — `app/(public)/fellows/page.tsx` FAQ clarifies **60**-day seat is access for writing not continuous lab time; missions **EXPOSURE**, **INGEST**, **BOUNDARY**, **LINEAGE**; capstone companion checklist; architecture table cites BIGINT financial engine and dedicated Postgres schema
* **Agent Boundary:** Academic plane is **Tier-2 isolated** from Path B tenant **`Tenant`/`Evidence`** models — **Irongate** (Agent 14) pattern on Mission 02; **Irontrust** (Agent 3) whole-cent math on Mission 01; **Ironwatch** (Agent 13) server receipts — not client-mocked pass; fellows plane must never write production **`AuditLog`** for lab self-test semantics (TAS Section 4.3 shadow isolation on production tenants only).
* **Step-by-Step Lab Validation (after module restore):**
  1. Run `tests/unit/fellowsExposureMath.test.ts` — **25000000n** SLE × **500n** milli ARO → **500000n** cents exposure.
  2. Run `tests/unit/fellowsDatabaseUrl.test.ts` — rejects primary URL without **`academic_fellows`** schema.
  3. Run `tests/unit/fellowsPortal.test.ts` — host allowlist for fellows subdomain.
  4. Run `scripts/dev/_smoke-fellows-prod.mjs` against staging fellows origin — apply → verify → mission telemetry → export.
  5. Confirm **`FELLOWS_DATABASE_URL`** unset throws at runtime outside CI placeholder build phase.

---

<a id="integrity-003"></a>

### 🛡️ Feature 146: Integrity Hub Schema-Lag Soft-Fail & Quiet Server Boundaries
* **GRC Function ID:** `INTEGRITY-003`
* **Exact Screen Coordinates:** `/integrity` — Integrity Hub vault snapshot, chaos ledger stream, synthetic employee targets, workforce agent registry LKG strip (server-rendered).
* **Operational Purpose:** Keeps Integrity Hub reachable on preview deployments and during migration lag when **`ThreatEvent.tenant_id`**, **`AgentRegistry`**, or shadow simulator count queries fail — returning empty or safe-default telemetry instead of poisoning the Next.js overlay with verbose server `console.error` stacks in the browser.
* **Technical Mechanics:**
  * `app/(dashboard)/integrity/page.tsx` — `readIntegrityVaultSnapshotWithRegistry` catch without log; `fetchResolvedChaosLedgerRows().catch(() => [])`; synthetic employee `findMany` catch returns `[]`
  * `integrityLedgerServer.ts` — try/catch around `threatEvent.findMany` — comment: preview DBs may lack **`ThreatEvent.tenant_id`**
  * `workforceAgentRegistryServer.ts` — `isMissingThreatEventTenantId()` latches **`threatEventTenantScopeUnavailable`**; `performWorkforceAudit()` outer try/catch soft-fail
  * `shadowSimulatorArmServer.ts` — try/catch returns **`infiltrBotSimActive: false`**, **`phishBotSimActive: false`**
* **Agent Boundary:** **Ironwatch** (Agent 13) health display degrades gracefully — not a substitute for skipping migration deploy on production; **Irontech** (Agent 9) self-heal still requires schema parity on live tenants.
* **Step-by-Step Lab Validation:**
  1. Open `/integrity` on preview without full threat schema — verify page loads without red overlay.
  2. Apply pending migrations — verify chaos ledger repopulates when **`tenant_id`** column exists.
  3. Confirm production Medshield tenant still shows non-empty ledger when threats resolved.

---

<a id="auth-003"></a>

### 🔗 Feature 147: Auth Next-Path Sanitize — Block API Logout Loops
* **GRC Function ID:** `AUTH-003`
* **Exact Screen Coordinates:** Invisible — Supabase auth callback and post-login redirect resolution; password recovery and invite flows using `sanitizeAuthNextPath`.
* **Operational Purpose:** Prevents open redirect and **logout↔login infinite loops** when `next` query parameters point at **`/api/auth/session-logout`** or other **`/api/*`** handlers instead of workspace routes like **`/integrity`** or **`/get-started`**.
* **Technical Mechanics:**
  * `app/lib/auth/publicAppUrl.ts` — **`sanitizeAuthNextPath(raw, fallback = "/integrity")`** rejects paths not starting with single `/`, protocol-relative `//`, embedded `://`, session-logout API paths, and any `/api` prefix
  * `tests/unit/publicAppUrl.test.ts` — cases for session-logout query, bare `/api/auth/session-logout`, `/api/health`, and valid `/exports` preservation
  * `middleware.ts` — **`isSessionLogoutRoute`** included in session-bootstrap passthrough alongside auth confirm/callback routes for IronBoard shared-context bridge
* **Agent Boundary:** **Ironguard** (Agent 12) session perimeter; no change to **`readSimulationPlaneEnabled()`** shadow gate.
* **Step-by-Step Lab Validation:**
  1. Run `tests/unit/publicAppUrl.test.ts` — sanitize block matrix passes.
  2. Attempt login with malicious `?next=/api/auth/session-logout` — verify landing **`/integrity`** (or configured fallback).
  3. Sign out via `/api/auth/session-logout?next=%2Flogin` — verify middleware passthrough without tenant cookie re-stamp loop.

---

## 🧬 Chapter 5: Nineteen-Agent Architecture Cross-Reference (Delta Verification)

Today's code delta touches the following agents. Use this matrix during audits to confirm boundary integrity for operational date **2026-09-22**:

| Agent # | Codename | Today's Delta Touchpoints |
|---------|----------|-------------------------|
| 1 | Ironcore | Ironintel September 21 OSINT manifest (`ironintel-osint-2026-09-21-live` — Feature **149**); Sales touch stage filters + Touch 3 Value Drop DISPATCH gates (Feature **150**); Touch 2 card pipeline with pending-draft deep links; Account Research Brief legacy hardening (Feature **151**); video slate October–November reschedule (Feature **152**); prospect-pool CRM scope completion on Approvals DISPATCH (Feature **144** retained) |
| 3 | Irontrust | Constitutional seed baselines unchanged (**1110000000**, **590000000**, **470000000**, **1600000000**); Path B anchor **499900** cents unchanged on Touch **1**–**2** EMAIL; Touch **3** Value Drop forbids economics body; September 21 OSINT `riskMetricsCents`: **5520000000**, **1388000000**, **1032000000**, **183200000** cents workday; SaaS disruption **5160000000**, **1456000000**, **1614000000**, **149400000** cents |
| 5 | Ironscribe | September 21 OSINT executive summaries and SaaS disruption memorandum cite BIGINT-cent board packet language; LinkedIn calendar voice-pass ready synopses must not auto-post (Mandate **27**); DARS **2026-O0025 Rev 3** and FAR **2026-001 Rev 3** regulatory chunks unchanged in binding |
| 6 | Ironlock | Prospect-pool RLS GUC on CRM writes — DISPATCH cannot bypass tenant scope via GLOBAL_ADMIN role alone (Mandate **30**) |
| 12 | Ironguard | **`withProspectPoolTenant`** on Approvals route mutations; touch filter is UI-only — server still validates DISPATCH perimeter |
| 13 | Ironwatch | Linux kernel KEV due-date closure **September 21**; Cisco ISE **CVE-2026-76460** post-deadline triage **day two** on **2026-09-22**; Pixel and Acronis post-deadline attestation; **Chromium V8 CVE-2026-87491** browser fleet; CISA bulletin sunset **September 28**; Revolut/Metaencryptor/Akira carryovers; **osint-08** Minnesota PLC and Cl0p Windchill attestation |
| 14 | Irongate | All September 21 OSINT bytes pass DMZ sanitization (`saas-01-dmz-ingress` binds **`ironintel-osint-2026-09-21-live`**) |
| 16 | Ironintel | Active manifest **`ironintel-osint-2026-09-21-live`** (authored **2026-09-21**) — kernel due-date closure, ISE day-two triage, KEV-first pivot, V8 browser fleet; CMMC DARS Rev 3 continuity |

**Ironintel OSINT note (2026-09-22):** Active manifest **`ironintel-osint-2026-09-21-live`** — run `npx tsx scripts/ingest-strategic-intel-manifest.ts` after deploy if not yet ingested. Operational **2026-09-22** is **Cisco ISE CVE-2026-76460** post-deadline forensic triage **day two** after FCEB **September 19** — finalize compromise assessment and iACL review receipts. **Linux kernel cluster** **CVE-2025-39964**, **CVE-2026-53266**, and **CVE-2025-39682** carryover attestation should close on **September 21** deadline receipts. **Google Pixel CVE-2026-58704** and **Acronis Backup CVE-2026-87886** remain in post-deadline verification. **Chromium V8 CVE-2026-87491** — enforce enterprise browser patch cadence. **CISA** weekly bulletin ends **September 28** — retune SOC feeds to KEV-first prioritization. All `riskMetricsCents` values are string-encoded BigInt integers — workday analysis **5520000000** cents median annual GRC program; SaaS disruption memorandum **5160000000**, **1456000000**, **1614000000**, **149400000** cents.

**Sales touch cadence note (2026-09-22):** Use `/dashboard/admin/approvals?kind=SALES&touch=TOUCH2` to review follow-ups only; Touch 2 queue cards with pending drafts open the shared editor — queue HITL draft before card is clickable. Touch **3** DISPATCH requires three-check Value Drop body — no **499900** cents pitch in EMAIL (Mandate **17**).

**Prospect-pool CRM note (2026-09-22):** If Approvals desk shows zero pending SALES drafts while SalesTeam poll logs drafts created, verify **`withProspectPoolTenant`** deployment and **`IRONFRAME_PROSPECT_POOL_TENANT_UUID`** alignment with seed **`11111111-1111-4111-8111-111111111111`**.

**Video slate note (2026-09-22):** V1 publish moved to **2026-10-21** — re-seed Ops calendar to refresh due dates (Feature **152**).

**Ops schedule GTM gate note (2026-08-11):** Cold outreach gate rescheduled to **August 17** — wait for LinkedIn week-1 + Video Phase 0. First SalesTeam DISPATCH batch rescheduled to **August 24** — after cold outreach gate. Re-seed calendar via `POST /api/admin/operations-hub/schedule` `{ action: "seed-all-projects" }` to refresh due dates on existing rows.

**LinkedIn Wed first-comment note (2026-08-11):** `wed-product-demo` body carries single `/product-demo` link; workflow-review CTA at `https://ironframegrc.com/register/contact` goes in **first comment** immediately after publish — not in main body. Research block documents held alternate `linkedin-drafts-hold-board-reporting-gap.md`. Never auto-post per Mandate 27.

**Video V1 production pack note (2026-08-10 retained):** Publishing desk video tab links **V1 kickoff — Phase 0 + shot list** and **V1 production script — shots + AI prompts**. Ops Calendar **Video Phase 0 — Style lock complete** due **2026-08-09** cites working pack href. Never auto-publish video markdown to GF quarantine per Mandate 25.

**Ironleads email gate note (2026-08-10 retained):** `isPromoteReadyWorkEmail()` remains the Email gate PASS predicate — company intakes (`info@`, `hello@`) stay **UNKNOWN** per Mandate 28. Employer-domain ownership gate removed — operators must HITL-verify Prospeo hits on non-matching domains before DISPATCH. Run `npx tsx scripts/dev/_remediate-brief-gates.ts` to rebuild legacy Email gates on active SUSPECT rows.

**Apex marketing redirect note (2026-08-10 retained):** Unauthenticated apex `/` guests redirect to `/marketing` — not `/login`. Signed-in operators still land Integrity Hub Command Center.

**Ops schedule catch-up note (2026-08-07 retained):** DORA supervision promoted **2026-08-06**. GF-2026-001 editorial + product-boundary **PASS** **2026-08-05**. Control-First newsletter quarantine draft awaiting Approve. EU AI Act claim alignment **PASS** in `docs/ops/eu-ai-act-claim-alignment-2026-08.md`. Stakeholder benefit-map **CANCELLED**. Stripe lifecycle **September 28**; LinkedIn Fri due **August 8**.

**Queue review idempotency note (2026-08-07):** `ensureQueueReviewActivity()` skips Medshield narrate dumps per Mandate 29. Closed **DONE**/**CANCELLED** calendar rows are not reopened on re-stage. Operators must manually re-seed if a closed review needs reopening.

**Apex marketing redirect note (2026-08-07 retained):** See 2026-08-10 note above for behavior confirmation.

**Ops Publishing LinkedIn note (2026-08-07 retained):** `discoverPublishedEmails()` prefers mailto hrefs from legal/terms/event pages before pattern guesses. `returnToQueue()` uses hard navigation — soft router no longer leaves operators stuck on dossier after HOLD. Employer-domain gate removed in 2026-08-08 delta — see Mandate 28.

**Ironleads leadership search note (2026-08-05 retained):** Provider cascade Brave → SerpAPI → Google CSE legacy when site scrape yields zero plausible names. Allowlisted press URLs only per Mandate 23. Email permutation candidates test first.last → f.last → first@ — MX PASS does not prove ownership on catch-all domains. Portal Research auto-continues up to **8** batches. Prospeo enrich warns when `@ironleads.local` persists per Mandate 24.

**Ops Publishing desk note (2026-08-05 retained):** GF quarantine desks at briefings/newsletters/research/video. Research drafts match `*-draft-research-*`. Video desk links When Risk Enters the Room campaign hub — narrative only. Draft reader modal with Escape dismiss. Published ledger kind splits Briefings index from newsletters and industry research per Mandate 26.

**Ironleads dossier note (2026-08-05 retained):** PROSPECT contacts redirect from suspect URL unless `?dossier=1`. Queue decision banners on portal after promote/hold/discard/restore. Active row links default to dossier mode.

**Ironleads readiness note (2026-08-03 retained):** Active queue sorts named buyers and fullest dossiers first (`compareSuspectReadiness`). Research batch sorts thinnest dossiers first to fill gaps. MSSPProviders paste accepts `=========` card blocks — firm name only. `readinessScore` is dossier tier — not USD cents per Mandate 22.
...
**Ops Publishing desk note (2026-08-02 retained):** Briefings and newsletters approve/deny live at `/dashboard/operations/publishing?desk=briefings|newsletters` — extended to research and video tabs in 2026-08-05 delta per Mandate 25. Ops Today (`/dashboard/operations`) is overview + calendar + workforce + CRM + Teams only. Legacy tab URLs redirect preserving `?draft=`. GF desk-run API unchanged.
...
**SaaS call KB note (2026-08-01):** **20+** curated entries in `saasCallKnowledgeBase.ts` with helper functions pinned to `commercial.ts`. Key refute: do not stack **499900** + **350000** cents for fourth tenant on Path B. Core includes **4** entities — fourth is inside Core at **3500000** cents/yr planned GA, not Core + Paid Enclave. **6** Paid enclaves (`PAID_ENCLAVES_TO_FILL_MULTI`) fill Multi from Core's **3** included Subtenants. New test suite `tests/unit/saasCallKnowledgeBase.test.ts`.

**Ops Calendar filter note (2026-08-01):** `calendarDueFrom` / `calendarDueTo` inclusive date bounds filter `dueAt` (fallback `completedAt`). **Today · WF review** preset for same-day workflow-review cards. Done column shows full filtered set when any filter active.

**Ops schedule seed note (2026-08-01):** Pre-outreach gate milestones marked **DONE** with PASS receipts dated **2026-07-19** — A1–A4 launch checklist clear, Textbelt SMS path ready, ICP shortlist with Pivot Point + BlueRadius PROSPECTs. Warm-network advisor asks **CANCELLED** — cold path from prospect-pool proceeds.

**Control-to-Capital federation note (2026-07-31 retained):** `docs/sales/control-to-capital-market-narrative.md` is READ-federated into IronBoard at runtime from `resolveDocsRoot()`. Perimeter Cloud Run images must include `docs/` tree — `.dockerignore` negation and Dockerfile COPY are mandatory for claim-ban grounding off-repo. Operators edit claim sheet locally, commit, redeploy perimeter workers per `PRODUCT_KNOWLEDGE_BLAST_RADIUS`.

**Heatmap Amnesty note (2026-07-31):** Priority-1 Path B demand gen — public landing only. Three-touch email sequence in `heatmap-amnesty-campaign.md` remains HITL DISPATCH only. Copy must not claim SEC requires FAIR, boards universally reject heatmaps, or competitors cannot quantify. Path B price **499900** cents displays via spine `formatPathBUsd()` only.

**SaaS call KB note (2026-07-31):** `capacity-clients-tenants` id answers: no platform technical ceiling like “50 max clients,” but commercial entitlement capped — Path B **1** Primary Entity + **2** Subtenant Enclaves; planned GA Command Core **1** Primary + **3** Subtenants; Paid Enclave tiers at published list. Do not promise unlimited enclaves at flat Core. `ale-reporting` id: whole-cent estimated loss exposure, ranges over single true ALE, SEC materiality nuance.

**Marketing copy note (2026-07-31):** Homepage and Heatmap Amnesty landing share Control-to-Capital doctrine — heatmaps as context, whole cents as decision layer. `animate-fadeIn` Tailwind keyframe is UI-only — no agent state machine phase change.

**Ironleads SUSPECT location note (2026-07-20 retained):** Website URLs normalize to HTTPS from metadata or deal domain. Postal addresses render as single-line `addressLine` for operator scan — never float geocodes. Forensic reports require `prospect-pool` tenant before SalesTeam outreach. Blocker codes are explicit — operators must clear each before expecting PROSPECT poll drafts.
...
### 🚨 Alert 42: Outbound Copy Violates Control-to-Capital Claim Bans (Mandate 16)
* **The Root Cause:** SalesTeam draft, Heatmap Amnesty landing, boardroom answer, or workflow-review assist emitted forbidden claims — “most urgent unmet need,” “true ALE,” “SEC requires FAIR,” “competitors cannot quantify,” unverified CAGR bands, or universal “single-entity only” competitor assertions — without grounding in `docs/sales/control-to-capital-market-narrative.md`.
* **How to Resolve It Yourself:**
  1. Open operator library slug **control-to-capital** at `/dashboard/operations/library/control-to-capital` — compare draft against **Claim bans** and **Gap framing** tables.
  2. Verify IronBoard startup log reports control-to-capital file loaded in docs matrix — if count is low, redeploy perimeter worker with `docs/` tree present (`Ironboard/Dockerfile` COPY).
  3. Edit offending copy to prefer **estimated annual loss exposure** / **P50–P90 ranges** with whole-cent storage language — heatmaps as optional context only.
  4. Run `npm run knowledge:check` — confirm `.fingerprint.json` aligns with `commercial.ts`; re-HITL draft before DISPATCH.
  5. Run `tests/unit/workflowReviewCallAssistCore.test.ts` — capacity and board-math answers must cite SaaS KB entitlement framing, not invented client ceilings.

### 🚨 Alert 43: Heatmap Amnesty Landing Returns 403 on Cloud Host
* **The Root Cause:** `/marketing/heatmap-amnesty` not classified as narrow public funnel path on cloud deployment — or stale middleware cache before 2026-07-31 `/marketing/*` wildcard coverage.
* **How to Resolve It Yourself:**
  1. Confirm `isPublicCloudIngressPath` includes `/marketing` and `/marketing/*` child paths in `deploymentQuarantine.ts`.
  2. Navigate to `/marketing/heatmap-amnesty` on cloud host without `IRONFRAME_ALLOW_PUBLIC_INGRESS=1` — expect HTTP **200**, not **403**.
  3. Verify `grcRouteMatch.ts` and `publicFunnelShell.ts` classify the route as public marketing.
  4. Run `tests/unit/deploymentQuarantine.test.ts` — narrow funnel includes marketing child paths.

### 🚨 Alert 44: Sales DISPATCH Blocked by Perimeter Gate (Mandate 17)
* **The Root Cause:** Operator attempted live DISPATCH on a SALES draft that violates `validateApprovalDispatch()` — company on HOLD shortlist (**blueradius**), EMAIL to `@ironleads.local` harvest placeholder, operator dry-run inbox without acknowledge checkbox, or Sales EMAIL body missing **$4,999**/Command Design Partner mention, missing workflow review CTA, or offering free pilot/PoC/trial.
* **How to Resolve It Yourself:**
  1. Read validation error list on Approvals desk — each error maps to a Mandate 17 gate.
  2. **HOLD company:** Purge draft or re-qualify with named buyer before Path B cold outreach — do not override server gate.
  3. **@ironleads.local:** Switch channel to SMS when E.164 phone present, or paste real buyer email for EMAIL DISPATCH.
  4. **Dry-run inbox:** Check **Acknowledge dry-run to my inbox** for smoke test only — use real prospect email for production DISPATCH.
...
### 🚨 Alert 53: Approvals Desk Empty While SalesTeam Poll Shows Pending Drafts
* **The Root Cause:** Ironboard CRM queries ran without prospect-pool RLS GUC — pre-**2026-09-20** unscoped `prisma.ironboardCrmInteraction.findMany` returned zero rows under tenant Row-Level Security even for GLOBAL_ADMIN operators.
* **How to Resolve It Yourself:**
  1. Deploy **`withProspectPoolTenant`** scoping on `approvalQueueCore.ts` and Approvals API routes per Mandate 30.
  2. Verify **`IRONFRAME_PROSPECT_POOL_TENANT_UUID`** matches seed prospect-pool UUID **`11111111-1111-4111-8111-111111111111`**.
  3. Confirm `resolveProspectPoolTenantId()` does not fall back to Medshield.
  4. Reload `/dashboard/admin/approvals?kind=SALES` — pending tag rows must appear.

### 🚨 Alert 54: Integrity Hub Red Overlay on Preview Database
* **The Root Cause:** Pre-**2026-09-20** server `console.error` from `ThreatEvent` queries when **`tenant_id`** column missing; or unhandled Prisma errors bubbled to Next.js dev overlay.
* **How to Resolve It Yourself:**
  1. Expected soft-fail after **2026-09-20** delta — page should render with empty chaos ledger until migrations apply.
  2. Run `npx prisma migrate deploy` on target database — restore **`ThreatEvent.tenant_id`** column.
  3. Reload `/integrity` — verify resolved threat rows populate ledger when schema current.

### 🚨 Alert 55: Login Redirect Loop Through Session Logout API
* **The Root Cause:** `next` query parameter pointed at **`/api/auth/session-logout`** or another **`/api/*`** route — post-login navigation re-entered logout handler.
* **How to Resolve It Yourself:**
  1. Deploy **`sanitizeAuthNextPath()`** in `publicAppUrl.ts` per Feature **147**.
  2. Run `tests/unit/publicAppUrl.test.ts` — session-logout and `/api/health` rejection cases pass.
  3. Clear poisoned bookmark URLs — use `/login` without unsafe `next` param.

### 🚨 Alert 56: Academic Fellows Lab Returns 404 or Empty Page
* **The Root Cause:** Working-tree fellows module bodies stubbed empty in **2026-09-21** delta — public `page.tsx`, APIs, and Prisma schema files contain no executable logic.
* **How to Resolve It Yourself:**
  1. Restore fellows blast-radius from git HEAD: `config/fellowsPortal.ts`, `app/(public)/fellows/**`, `app/api/fellows/**`, `app/lib/fellows/**`, `prisma/fellows/**`, `lib/fellowsDatabaseUrl.ts`, `lib/prismaFellows.ts`, `lib/capstone/exportPackage.ts`, fellows unit tests.
  2. Set **`FELLOWS_DATABASE_URL`** with **`schema=academic_fellows`** — run fellows migrations via `prisma.fellows.config.ts`.
  3. Run `tests/unit/fellowsExposureMath.test.ts` and `scripts/dev/_smoke-fellows-prod.mjs` before promoting fellows host.

---

## 📋 Chapter 7: Unit Test Verification Checklist (Today's Delta)

Independent learners and compliance auditors must confirm the following Vitest suites pass before signing a daily lab receipt for operational date **2026-09-21**:

| Test file | Validates |
|-----------|-----------|
| `tests/unit/publicAppUrl.test.ts` | **`sanitizeAuthNextPath()`** rejects `/api/auth/session-logout` and `/api/*` post-login targets; preserves safe internal paths like `/exports` |
| `tests/architecture/tenantBoundRequestSurfaces.test.ts` | **`withProspectPoolTenant`** markers on Approvals, Touch-2, and outreach receipt server surfaces |
| `tests/unit/strategicIntelIngress.test.ts` | Irongate DMZ sanitization before Strategic Intel CRM persistence (September 21 manifest bytes — `ironintel-osint-2026-09-21-live`) |
| `tests/unit/fellowsExposureMath.test.ts` | Whole-cent **`estimatedExposureCents`** BigInt floor — **restore file from HEAD if working tree stubbed empty** |
| `tests/unit/fellowsDatabaseUrl.test.ts` | **`academic_fellows`** schema isolation — **restore file from HEAD if stubbed** |
| `tests/unit/fellowsPortal.test.ts` | Fellows host allowlist — **restore file from HEAD if stubbed** |
| `tests/unit/fellowsMissionLessons.test.ts` | Four mission lesson checks — **restore file from HEAD if stubbed** |
...
| `tests/unit/workflowReviewCallAssistCore.test.ts` | SaaS KB first lookup order; capacity entitlement answers (no platform technical ceiling / commercial entitlement capped); Primary Entity + Subtenant enclave framing; banNote SaaS KB citation; SOC 2 / free PoC / HITL pocket Q&A locks |
...
| `IRONFRAME_PROSPECT_POOL_TENANT_UUID` | Prospect pool tenant for unauthenticated sales agent, Approvals CRM scope, and Touch-2 queue (code default **`11111111-1111-4111-8111-111111111111`** — never Medshield fallback after **2026-09-22** delta) |
| `FELLOWS_DATABASE_URL` | Academic Fellowship Postgres URL — must include **`schema=academic_fellows`** or use dedicated DB/role (`ironframe_fellows_app`); never silent share of primary Path B URL without isolation (Mandate 31) |
| `FELLOWS_DIRECT_URL` | Direct connection URL for fellows Prisma migrate (non-pooled) |
| `FELLOWS_PORTAL_PUBLIC_ORIGIN` | Public origin for fellows lab links (default `https://fellows.ironframegrc.com`) |
| `FELLOWS_PORTAL_HOSTS` | Comma-separated extra hosts treated as fellows portal (`isFellowsPortalHost`) |
...
| `PROSPEO_API_KEY` | Ironleads HITL named-buyer work-email enrich on SUSPECT page — never auto-DISPATCH |
| `BRAVE_SEARCH_API_KEY` / `BRAVE_API_KEY` | Ironleads leadership OSINT search — primary provider when site scrape is thin (Research only) |
| `SERPAPI_API_KEY` | Ironleads leadership OSINT search — secondary provider and Brave failover |
| `GOOGLE_CSE_API_KEY` / `GOOGLE_CSE_CX` | Ironleads leadership OSINT search — legacy Google Custom Search JSON API (closed to new GCP customers; paste allowlisted domains from `docs/ops/google-cse-ironleads-sites.txt`) |
...
## ✅ Chapter 9: Daily Writer Receipt (2026-09-22)

**Delta classification:** Structural-adjacent Intel (`Ironboard/src/knowledge/grcProfessionalResearch.manifest.json` September 21 OSINT binding **`ironintel-osint-2026-09-21-live`** — Feature **149** — Linux kernel KEV due-date closure **September 21**, Cisco ISE **CVE-2026-76460** post-deadline triage **day two** on **2026-09-22**, Pixel and Acronis post-deadline attestation, **Chromium V8 CVE-2026-87491** browser-fleet priority, CISA bulletin retirement **September 28**, DARS **2026-O0025 Rev 3** CMMC continuity, Revolut (~**680** records), Metaencryptor Beckman Coulter, Akira Manders (~**70GB** claim), `riskMetricsCents` registers unchanged as string-encoded BigInt) + Backend Logic (Sales **`expectedTouch`** DISPATCH gates and **`salesTouch3Body.ts`** validators; **`withProspectPoolTenant`** completion on Approvals DISPATCH route; **`salesTouch2QueueCore.ts`** pending TOUCH2 draft join; **`ironleadsAccountResearchBrief.ts`** legacy metadata null/rebuild paths; **`salesHumanVoice.ts`** optional peer CTA skip) + UI (`AdminApprovalDashboardClient.tsx` touch filters; **`SalesTouch2QueuePanel.tsx`** card grid + draft deep links) + Ops schedule (`opsScheduleSeedSpecs.ts` video October–November slip; LinkedIn voice-pass ready synopses) + Tests (`tests/unit/ironleadsAccountResearchBrief.test.ts` incomplete gates; `tests/unit/publicAppUrl.test.ts` sanitize cases retained).

**Financial boundary verification:** All ALE references in this document use BigInt integer cents exclusively for persistence and internal telemetry. Constitutional Ironframe seed tenant baselines unchanged: Medshield **1110000000**, Vaultbank **590000000**, Gridcore **470000000**, Defense **1600000000** cents. Design Partner Path B on-ramp documented at **499900** cents via `DESIGN_PARTNER_PATH_B_CENTS` — applies to Touch **1**–**2** EMAIL DISPATCH only. Touch **3** Value Drop EMAIL must not carry Path B economics literals. Planned GA Command **3500000** cents/yr via `PLANNED_GA_COMMAND_CENTS`. Planned GA Growth **7500000** cents/yr via `PLANNED_GA_GROWTH_CENTS`. September 21 OSINT `riskMetricsCents` (workday analysis): **5520000000**, **1388000000**, **1032000000**, **183200000** cents. September 21 OSINT `riskMetricsCents` (SaaS disruption memorandum): **5160000000**, **1456000000**, **1614000000**, **149400000** cents. Industry profile `peerAleBaselineCents` unchanged (Finance **1800000000**, Healthcare **1210000000**, Technology **950000000**, Defense **2500000000**, Public Sector **1500000000**, Manufacturing **1400000000** cents). `readinessScore`, `priorityScore`, and `aiFitnessScore` are not USD cents.

**Threat simulation verification:** Ironintel September 21 manifest (**`ironintel-osint-2026-09-21-live`**) ingested through Irongate DMZ — operational **2026-09-22**: ISE **CVE-2026-76460** post-deadline triage **day two**; kernel **CVE-2025-39964** / **CVE-2026-53266** due-date closure receipts; **CVE-2025-39682** carryover attestation; **Google Pixel CVE-2026-58704** and **Acronis Backup CVE-2026-87886** post-deadline verification; **Chromium V8 CVE-2026-87491** enterprise browser remediation; CISA bulletin sunset **September 28**; Revolut legal-request fraud; Metaencryptor Beckman Coulter; Akira Manders listing; **osint-08** Minnesota PLC **AA26-097A** and Cl0p Windchill **CVE-2026-12569** carryover attestation. No new production `SimulationDiagnosticLog` writes from OSINT ingest. Shadow-plane self-test controls remain gated on `isSimulationMode === true` per TAS Section 4.3.

**Tenant isolation verification:** **`withProspectPoolTenant`** on Approvals DISPATCH mutations binds prospect-pool UUID explicitly. **`PROSPECT_POOL_TENANT_UUID`** **`11111111-1111-4111-8111-111111111111`** — Medshield fallback removed from `resolveProspectPoolTenantId()`.

**Ironleads pipeline verification:** Mandates **28**–**30** retained; Feature **151** hardens incomplete Account Research Brief metadata without cross-tenant bleed.

**GF publication verification:** No GF quarantine promote changes — Mandates **9**, **12**, **14** unchanged.

**Product knowledge verification:** No `commercial.ts` changes — Path B **499900** cents spine unchanged.

**CI verification:** Ingest **`ironintel-osint-2026-09-21-live`** via `npx tsx scripts/ingest-strategic-intel-manifest.ts`. Run `tests/unit/ironleadsAccountResearchBrief.test.ts` and `tests/unit/publicAppUrl.test.ts`. Run `tests/architecture/tenantBoundRequestSurfaces.test.ts` for prospect-pool markers.

**Empty-diff pivot:** Not applicable — `daily_code_diff.txt` contains substantive deltas across Strategic Intel manifest rotation to **September 21** live binding, Sales touch cadence HITL desk, Touch 3 DISPATCH perimeter, Touch 2 queue card pipeline, Account Research Brief resilience, prospect-pool CRM scope on Approvals route, ops video slate reschedule, and unit tests. Primary modules: `grcProfessionalResearch.manifest.json`, `AdminApprovalDashboardClient.tsx`, `SalesTouch2QueuePanel.tsx`, `approvalDraftKinds.ts`, `approvalDispatchValidation.ts`, `app/api/admin/approvals/[id]/route.ts`, `approvalQueueCore.ts`, `approvalNeedsEnrichmentCore.ts`, `salesTouch2QueueCore.ts`, `salesAgentConsoleCore.ts`, `ironleadsAccountResearchBrief.ts`, `ironleadsSuspectReportCore.ts`, `SuspectOperatorEditPanel.tsx`, `opsScheduleSeedSpecs.ts`, `salesHumanVoice.ts`, `tests/unit/ironleadsAccountResearchBrief.test.ts`, `tests/unit/publicAppUrl.test.ts`.

---

*End of GRC Master Operations Manual & Technical Feature Glossary — Writer Narrative Architect complete mandate execution for operational date 2026-09-22.*
