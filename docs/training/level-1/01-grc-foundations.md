# Chapter 1 — GRC Foundations & Command Deck Orientation

> **Track:** LEVEL_1 · **Author agent:** board-trainer · **Release:** `v0.1.0-ga-epic17`
> **Target length:** ~1600 words · **Primary route:** `/integrity`

## Learning objectives

After completing this chapter, you will be able to navigate to `/integrity`, execute the prescribed task flow, and verify INGRESS-001 / AUTH-001 controls using tenant-scoped session context.
...
| Capture route | `/integrity` |
| GRC function IDs | INGRESS-001, AUTH-001 |
...
1. Open browser to http://127.0.0.1:3000/login
2. Authenticate with assigned student credentials
3. Land on /integrity (Integrity Hub) after session gate
4. Observe tripane layout: left 22% · center 48% · right 30%

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | Open browser to http://127.0.0.1:3000/login | /integrity |
| 2 | Authenticate with assigned student credentials | /integrity |
| 3 | Land on /integrity (Integrity Hub) after session gate | /integrity |
| 4 | Observe tripane layout: left 22% · center 48% · right 30% | /integrity |
## Reference screenshot

![Chapter 1 — GRC Foundations & Command Deck Orientation](/docs/training/assets/level-1-01-grc-foundations.png)

*Figure: Chapter 1 — GRC Foundations & Command Deck Orientation — captured at `/integrity`. Asset path: `/docs/training/assets/level-1-01-grc-foundations.png`.*

source-file: public/docs/training/assets/level-1-01-grc-foundations.png
## Hands-on lab sequence

### Lab 1: INGRESS-001

1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Open browser to http://127.0.0.1:3000/login
3. Locate the feature in the SaaS UI at route `/integrity`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Land on /integrity (Integrity Hub) after session gate
3. Locate the feature in the SaaS UI at route `/integrity`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Observe tripane layout: left 22% · center 48% · right 30%
3. Locate the feature in the SaaS UI at route `/integrity`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: INGRESS-001

This procedure validates INGRESS-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Open browser to http://127.0.0.1:3000/login
...
Cross-reference `docs/qa/complete-feature-glossary.md` for INGRESS-001 before submitting the lab.

### Procedure 2: AUTH-001

This procedure validates AUTH-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Authenticate with assigned student credentials
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for AUTH-001 before submitting the lab.

### Procedure 3: INGRESS-001

This procedure validates INGRESS-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Land on /integrity (Integrity Hub) after session gate
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for INGRESS-001 before submitting the lab.
...
This procedure validates AUTH-001 against the live Ironframe workspace at `/integrity`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Observe tripane layout: left 22% · center 48% · right 30%
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for AUTH-001 before submitting the lab.

### Procedure 5: INGRESS-001
...
A: Use primary route `/integrity`, follow the navigation path table in this chapter, and archive screenshot `level-1-01-grc-foundations.png`.
...
- [ ] INGRESS-001 — procedure completed and screenshot archived
...
- source-file: docs/qa/complete-feature-glossary.md
...
ref: config/training-corpus-manifest.json · slug=training/level-1/01-grc-foundations