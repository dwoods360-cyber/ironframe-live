...
- [ ] Confirm Ironframe URL (production or staging)
- [ ] Verify `.env` / Vercel access if you are an operator (not required for end-users)
...
- [ ] Visit **`/dashboard/exports`** — download sample CSV (tenant-scoped)
...
- [ ] Set Vercel env vars per [Technical Requirements](../stakeholders/technical-requirements.md)
- [ ] Enable `IRONWATCH_SUSTAINABILITY_FALLBACK_ENABLED=true` on staging if no Electricity Maps key
- [ ] Run smoke: `npm run test:vercel-smoke` against deploy URL
