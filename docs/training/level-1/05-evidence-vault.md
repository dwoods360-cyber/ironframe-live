...
> **Target length:** ~1600 words · **Primary route:** `/evidence`

## Learning objectives

After completing this chapter, you will be able to navigate to `/evidence`, execute the prescribed task flow, and verify EVID-001 controls using tenant-scoped session context.
...
1. TopNav → Evidence or direct /evidence
2. Review immutable evidence locker entries
3. Cross-reference Epic 12 WORM constraints in docs/TAS.md
4. Complete EXPORT-001 lab: download tenant-scoped export if enabled

## Navigation path (step-by-step)

| Step | Action | Primary route |
|------|--------|---------------|
| 1 | TopNav → Evidence or direct /evidence | /evidence |
| 2 | Review immutable evidence locker entries | /evidence |
| 3 | Cross-reference Epic 12 WORM constraints in docs/TAS.md | /evidence |
| 4 | Complete EXPORT-001 lab: download tenant-scoped export if enabled | /evidence |
## Reference screenshot

![Chapter 5 — Evidence Vault & WORM Ledger Operations](/docs/training/assets/level-1-05-evidence-vault.png)

*Figure: Chapter 5 — Evidence Vault & WORM Ledger Operations — captured at `/evidence`. Asset path: `/docs/training/assets/level-1-05-evidence-vault.png`.*
...
2. TopNav → Evidence or direct /evidence
3. Locate the feature in the SaaS UI at route `/evidence`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Cross-reference Epic 12 WORM constraints in docs/TAS.md
3. Locate the feature in the SaaS UI at route `/evidence`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Complete EXPORT-001 lab: download tenant-scoped export if enabled
3. Locate the feature in the SaaS UI at route `/evidence`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
### Procedure 1: EVID-001

This procedure validates EVID-001 against the live Ironframe workspace at `/evidence`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: TopNav → Evidence or direct /evidence
...
Cross-reference `docs/qa/complete-feature-glossary.md` for EVID-001 before submitting the lab.

### Procedure 2: EVID-001

This procedure validates EVID-001 against the live Ironframe workspace at `/evidence`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Review immutable evidence locker entries
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for EVID-001 before submitting the lab.

### Procedure 3: EVID-001

This procedure validates EVID-001 against the live Ironframe workspace at `/evidence`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Cross-reference Epic 12 WORM constraints in docs/TAS.md
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for EVID-001 before submitting the lab.

### Procedure 4: EVID-001

This procedure validates EVID-001 against the live Ironframe workspace at `/evidence`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Complete EXPORT-001 lab: download tenant-scoped export if enabled
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for EVID-001 before submitting the lab.

### Procedure 5: EVID-001
...
A: Use primary route `/evidence`, follow the navigation path table in this chapter, and archive screenshot `level-1-05-evidence-vault.png`.
...
- [ ] EVID-001 — procedure completed and screenshot archived
...
- source-file: app/(dashboard)/evidence
...
ref: config/training-corpus-manifest.json · slug=training/level-1/05-evidence-vault