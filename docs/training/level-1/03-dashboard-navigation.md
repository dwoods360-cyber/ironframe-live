# Chapter 3 — Dashboard Navigation & TopNav Command Paths

> **Track:** LEVEL_1 · **Author agent:** board-trainer · **Release:** `v0.1.0-ga-epic17`
> **Target length:** ~1600 words · **Primary route:** `/`

## Learning objectives

After completing this chapter, you will be able to navigate to `/`, execute the prescribed task flow, and verify NAV-001 controls using tenant-scoped session context.
...
1. From authenticated session, open TopNav workspace links
2. Visit /integrity, /cockpit, /board-report in sequence
3. Confirm billing gate overlay when entitlement missing
4. Return to /docs for in-app handbook cross-links
...
![Chapter 3 — Dashboard Navigation & TopNav Command Paths](/docs/training/assets/level-1-03-dashboard-navigation.png)

*Figure: Chapter 3 — Dashboard Navigation & TopNav Command Paths — captured at `/integrity`. Asset path: `/docs/training/assets/level-1-03-dashboard-navigation.png`.*

source-file: public/docs/training/assets/level-1-03-dashboard-navigation.png
## Hands-on lab sequence

### Lab 1: NAV-001

1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. From authenticated session, open TopNav workspace links
3. Locate the feature in the SaaS UI at route `/`.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Confirm billing gate overlay when entitlement missing
3. Locate the feature in the SaaS UI at route `/`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
1. Start from authenticated session on tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01`.
2. Return to /docs for in-app handbook cross-links
3. Locate the feature in the SaaS UI at route `/`.
4. Record observations in your lab journal with timestamp 2026-06-18T05:29:31.461Z.
5. Cross-check against source anchors before marking complete.
...
This procedure validates NAV-001 against the live Ironframe workspace at `/`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: From authenticated session, open TopNav workspace links
...
Cross-reference `docs/qa/complete-feature-glossary.md` for NAV-001 before submitting the lab.

### Procedure 2: NAV-001

This procedure validates NAV-001 against the live Ironframe workspace at `/`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Visit /integrity, /cockpit, /board-report in sequence
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for NAV-001 before submitting the lab.
...
This procedure validates NAV-001 against the live Ironframe workspace at `/`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Confirm billing gate overlay when entitlement missing
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for NAV-001 before submitting the lab.
...
This procedure validates NAV-001 against the live Ironframe workspace at `/`.
Begin from tenant `5c420f5a-8f1f-4bbf-b42d-7f8dd4bb6a01` with DORA status COMPLIANT.
Execute: Return to /docs for in-app handbook cross-links
Confirm the UI element appears in the expected tripane column (left 22%, center 48%, or right 30%).
Document the outcome with a timestamp aligned to telemetry mirror 2026-06-18T05:29:31.461Z.
If the route requires elevated roles per route manifest, verify your session includes the correct GRC role assignment.
Cross-reference `docs/qa/complete-feature-glossary.md` for NAV-001 before submitting the lab.
...
A: Use primary route `/`, follow the navigation path table in this chapter, and archive screenshot `level-1-03-dashboard-navigation.png`.
...
- [ ] NAV-001 — procedure completed and screenshot archived
...
- source-file: docs/user-manuals/dashboard-guide.md
...
ref: config/training-corpus-manifest.json · slug=training/level-1/03-dashboard-navigation